        ...
        if oracle_version >= 12:
            if target.name == "did_meta":
                target.append_constraint(
                    CheckConstraint("META IS JSON", "ORACLE_META_JSON_CHK")
                )
            if target.name == "virtual_placements":
                target.append_constraint(
                    CheckConstraint("PLACEMENTS IS JSON", "ORACLE_PLACEMENTS_JSON_CHK")
                )
...
def _add_hint(conn, element, multiparams, params, execution_options):
    if (
        ...
        and isinstance(element, Delete)
        and element.table.name == "locks"
    ):
        element = element.prefix_with("/*+ INDEX(LOCKS LOCKS_PK) */")
    if (
        conn.dialect.name == "oracle"
        and isinstance(element, Delete)
        and element.table.name == "replicas"
    ):
        element = element.prefix_with("/*+ INDEX(REPLICAS REPLICAS_PK) */")
    if (
        conn.dialect.name == "oracle"
        and isinstance(element, Delete)
        and element.table.name == "dids"
    ):
        element = element.prefix_with("/*+ INDEX(DIDS DIDS_PK) */")
    if (
        conn.dialect.name == "oracle"
        and isinstance(element, Delete)
        and element.table.name == "updated_dids"
    ):
        element = element.prefix_with(
            "/*+ INDEX(updated_dids UPDATED_DIDS_SCOPERULENAME_IDX) */"
        )
    if (
        conn.dialect.name == "oracle"
        and isinstance(element, Delete)
        and element.table.name == "tokens"
    ):
...
def _fk_constraint_name(const, table):
    ...
    const.name = "fk_%s_%s_%s" % (table.name, fk.parent.name, reftable)
...
def _ck_constraint_name(const, table):
    ...
    if const.name is None:
        ...
        const.name == "SUBSCRIPTIONS_RETROACTIVE_CHK"
        and table.name.upper() == "SUBSCRIPTIONS_HISTORY"
    ):
        const.name = "SUBS_HISTORY_RETROACTIVE_CHK"
    elif (
        const.name == "SUBSCRIPTIONS_STATE_CHK"
        and table.name.upper() == "SUBSCRIPTIONS_HISTORY"
    ):
        const.name = "SUBS_HISTORY_STATE_CHK"
    elif (
        const.name == "QUARANTINED_REPLICAS_CREATED_NN"
        and table.name.upper() == "QUARANTINED_REPLICAS"
    ):
        const.name = "QURD_REPLICAS_CREATED_NN"
    elif (
        const.name == "QUARANTINED_REPLICAS_UPDATED_NN"
        and table.name.upper() == "QUARANTINED_REPLICAS"
    ):
        const.name = "QURD_REPLICAS_UPDATED_NN"
    elif (
        const.name == "QUARANTINED_REPLICAS_HISTORY_CREATED_NN"
        and table.name.upper() == "QUARANTINED_REPLICAS_HISTORY"
    ):
        const.name = "QURD_REPLICAS_HIST_CREATED_NN"
    elif (
        const.name == "QUARANTINED_REPLICAS_HISTORY_UPDATED_NN"
        and table.name.upper() == "QUARANTINED_REPLICAS_HISTORY"
    ):
        const.name = "QURD_REPLICAS_HIST_UPDATED_NN"
    elif (
        const.name == "ARCHIVE_CONTENTS_HISTORY_CREATED_NN"
        and table.name.upper() == "ARCHIVE_CONTENTS_HISTORY"
    ):
        const.name = "ARCH_CNTS_HIST_CREATED_NN"
    elif (
        const.name == "ARCHIVE_CONTENTS_HISTORY_UPDATED_NN"
        and table.name.upper() == "ARCHIVE_CONTENTS_HISTORY"
    ):
        const.name = "ARCH_CNTS_HIST_UPDATED_NN"
    elif (
        const.name == "ACCOUNT_USAGE_HISTORY_CREATED_NN"
        and table.name.upper() == "ACCOUNT_USAGE_HISTORY"
    ):
        const.name = "ACCOUNT_USAGE_HIST_CREATED_NN"
    elif (
        const.name == "ACCOUNT_USAGE_HISTORY_UPDATED_NN"
        and table.name.upper() == "ACCOUNT_USAGE_HISTORY"
    ):
        const.name = "ACCOUNT_USAGE_HIST_UPDATED_NN"
    elif (
        const.name == "SUBSCRIPTIONS_HISTORY_CREATED_NN"
        and table.name.upper() == "SUBSCRIPTIONS_HISTORY"
    ):
        const.name = "SUBSCRIPTIONS_HIST_CREATED_NN"
    elif (
        const.name == "SUBSCRIPTIONS_HISTORY_UPDATED_NN"
        and table.name.upper() == "SUBSCRIPTIONS_HISTORY"
    ):
    ...
    if const.name == "REQUESTS_TYPE_CHK" and table.name.upper() == "REQUESTS_HISTORY":
        ...
        const.name == "REQUESTS_DIDTYPE_CHK"
        and table.name.upper() == "REQUESTS_HISTORY"
    ):
        ...
        const.name == "REQUESTS_STATE_CHK" and table.name.upper() == "REQUESTS_HISTORY"
    ):
    ...
    def __table_args__(cls):  # pylint: disable=no-self-argument
        ...
        if cls.__tablename__.upper() == "UPDATED_ACCOUNT_COUNTERS":
            return cls._table_args + (
                CheckConstraint(
                    "CREATED_AT IS NOT NULL", "UPDATED_ACCNT_CNTRS_CREATED_NN"
                ),
                CheckConstraint(
                    "UPDATED_AT IS NOT NULL", "UPDATED_ACCNT_CNTRS_UPDATED_NN"
                ),
                {"mysql_engine": "InnoDB"},
            )
        # pylint: disable=maybe-no-member
            ...
            return cls._table_args + (
                CheckConstraint(
                    "CREATED_AT IS NOT NULL", "UPDATED_RSE_CNTRS_CREATED_NN"
                ),
                CheckConstraint(
                    "UPDATED_AT IS NOT NULL", "UPDATED_RSE_CNTRS_UPDATED_NN"
                ),
                {"mysql_engine": "InnoDB"},
            )
        # pylint: disable=maybe-no-member
            ...
            return cls._table_args + (
                CheckConstraint(
                    "CREATED_AT IS NOT NULL", "DIDS_FOLLOWED_EVENTS_CRE_NN"
                ),
                CheckConstraint(
                    "UPDATED_AT IS NOT NULL", "DIDS_FOLLOWED_EVENTS_UPD_NN"
                ),
                {"mysql_engine": "InnoDB"},
            )
...
class Account(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("account", name="ACCOUNTS_PK"),
...
class AccountAttrAssociation(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("account", "key", name="ACCOUNT_ATTR_MAP_PK"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="ACCOUNT_ATTR_MAP_ACCOUNT_FK"
        ),
...
class Identity(BASE, SoftModelBase):
    ...
    email = Column(String(255))
    _table_args = (
        PrimaryKeyConstraint("identity", "identity_type", name="IDENTITIES_PK"),
...
class IdentityAccountAssociation(BASE, ModelBase):
    ...
    account = Column(InternalAccountString(get_schema_value("ACCOUNT_LENGTH")))
    is_default = Column(
        Boolean(name="ACCOUNT_MAP_DEFAULT_CHK", create_constraint=True), default=False
    )
    _table_args = (
        PrimaryKeyConstraint(
            "identity", "identity_type", "account", name="ACCOUNT_MAP_PK"
        ),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="ACCOUNT_MAP_ACCOUNT_FK"
        ),
        ForeignKeyConstraint(
...
class Scope(BASE, ModelBase):
    ...
    account = Column(InternalAccountString(get_schema_value("ACCOUNT_LENGTH")))
    is_default = Column(
        Boolean(name="SCOPES_DEFAULT_CHK", create_constraint=True), default=False
    )
    status = Column(
        Enum(
            ScopeStatus,
            name="SCOPE_STATUS_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        ),
    ...
    deleted_at = Column(DateTime)
    _table_args = (
        PrimaryKeyConstraint("scope", name="SCOPES_SCOPE_PK"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="SCOPES_ACCOUNT_FK"
        ),
...
class DataIdentifier(BASE, ModelBase):
    ...
    is_open = Column(Boolean(name="DIDS_IS_OPEN_CHK", create_constraint=True))
    monotonic = Column(
        Boolean(name="DIDS_MONOTONIC_CHK", create_constraint=True), server_default="0"
    )
    hidden = Column(
        Boolean(name="DIDS_HIDDEN_CHK", create_constraint=True), server_default="0"
    )
    obsolete = Column(
        Boolean(name="DIDS_OBSOLETE_CHK", create_constraint=True), server_default="0"
    )
    complete = Column(
        Boolean(name="DIDS_COMPLETE_CHK", create_constraint=True), server_default=None
    )
    is_new = Column(
        Boolean(name="DIDS_IS_NEW_CHK", create_constraint=True), server_default="1"
    )
    availability = Column(
        Enum(
            DIDAvailability,
            name="DIDS_AVAILABILITY_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        ),
    ...
    suppressed = Column(
        Boolean(name="FILES_SUPP_CHK", create_constraint=True), server_default="0"
    )
    ...
    purge_replicas = Column(
        Boolean(name="DIDS_PURGE_RPLCS_CHK", create_constraint=True), server_default="1"
    )
    deleted_at = Column(DateTime)
    ...
    transient = Column(
        Boolean(name="DID_TRANSIENT_CHK", create_constraint=True), server_default="0"
    )
    accessed_at = Column(DateTime)
    closed_at = Column(DateTime)
    eol_at = Column(DateTime)
    is_archive = Column(Boolean(name="DIDS_ARCHIVE_CHK", create_constraint=True))
    constituent = Column(Boolean(name="DIDS_CONSTITUENT_CHK", create_constraint=True))
    access_cnt = Column(Integer())
    _table_args = (
        PrimaryKeyConstraint("scope", "name", name="DIDS_PK"),
        ForeignKeyConstraint(
            ["account"],
            ["accounts.account"],
            ondelete="CASCADE",
            name="DIDS_ACCOUNT_FK",
        ),
...
class VirtualPlacements(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("scope", "name", name="VP_PK"),
        ForeignKeyConstraint(
            ["scope", "name"], ["dids.scope", "dids.name"], name="VP_FK"
        ),
    )
...
class DidMeta(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("scope", "name", name="DID_META_PK"),
        ForeignKeyConstraint(
            ["scope", "name"], ["dids.scope", "dids.name"], name="DID_META_FK"
        ),
...
class DeletedDataIdentifier(BASE, ModelBase):
    ...
    did_type = Column(
        Enum(
            DIDType,
            name="DEL_DIDS_TYPE_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        )
    )
    is_open = Column(Boolean(name="DEL_DIDS_IS_OPEN_CHK", create_constraint=True))
    monotonic = Column(
        Boolean(name="DEL_DIDS_MONO_CHK", create_constraint=True), server_default="0"
    )
    hidden = Column(
        Boolean(name="DEL_DIDS_HIDDEN_CHK", create_constraint=True), server_default="0"
    )
    obsolete = Column(
        Boolean(name="DEL_DIDS_OBSOLETE_CHK", create_constraint=True),
        server_default="0",
    )
    complete = Column(Boolean(name="DEL_DIDS_COMPLETE_CHK", create_constraint=True))
    is_new = Column(
        Boolean(name="DEL_DIDS_IS_NEW_CHK", create_constraint=True), server_default="1"
    )
    availability = Column(
        Enum(
            DIDAvailability,
            name="DEL_DIDS_AVAIL_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        ),
        default=DIDAvailability.AVAILABLE,
    ...
    suppressed = Column(
        Boolean(name="DEL_FILES_SUPP_CHK", create_constraint=True), server_default="0"
    )
    bytes = Column(BigInteger)
    length = Column(BigInteger)
    md5 = Column(String(32))
    ...
    phys_group = Column(String(25))
    transient = Column(
        Boolean(name="DEL_DID_TRANSIENT_CHK", create_constraint=True),
        server_default="0",
    )
    purge_replicas = Column(
    ...
    closed_at = Column(DateTime)
    eol_at = Column(DateTime)
    is_archive = Column(Boolean(name="DEL_DIDS_ARCH_CHK", create_constraint=True))
    constituent = Column(Boolean(name="DEL_DIDS_CONST_CHK", create_constraint=True))
    access_cnt = Column(Integer())
...
class UpdatedDID(BASE, ModelBase):
    ...
    scope = Column(InternalScopeString(get_schema_value("SCOPE_LENGTH")))
    name = Column(String(get_schema_value("NAME_LENGTH")))
    rule_evaluation_action = Column(
        Enum(
            DIDReEvaluation,
            name="UPDATED_DIDS_RULE_EVAL_ACT_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        )
    )
    _table_args = (
        PrimaryKeyConstraint("id", name="UPDATED_DIDS_PK"),
        CheckConstraint("SCOPE IS NOT NULL", name="UPDATED_DIDS_SCOPE_NN"),
        CheckConstraint("NAME IS NOT NULL", name="UPDATED_DIDS_NAME_NN"),
        Index(
            "UPDATED_DIDS_SCOPERULENAME_IDX", "scope", "rule_evaluation_action", "name"
        ),
    )
...
class BadReplicas(BASE, ModelBase):
    ...
    _table_args = (
        ...
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="BAD_REPLICAS_ACCOUNT_FK"
        ),
...
class BadPFNs(BASE, ModelBase):
    ...
    expires_at = Column(DateTime)
    _table_args = (
        PrimaryKeyConstraint("path", "state", name="BAD_PFNS_PK"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="BAD_PFNS_ACCOUNT_FK"
        ),
    )
...
class QuarantinedReplica(BASE, ModelBase):
    ...
    name = Column(String(get_schema_value("NAME_LENGTH")))
    _table_args = (
        PrimaryKeyConstraint("rse_id", "path", name="QURD_REPLICAS_STATE_PK"),
...
class QuarantinedReplicaHistory(BASE, ModelBase):
    ...
    name = Column(String(get_schema_value("NAME_LENGTH")))
    deleted_at = Column(DateTime)
    __mapper_args__ = {"primary_key": [rse_id, path]}  # Fake primary key for SQLA
    _table_args = ()
...
class DIDKey(BASE, ModelBase):
    ...
    key = Column(String(255))
    is_enum = Column(
        Boolean(name="DID_KEYS_IS_ENUM_CHK", create_constraint=True), server_default="0"
    )
    key_type = Column(
        Enum(
            KeyType,
            name="DID_KEYS_TYPE_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        )
    )
    ...
    _table_args = (
        PrimaryKeyConstraint("key", name="DID_KEYS_PK"),
...
class DIDKeyValueAssociation(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("key", "value", name="DID_KEY_MAP_PK"),
...
class DataIdentifierAssociation(BASE, ModelBase):
    ...
    scope = Column(
        InternalScopeString(get_schema_value("SCOPE_LENGTH"))
    )  # dataset scope
    ...
    guid = Column(GUID())
    events = Column(BigInteger)
    rule_evaluation = Column(
        Boolean(name="CONTENTS_RULE_EVALUATION_CHK", create_constraint=True)
    )
    _table_args = (
        PrimaryKeyConstraint(
            "scope", "name", "child_scope", "child_name", name="CONTENTS_PK"
        ),
        ForeignKeyConstraint(
            ["scope", "name"], ["dids.scope", "dids.name"], name="CONTENTS_ID_FK"
        ),
        ForeignKeyConstraint(
        ...
        Index(
            "CONTENTS_CHILD_SCOPE_NAME_IDX",
            "child_scope",
            "child_name",
            "scope",
            "name",
        ),
        Index("CONTENTS_RULE_EVAL_FB_IDX", "rule_evaluation"),
    )  # Under Oracle this is a FB index
...
class ConstituentAssociation(BASE, ModelBase):
    ...
    child_scope = Column(
        InternalScopeString(get_schema_value("SCOPE_LENGTH"))
    )  # Constituent file scope
    child_name = Column(
        String(get_schema_value("NAME_LENGTH"))
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "child_scope", "child_name", "scope", "name", name="ARCH_CONTENTS_PK"
        ),
        ForeignKeyConstraint(
            ["scope", "name"],
            ["dids.scope", "dids.name"],
...
class DataIdentifierAssociationHistory(BASE, ModelBase):
    ...
    did_type = Column(
        Enum(
            DIDType,
            name="CONTENTS_HIST_DID_TYPE_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        )
    )
    child_type = Column(
        Enum(
            DIDType,
            name="CONTENTS_HIST_CHILD_TYPE_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        )
    )
...
class RSE(BASE, SoftModelBase):
    ...
    deterministic = Column(
        Boolean(name="RSE_DETERMINISTIC_CHK", create_constraint=True), default=True
    )
    volatile = Column(
        Boolean(name="RSE_VOLATILE_CHK", create_constraint=True), default=False
    )
    staging_area = Column(
        Boolean(name="RSE_STAGING_AREA_CHK", create_constraint=True), default=False
    )
    ...
    _table_args = (
        PrimaryKeyConstraint("id", name="RSES_PK"),
...
class RSELimit(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("rse_id", "name", name="RSE_LIMITS_PK"),
...
class RSETransferLimit(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("rse_id", "activity", name="RSE_TRANSFER_LIMITS_PK"),
        ForeignKeyConstraint(
            ["rse_id"], ["rses.id"], name="RSE_TRANSFER_LIMITS_RSE_ID_FK"
        ),
    )
...
class RSEUsage(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("rse_id", "source", name="RSE_USAGE_PK"),
...
class RSEUsageHistory(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "rse_id", "source", "updated_at", name="RSE_USAGE_HISTORY_PK"
        ),
    )
...
class UpdatedRSECounter(BASE, ModelBase):
    ...
    bytes = Column(BigInteger)
    _table_args = (
        PrimaryKeyConstraint("id", name="UPDATED_RSE_CNTRS_PK"),
        ForeignKeyConstraint(
            ["rse_id"], ["rses.id"], name="UPDATED_RSE_CNTRS_RSE_ID_FK"
        ),
...
class RSEAttrAssociation(BASE, ModelBase):
    ...
    value = Column(BooleanString(255))
    _table_args = (
        PrimaryKeyConstraint("rse_id", "key", name="RSE_ATTR_MAP_PK"),
...
class RSEProtocols(BASE, ModelBase):
    ...
    )  # For protocol without host e.g. POSIX on local file systems localhost is assumed as beeing default
    port = Column(
        Integer, server_default="0"
    )  # like host, for local protocol the port 0 is assumed to be default
    prefix = Column(String(1024), nullable=True)
    impl = Column(String(255), nullable=False)
    read_lan = Column(
        Integer, server_default="0"
    )  # if no value is provided, 0 i.e. not supported is assumed as default value
    write_lan = Column(
        Integer, server_default="0"
    )  # if no value is provided, 0 i.e. not supported is assumed as default value
    delete_lan = Column(
    ...
    third_party_copy = Column(
        Integer, server_default="0"
    )  # if no value is provided, 0 i.e. not supported is assumed as default value
    third_party_copy_read = Column(
        Integer, server_default="0"
    )  # if no value is provided, 0 i.e. not supported is assumed as default value
    third_party_copy_write = Column(
        Integer, server_default="0"
    )  # if no value is provided, 0 i.e. not supported is assumed as default value
    extended_attributes = Column(String(4000), nullable=True)
    _table_args = (
        PrimaryKeyConstraint(
            "rse_id", "scheme", "hostname", "port", name="RSE_PROTOCOL_PK"
        ),
...
class RSEQoSAssociation(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("rse_id", "qos_policy", name="RSE_QOS_MAP_PK"),
...
class AccountLimit(BASE, ModelBase):
    ...
    bytes = Column(BigInteger)
    _table_args = (
        PrimaryKeyConstraint("account", "rse_id", name="ACCOUNT_LIMITS_PK"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="ACCOUNT_LIMITS_ACCOUNT_FK"
        ),
...
class AccountGlobalLimit(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "account", "rse_expression", name="ACCOUNT_GLOBAL_LIMITS_PK"
        ),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="ACCOUNT_GLOBAL_LIMITS_ACC_FK"
        ),
    )
...
class AccountUsage(BASE, ModelBase):
    ...
    bytes = Column(BigInteger)
    _table_args = (
        PrimaryKeyConstraint("account", "rse_id", name="ACCOUNT_USAGE_PK"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="ACCOUNT_USAGE_ACCOUNT_FK"
        ),
...
class AccountUsageHistory(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "account", "rse_id", "updated_at", name="ACCOUNT_USAGE_HISTORY_PK"
        ),
    )
...
class RSEFileAssociation(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("scope", "name", "rse_id", name="REPLICAS_PK"),
        ForeignKeyConstraint(
            ["scope", "name"], ["dids.scope", "dids.name"], name="REPLICAS_LFN_FK"
        ),
        ...
        Index(
            "REPLICAS_PATH_IDX", "path", mysql_length=get_schema_value("NAME_LENGTH")
        ),
...
class CollectionReplica(BASE, ModelBase):
    ...
    accessed_at = Column(DateTime)
    _table_args = (
        PrimaryKeyConstraint("scope", "name", "rse_id", name="COLLECTION_REPLICAS_PK"),
        ForeignKeyConstraint(
            ["scope", "name"],
            ["dids.scope", "dids.name"],
        ...
        ForeignKeyConstraint(
            ["rse_id"], ["rses.id"], name="COLLECTION_REPLICAS_RSE_ID_FK"
        ),
...
class UpdatedCollectionReplica(BASE, ModelBase):
    ...
    rse_id = Column(GUID())
    _table_args = (
        PrimaryKeyConstraint("id", name="UPDATED_COL_REP_PK"),
...
class RSEFileAssociationHistory(BASE, ModelBase):
    ...
    bytes = Column(BigInteger)
    _table_args = (
        PrimaryKeyConstraint("rse_id", "scope", "name", name="REPLICAS_HIST_PK"),
...
class ReplicationRule(BASE, ModelBase):
    ...
    locked = Column(
        Boolean(name="RULES_LOCKED_CHK", create_constraint=True), default=False
    )
    ...
    purge_replicas = Column(
        Boolean(name="RULES_PURGE_REPLICAS_CHK", create_constraint=True), default=False
    )
    ignore_availability = Column(
        Boolean(name="RULES_IGNORE_AVAILABILITY_CHK", create_constraint=True),
        default=False,
    )
    ignore_account_limit = Column(
    ...
    eol_at = Column(DateTime)
    split_container = Column(
        Boolean(name="RULES_SPLIT_CONTAINER_CHK", create_constraint=True), default=False
    )
    meta = Column(String(4000))
    _table_args = (
        PrimaryKeyConstraint("id", name="RULES_PK"),
        ForeignKeyConstraint(
            ["scope", "name"], ["dids.scope", "dids.name"], name="RULES_SCOPE_NAME_FK"
        ),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="RULES_ACCOUNT_FK"
        ),
        ForeignKeyConstraint(
            ["subscription_id"], ["subscriptions.id"], name="RULES_SUBS_ID_FK"
        ),
        ForeignKeyConstraint(
            ["child_rule_id"], ["rules.id"], name="RULES_CHILD_RULE_ID_FK"
        ),
        ...
        CheckConstraint(
            "LOCKS_REPLICATING_CNT IS NOT NULL", name="RULES_LOCKS_REPLICATING_CNT_NN"
        ),
        ...
        Index(
            ...
            "copies",
            unique=True,
            mysql_length={"rse_expression": 767},
        ),
        Index("RULES_SCOPE_NAME_IDX", "scope", "name"),
        Index("RULES_EXPIRES_AT_IDX", "expires_at"),
        Index(
            "RULES_STUCKSTATE_IDX", "state"
        ),  # This Index is only needed for the STUCK state, there also is FB: RULES_INJECTSTATE_IDX, RULES_APPROVALSTATE_IDX
...
class ReplicaLock(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("scope", "name", "rule_id", "rse_id", name="LOCKS_PK"),
        ForeignKeyConstraint(["rule_id"], ["rules.id"], name="LOCKS_RULE_ID_FK"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="LOCKS_ACCOUNT_FK"
        ),
...
class DatasetLock(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "scope", "name", "rule_id", "rse_id", name="DATASET_LOCKS_PK"
        ),
        ForeignKeyConstraint(
            ["scope", "name"], ["dids.scope", "dids.name"], name="DATASET_LOCKS_DID_FK"
        ),
        ForeignKeyConstraint(
            ["rule_id"], ["rules.id"], name="DATASET_LOCKS_RULE_ID_FK"
        ),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="DATASET_LOCKS_ACCOUNT_FK"
        ),
...
class UpdatedAccountCounter(BASE, ModelBase):
    ...
    bytes = Column(BigInteger)
    _table_args = (
        PrimaryKeyConstraint("id", name="UPDATED_ACCNT_CNTRS_PK"),
        ForeignKeyConstraint(
            ["rse_id"], ["rses.id"], name="UPDATED_ACCNT_CNTRS_RSE_ID_FK"
        ),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="UPDATED_ACCNT_CNTRS_ACCOUNT_FK"
        ),
...
class Request(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("id", name="REQUESTS_PK"),
        ForeignKeyConstraint(
            ["scope", "name"], ["dids.scope", "dids.name"], name="REQUESTS_DID_FK"
        ),
        ForeignKeyConstraint(["dest_rse_id"], ["rses.id"], name="REQUESTS_RSES_FK"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="REQUESTS_ACCOUNT_FK"
        ),
        CheckConstraint("dest_rse_id IS NOT NULL", name="REQUESTS_RSE_ID_NN"),
        Index(
            "REQUESTS_SCOPE_NAME_RSE_IDX",
            "scope",
            "name",
            "dest_rse_id",
            "request_type",
...
class TransferHop(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            ...
            "next_hop_request_id",
            "initial_request_id",
            name="TRANSFER_HOPS_PK",
        ),
        ForeignKeyConstraint(
            ["initial_request_id"], ["requests.id"], name="TRANSFER_HOPS_INIT_REQ_ID_FK"
        ),
        ForeignKeyConstraint(
            ["request_id"], ["requests.id"], name="TRANSFER_HOPS_REQ_ID_FK"
        ),
        ForeignKeyConstraint(
            ["next_hop_request_id"], ["requests.id"], name="TRANSFER_HOPS_NH_REQ_ID_FK"
        ),
...
class RequestHistory(BASE, ModelBase):
    ...
    did_type = Column(
        Enum(
            DIDType,
            name="REQUESTS_HIST_DIDTYPE_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        ),
        default=DIDType.FILE,
    ...
    priority = Column(Integer)
    transfertool = Column(String(64))
    __mapper_args__ = {"primary_key": [id]}  # Fake primary key for SQLA
    _table_args = (
        Index("REQ_HIST_SCOPE_NAME_RSE_IDX", "scope", "name", "dest_rse_id"),
    )
...
class Source(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "request_id", "rse_id", "scope", "name", name="SOURCES_PK"
        ),
...
class SourceHistory(BASE, ModelBase):
    ...
    ranking = Column(Integer())
    is_using = Column(Boolean(), default=False)
    __mapper_args__ = {"primary_key": [request_id]}  # Fake primary key for SQLA
...
class Distance(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("src_rse_id", "dest_rse_id", name="DISTANCES_PK"),
        ForeignKeyConstraint(["src_rse_id"], ["rses.id"], name="DISTANCES_SRC_RSES_FK"),
        ForeignKeyConstraint(
            ["dest_rse_id"], ["rses.id"], name="DISTANCES_DEST_RSES_FK"
        ),
...
class Subscription(BASE, ModelBase):
    ...
    retroactive = Column(
        Boolean(name="SUBSCRIPTIONS_RETROACTIVE_CHK", create_constraint=True),
        default=False,
    )
    expired_at = Column(DateTime)
    _table_args = (
        PrimaryKeyConstraint("id", name="SUBSCRIPTIONS_PK"),
        UniqueConstraint("name", "account", name="SUBSCRIPTIONS_NAME_ACCOUNT_UQ"),
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="SUBSCRIPTIONS_ACCOUNT_FK"
        ),
        ...
        Index("SUBSCRIPTIONS_STATE_IDX", "state"),
    )  # Under Oracle this is a FB index
...
class SubscriptionHistory(BASE, ModelBase):
    ...
    state = Column(
        Enum(
            SubscriptionState,
            name="SUBSCRIPTIONS_HIST_STATE_CHK",
            create_constraint=True,
            values_callable=lambda obj: [e.value for e in obj],
        ),
        default=SubscriptionState.ACTIVE,
    ...
    comments = Column(String(4000))
    retroactive = Column(
        Boolean(name="SUBS_HISTORY_RETROACTIVE_CHK", create_constraint=True),
        default=False,
    )
    expired_at = Column(DateTime)
...
class Token(BASE, ModelBase):
    ...
    token = Column(
        String(3072)
    )  # account-identity-appid-uuid -> max length: (+ 30 1 255 1 32 1 32)
    account = Column(InternalAccountString(get_schema_value("ACCOUNT_LENGTH")))
    refresh_token = Column(String(315), default=None)
    refresh = Column(
        Boolean(name="TOKENS_REFRESH_CHK", create_constraint=True), default=False
    )
    ...
    )  # scopes define the specific actions applications can be allowed to do on a user's behalf
    identity = Column(String(2048))
    audience = Column(String(315), default=None)
    expired_at = Column(
        DateTime,
        default=lambda: datetime.datetime.utcnow() + datetime.timedelta(seconds=3600),
    )  # one hour lifetime by default
    ip = Column(String(39), nullable=True)
    _table_args = (
        PrimaryKeyConstraint(
            "token", name="TOKENS_TOKEN_PK"
        ),  # not supported for primary key constraint mysql_length=255
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="TOKENS_ACCOUNT_FK"
        ),
...
class OAuthRequest(BASE, ModelBase):
    ...
    expired_at = Column(
        DateTime,
        default=lambda: datetime.datetime.utcnow() + datetime.timedelta(seconds=600),
    )  # 10 min lifetime by default
    _table_args = (
        PrimaryKeyConstraint("state", name="OAUTH_REQUESTS_STATE_PK"),
...
class Message(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("id", name="MESSAGES_ID_PK"),
...
class ConfigHistory(BASE, ModelBase):
    ...
    opt = Column(String(128))
    value = Column(String(4000))
    __mapper_args__ = {"primary_key": [section, opt]}  # Fake primary key for SQLA
    _table_args = ()
...
class Heartbeats(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "executable", "hostname", "pid", "thread_id", name="HEARTBEATS_PK"
        ),
    )
...
class NamingConvention(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("scope", name="NAMING_CONVENTIONS_PK"),
        ForeignKeyConstraint(
            ["scope"], ["scopes.scope"], name="NAMING_CONVENTIONS_SCOPE_FK"
        ),
    )
...
class TemporaryDataIdentifier(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("scope", "name", name="TMP_DIDS_PK"),
...
class LifetimeExceptions(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "id", "scope", "name", "did_type", "account", name="LIFETIME_EXCEPT_PK"
        ),
        ...
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="LIFETIME_EXCEPT_ACCOUNT_FK"
        ),
    )
...
class DidsFollowed(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint("scope", "name", "account", name="DIDS_FOLLOWED_PK"),
        ...
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="DIDS_FOLLOWED_ACCOUNT_FK"
        ),
        ForeignKeyConstraint(
            ["scope", "name"],
            ["dids.scope", "dids.name"],
...
class FollowEvents(BASE, ModelBase):
    ...
    _table_args = (
        PrimaryKeyConstraint(
            "scope", "name", "account", name="DIDS_FOLLOWED_EVENTS_PK"
        ),
        ...
        ForeignKeyConstraint(
            ["account"], ["accounts.account"], name="DIDS_FOLLOWED_EVENTS_ACC_FK"
        ),
