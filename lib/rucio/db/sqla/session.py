...
try:
    if CURRENT_COMPONENT:
        sql_connection = config_get(
            "%s-database" % CURRENT_COMPONENT, "default", check_config_table=False
        ).strip()
...
DEFAULT_SCHEMA_NAME = config_get(
    DATABASE_SECTION,
    "schema",
...
def psql_convert_decimal_to_float(dbapi_conn, connection_rec):
    ...
    try:
        ...
        raise RucioException(
            "Trying to use PostgreSQL without psycopg2 or psycopg2-binary installed!"
        )
...
def get_engine():
    ...
    if not _ENGINE:
        sql_connection = config_get(
            DATABASE_SECTION, "default", check_config_table=False
        )
        config_params = [
            ("pool_size", int),
            ("max_overflow", int),
        ...
        if "mysql" in sql_connection:
            conv = mysql_convert_decimal_to_float(
                pymysql=sql_connection.startswith("mysql+pymysql")
            )
            params["connect_args"] = {"conv": conv}
        for param, param_type in config_params:
            try:
                params[param] = param_type(
                    config_get(DATABASE_SECTION, param, check_config_table=False)
                )
            except:
                pass
        # using sqlAlchemy 2.0 with future=True.
        # if backing up from 2.0, need to remove all the future=True from this
        # file.
...
def get_dump_engine(echo=False):
    ...
    engine = create_engine(
        sql_connection, echo=echo, strategy="mock", executor=dump, future=True
    )
    return engine
...
def read_session(function):
    ...
    @retry(
        retry_on_exception=retry_if_db_connection_error,
        wait_fixed=500,
