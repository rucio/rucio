...
class FTSThrottler(object):
    ...
    def tune(self):
        ...
        if result is not None:
            ...
            for rse in rses:
                # if a rse has a failure ratio above 25% we tune it.
                if rse['failure_ratio'].get('value') > 25:
                    ...
                    print('\033[91m RSE \033[0m' + rse_info[0] + '\033[91m on FTS host \033[0m' + rse_info[1] +
                          '\033[91m has failure ratio \033[0m' + str(rse['failure_ratio'].get('value')) + '\033[91m on storage \033[0m' + url)
        ...
        if result is not None:
            ...
            for rse in rses:
                ...
                if rse['failure_ratio'].get('value') > 25:
                    ...
                    tmp = rse['destination']['hits']['hits'][0]['_source']['payload']['dst-url'].split(':', 2)
                    url = tmp[0] + ':' + tmp[1]
                    print('\033[91m RSE \033[0m' + rse_info[0] + '\033[91m on FTS host \033[0m' + rse_info[1] + '\033[91m has failure ratio \033[0m' + str(rse['failure_ratio'].get('value')) + '\033[91m on storage \033[0m' + url)
...
def fts_throttler(once=False, cycle_interval=3600):
    ...
    # before we stop, try to revert the recent tuning. do not revert if there are other instances
    if heart_beat['nr_threads'] < 2:
        try:
            FTSThrottler().revert()
            pass
