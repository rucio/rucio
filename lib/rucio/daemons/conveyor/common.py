...
def bulk_group_transfers_for_fts(transfers, policy='rule', group_bulk=200, source_strategy=None, max_time_in_queue=None, session=None, logger=logging.log, archive_timeout_override=None):
    ...
    for transfer in chain.from_iterable(transfers):
        ...
        print(f"TIM: activity = {activity}")
        print(f"TIM: transfer = {transfer}")
    ...
    if multi_vo:
        for vo in grouped_transfers:
            for job_key in grouped_transfers[vo]:
                # for all policy groups in job_key, the job_params is the same.
                for policy_key in grouped_transfers[vo][job_key]:
                    job_params = grouped_transfers[vo][job_key][policy_key]['job_params']
                    for xfers_files in chunks(grouped_transfers[vo][job_key][policy_key]['files'], group_bulk):
