...
'''
   OBSOLETE Hermes-Legacy is a daemon to deliver messages: to a messagebroker via STOMP, or emails via SMTP.
'''
...
RECONNECT_COUNTER = METRICS.counter(
    name='reconnect.{host}',
    documentation='Counts Hermes-Legacy reconnects to different ActiveMQ brokers',
    labelnames=('host',)
)
...
def deliver_emails(once: bool = False,
    ...
    run_daemon(
        ...
        executable='hermes-legacy [email]',
        logger_prefix='hermes-legacy_email',
...
def deliver_messages(once: bool = False,
    ...
    run_daemon(
        once=once,
        graceful_stop=graceful_stop,
        executable='hermes-legacy [broker]',
        logger_prefix='hermes-legacy_broker',
        partition_wait_time=1,
        sleep_time=sleep_time,
        run_once_fnc=functools.partial(
            run_once_messages,
...
def run_once_messages(heartbeat_handler: HeartbeatHandler,
    ...
    for broker in brokers_resolved:
        ...
        con.set_listener('rucio-hermes-legacy',
...
def run(once: bool = False,
        ...
        sleep_time: int = 60) -> None:
    '''
    Starts up the hermes-legacy threads.
    '''
    setup_logging()
    ...
    if once:
        logging.info('executing one hermes-legacy iteration only')
        ...
        logging.info('starting hermes-legacy threads')
