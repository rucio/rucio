...
from rucio.core.replica import (list_unlocked_replicas, update_replicas_states,
                                delete_replicas)
from rucio.core.reaper import (get_thread_number,
                               list_rses_for_thread_hash, list_rses_for_thread_sequential)
...
def reaper(rses, worker_number=1, child_number=1, total_children=1, chunk_size=100,
           ...
           dry_mode=False, dynamic=False, map_function='sequential'):
    """
    ...
    :param dynamic: Consider only RSEs with something to delete.
    :param map_function: Method to assign RSEs to threads.
    """
    pid = os.getpid()
    ...
    while not GRACEFUL_STOP.is_set():
        try:
            ...
            if auto:
                ...
                if dynamic:
                    thread_number['dynamic'] = True

                if map_function == 'hash':
                    rses, partitions = list_rses_for_thread_hash(**thread_number)
                elif map_function == 'sequential':
                    rses, partitions = list_rses_for_thread_sequential(**thread_number)
                else:
                    raise NotImplementedError

                if not rses:
                    logging.info('Reaper({0[worker_number]}/{0[child_number]}):'
                                 ' No assigned RSEs. Will sleep a while.'.format(locals()))
                    time.sleep(60)
                    break
                    ...
                    with monitor.record_timer_block('reaper.list_unlocked_replicas'):
                        if auto:
                            ...
                            replicas = list_unlocked_replicas(
                                ...
                                worker_number=partitions[rse['rse']]['rse_thread_number'],
                                total_workers=partitions[rse['rse']]['total_threads_per_rse'],
...
def run(total_workers=1, chunk_size=100, threads_per_worker=None, once=False,
        ...
        delay_seconds=0, auto=False, dry_mode=False, dynamic=False,
        map_function='sequential'):
    """
