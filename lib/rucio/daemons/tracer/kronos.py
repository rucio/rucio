...
def run_once_kronos_file(heartbeat_handler: HeartbeatHandler, stomp_conn_mngr: StompConnectionManager, dataset_queue: Queue, sleep_time: int, **kwargs) -> None:
    ...
    prefetch_size = config_get_int(Config.tracer_kronos.name, Config.tracer_kronos.prefetch_size.name)
    subscription_id = config_get(Config.tracer_kronos.name, Config.tracer_kronos.subscription_id.name)
    try:
        bad_files_patterns = []
    ...
    if not use_ssl:
        username = config_get(Config.tracer_kronos.name, Config.tracer_kronos.username.name)
        password = config_get(Config.tracer_kronos.name, Config.tracer_kronos.password.name)

    excluded_usrdns = set(config_get_list(Config.tracer_kronos.name, Config.tracer_kronos.excluded_usrdns.name))
    vhost = config_get('tracer-kronos', 'broker_virtual_host', raise_exception=False)

    brokers_alias = config_get_list(Config.tracer_kronos.name, Config.tracer_kronos.brokers.name)
    port = config_get_int(Config.tracer_kronos.name, Config.tracer_kronos.port.name)
    reconnect_attempts = config_get_int(Config.tracer_kronos.name, Config.tracer_kronos.reconnect_attempts.name)
    ssl_key_file = config_get(Config.tracer_kronos.name, Config.tracer_kronos.ssl_key_file.name, raise_exception=False)
    ssl_cert_file = config_get(Config.tracer_kronos.name, Config.tracer_kronos.ssl_cert_file.name, raise_exception=False)
    ...
    for conn in created_conns:
        if not conn.is_connected():
            ...
            conn.set_listener('rucio-tracer-kronos', AMQConsumer(broker=conn.transport._Transport__host_and_ports[0],
                                                                 conn=conn,
                                                                 queue=config_get(Config.tracer_kronos.name, Config.tracer_kronos.queue.name),
                                                                 chunksize=chunksize,
