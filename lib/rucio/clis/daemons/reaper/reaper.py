...
Check if the replica exists::
 ...
 ''')
    ...
    parser.add_argument('--dynamic', action='store_true', default=False, help='Consider only RSEs with something to delete')
    parser.add_argument('--map-function', action='store', default='sequential', help='Method to assign RSEs to threads')
    return parser
...
def main():
    ...
    try:
        run(total_workers=args.total_workers, chunk_size=args.chunk_size,
            ...
            auto=args.auto, dry_mode=args.dry_mode, dynamic=args.dynamic,
            map_function=args.map_function)
