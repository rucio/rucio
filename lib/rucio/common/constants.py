...
class RSEAttrObj(str):
    """Representation of the strings used in RSE Attributes. Include the type and a description."""
    def __init__(self, name: str, type_: Union[type[str], type[bool]], doc: str):
        self.name = name
        self.type_ = type_
        self.doc = doc

    def __new__(cls, name, type_, doc) -> 'RSEAttrObj':
...
class RseAttr:
    ...
    AVAILABLE_FOR_MULTIHOP = RSEAttrObj(
        name='available_for_multihop',
        type_=bool,
        doc=(
            "If ``True``, allow this RSE to be used as an intermediate hop in a "
            "multi‑hop transfer.\n"
            "Default: ``False``."
        ),
    )
    ...
    DRIVER_NAME_RSE_ATTRIBUTE = RSEAttrObj(
        name='driver_name_rse_attribute',
        type_=str,
        doc=(
            "Used to specify alternate drivers when using the Bittorrent transfer "
            "manager.\n"
            "Default: ``bittorrent_driver``."
        ),
    )
    ...
    GROUP_BY_RSE_ATTRIBUTE = RSEAttrObj(
        name='group_by_rse_attribute',
        type_=str,
        doc=(
            "Control the RSE attribute (such as ``country``) which transfer source "
    ...
    ISTAPE = RSEAttrObj(
        ...
        type_=bool,
        doc=(
            "Designates the RSE as a tape storage element.\n"
            "Default: ``False``."
        ),
    )
    ...
    OVERWRITE_WHEN_ONLY_ON_DISK = RSEAttrObj(
        name='overwrite_when_only_on_disk',
        type_=bool,
        doc=(
            "On a ``TAPE`` RSE, controls if file can be overwritten. A file may only "
            "be overwritten if it has not yet been written to the tape backend.\n"
            "Default: ``False``."
        ),
    )

    OVERWRITE = RSEAttrObj(
        name='overwrite',
        type_=bool,
        doc=(
            "Controls if a file can be overwritten on the RSE.\n"
            "Default: ``True`` for ``rse_type: DISK``; ``False`` for ``rse_type: TAPE``."
        ),
    )
...
str_like_attrs = [attr for attr in RseAttr.__dict__.values() if isinstance(attr, RSEAttrObj) and attr.type_ == str]
RSE_ATTRS_STR = Literal[str_like_attrs]

bool_like_attrs = [attr for attr in RseAttr.__dict__.values() if isinstance(attr, RSEAttrObj) and attr.type_ == bool]
