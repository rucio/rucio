...
def run_startup_checks(
    ...
    - **Soft time budget**: ``startup_checks.timeout_ms`` sets a soft limit for the whole run.
      Exceeding it logs a warning and execution continues.
    ...
    for check in checks:
        ...
        if soft_timeout_ms is not None and duration_ms > soft_timeout_ms:
            log.warning(
                'Startup check "%s" exceeded soft timeout (%.0f > %d ms)',
                check.name,
                duration_ms,
                soft_timeout_ms,
            )
