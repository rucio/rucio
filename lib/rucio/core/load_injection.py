...
from sqlalchemy.sql.expression import and_, delete, exists, not_, select, update
...
def _affected_rows(result: "Result[Any]") -> int:
    ...
    Narrows the generic SQLAlchemy Result to CursorResult, which is the
    concrete type returned for UPDATE/DELETE statements and the only
    Result subclass exposing rowcount.
    """
...
def _apply_vo_filter(stmt: "Select", vo: "Optional[str]") -> "Select":
    """
