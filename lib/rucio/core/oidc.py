...
def __get_init_oidc_client(token_object: models.Token = None, token_type: str = None, **kwargs) -> dict[Any, Any]:
    ...
    try:
        ...
        if token_object:
            issuer = get_issuer_from_identity_string(str(token_object.identity))
...
def validate_jwt(json_web_token: str, *, session: "Session") -> dict[str, Any]:
    ...
    try:
        ...
        if not token_dict:
            raise CannotAuthenticate(traceback.format_exc())
        issuer = get_issuer_from_identity_string(token_dict['identity'])
        oidc_client = OIDC_CLIENTS[issuer]
...
def get_issuer_from_identity_string(identity_string: str) -> str:
    return identity_string.rsplit('ISS=', 1)[1]
