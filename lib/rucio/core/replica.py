...
def __bulk_add_replicas(rse_id, files, account, *, session: "Session"):
    ...
    available_replicas = [dict([(column, getattr(row, column)) for column in row._fields]) for row in session.execute(stmt)]
    ...
    try:
        ...
        stmt = update(
            ...
            models.RSEFileAssociation.tombstone: case((
                models.RSEFileAssociation.tombstone.not_in([OBSOLETE,
                                                            none_value]),
                accessed_at),
