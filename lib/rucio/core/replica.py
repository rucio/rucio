...
def _list_replicas(dataset_clause, file_clause, state_clause, show_pfns, schemes, files, rse_clause, client_location, domain, sign_urls, session):
                ...
                for tmp_protocol in tmp_protocols[rse]:
                    ...
                    try:
                        ...
                        if sign_urls and protocol.attributes['scheme'] == 'https':
                            sign = get_rse_attribute('sign_url_gcs', rse_info[rse]['id'], session=session)
                            if sign and isinstance(sign, list) and sign[0]:
                                pfn = get_signed_url(service='gcs', operation='read', url=pfn)
...
def list_replicas(dids, schemes=None, unavailable=False, request_id=None,
    ...
    for file in _list_replicas(dataset_clause, file_clause, state_clause, pfns, schemes, files, rse_clause, client_location, domain, sign_urls, session):
        yield file
