...
from rucio.db.sqla.util import (
    create_scope_name_temp_table,
    create_association_temp_table,
)
...
def get_bad_replicas_summary(
    rse_expression=None, from_date=None, to_date=None, filter_=None, session=None
):
    """
    ...
    if rse_expression:
        for rse in parse_expression(
            expression=rse_expression, filter_=filter_, session=session
        ):
            ...
            for chunk in chunks(clause, 10):
                query = (
                    ...
                    .outerjoin(
                        models.BadReplicas,
                        and_(
                            models.RSEFileAssociation.scope == models.BadReplicas.scope,
                ...
                for path, scope, name, rse_id, size, state in query.all():
                    ...
                    return_list.append(
                        (scope, name, path, True, already_declared, size)
                    )
...
def list_bad_replicas_status(
    ...
    for badfile in query.yield_per(1000):
        if badfile.scope.vo == vo:
            if list_pfns:
                result.append(
                    {"scope": badfile.scope, "name": badfile.name, "type": DIDType.FILE}
                )
            else:
                result.append(
                    {
                        "scope": badfile.scope,
                        "name": badfile.name,
                        "rse": get_rse_name(rse_id=badfile.rse_id, session=session),
                        "rse_id": badfile.rse_id,
                        "state": badfile.state,
                        "created_at": badfile.created_at,
                        "updated_at": badfile.updated_at,
                    }
                )
    if list_pfns:
        reps = []
...
def __declare_bad_file_replicas(
    pfns, rse_id, reason, issuer, status=BadFilesStatus.BAD, scheme="srm", session=None
):
    """
    ...
    if len(pfns) > 0 and type(pfns[0]) is str:
        ...
        if rse_info["deterministic"]:
            ...
            for pfn in parsed_pfn:
                ...
                replicas.append(
                    {"scope": scope, "name": name, "rse_id": rse_id, "state": status}
                )
    ...
    for replica in replicas:
        scope, name, rse_id, path = (
            replica["scope"],
            replica["name"],
            replica["rse_id"],
            replica.get("path", None),
        )
        replicas_list.append((scope, name, path))

    for scope, name, path, __exists, already_declared, size in __exist_replicas(
        rse_id=rse_id, replicas=replicas_list, session=session
    ):
        if __exists and (
            (status == BadFilesStatus.BAD and not already_declared)
            or status == BadFilesStatus.SUSPICIOUS
        ):
            declared_replicas.append(
                {
                    "scope": scope,
                    "name": name,
                    "rse_id": rse_id,
            ...
            new_bad_replica = models.BadReplicas(
                scope=scope,
                name=name,
                rse_id=rse_id,
                reason=reason,
            ...
            if already_declared:
                unknown_replicas.append(
                    "%s %s"
                    % (
                        path_pfn_dict.get(path, "%s:%s" % (scope, name)),
                        "Already declared",
                    )
                )
            elif path:
                ...
                for char in str(path):
                    if not isprint(char):
                        unknown_replicas.append(
                            "%s %s" % (path, "PFN contains hidden chars")
                        )
                        no_hidden_char = False
                        break
                if no_hidden_char:
                    unknown_replicas.append(
                        "%s %s" % (path_pfn_dict[path], "Unknown replica")
                    )
...
def add_bad_dids(dids, rse_id, reason, issuer, state=BadFilesStatus.BAD, session=None):
    ...
    for scope, name, _, __exists, already_declared, size in __exist_replicas(
        rse_id=rse_id, replicas=replicas_list, session=session
    ):
        if __exists and not already_declared:
            replicas_for_update.append(
                {
                    "scope": scope,
                    "name": name,
                    "rse_id": rse_id,
            ...
            session.query(models.Source).filter_by(
                scope=scope, name=name, rse_id=rse_id
            ).delete(synchronize_session=False)
        else:
            if already_declared:
                unknown_replicas.append(
                    "%s:%s %s" % (did["scope"], name, "Already declared")
                )
            else:
                unknown_replicas.append(
                    "%s:%s %s" % (did["scope"], name, "Unknown replica")
                )
...
def declare_bad_file_replicas(
    pfns, reason, issuer, status=BadFilesStatus.BAD, session=None
):
    """
    Declare a list of bad replicas.
    ...
    if pfns:
        ...
        if type_ == str:
            scheme, files_to_declare, unknown_replicas = get_pfn_to_rse(
                pfns, vo=issuer.vo, session=session
            )
        else:
            scheme = None
            for pfn in pfns:
        ...
        for rse_id in files_to_declare:
            notdeclared = __declare_bad_file_replicas(
                files_to_declare[rse_id],
                rse_id,
                reason,
                issuer,
...
def get_pfn_to_rse(pfns, vo="def", session=None):
    ...
    for surl in surls:
        if surl.split(":")[0] != scheme:
            raise exception.InvalidType(
                "The PFNs specified must have the same protocol"
            )
        ...
        if hint:
            ...
            for rse_id in protocols:
                for pattern in protocols[rse_id]:
                    if (
                        surl.find(pattern) > -1
                        and get_rse_vo(rse_id=rse_id, session=session) == vo
                    ):
                        mult_rse_match += 1
                        if mult_rse_match > 1:
                            print("ERROR, multiple matches : %s at %s" % (surl, rse_id))
                            raise exception.RucioException(
                                "ERROR, multiple matches : %s at %s"
                                % (surl, get_rse_name(rse_id=rse_id, session=session))
                            )
                        hint = rse_id
...
def get_bad_replicas_backlog(session=None):
    ...
    if session.bind.dialect.name == "oracle":
        ...
        query = (
            ...
            .with_hint(
                models.RSEFileAssociation,
                "INDEX(REPLICAS REPLICAS_STATE_IDX)",
                "oracle",
            )
            .filter(
                text(
                    "CASE WHEN (%sreplicas.state != 'A') THEN %sreplicas.rse_id END IS NOT NULL"
    ...
    query = (
        query.join(
            ...
            and_(
                models.DataIdentifier.scope == models.RSEFileAssociation.scope,
...
def list_bad_replicas(
    limit=10000, thread=None, total_threads=None, rses=None, session=None
):
    """
    ...
    query = filter_thread_work(
        ...
        thread_id=thread,
        hash_variable="%sreplicas.name" % (schema_dot),
    )
    query = query.join(
        models.DataIdentifier,
        and_(
...
def get_did_from_pfns(pfns, rse_id=None, vo="def", session=None):
    ...
    dict_rse = {}
    if not rse_id:
        scheme, dict_rse, unknown_replicas = get_pfn_to_rse(
            pfns, vo=vo, session=session
        )
        if unknown_replicas:
            raise Exception
    else:
    ...
    for rse_id in dict_rse:
        ...
        if rse_info["deterministic"]:
            ...
            for pfn in parsed_pfn:
                ...
                condition.append(
                    and_(
                        models.RSEFileAssociation.path == path,
                        models.RSEFileAssociation.rse_id == rse_id,
                    )
                )
            for scope, name, pfn in session.query(
                models.RSEFileAssociation.scope,
                models.RSEFileAssociation.name,
                models.RSEFileAssociation.path,
...
def _resolve_dids(
    dids, unavailable, ignore_availability, all_states, resolve_archives, session
):
    """
    ...
    files_wo_replica = []
    for did in [
        dict(tupleized) for tupleized in set(tuple(item.items()) for item in dids)
    ]:
        if (
            "type" in did
            and did["type"] in (DIDType.FILE, DIDType.FILE.value)
            or "did_type" in did
            and did["did_type"] in (DIDType.FILE, DIDType.FILE.value)
        ):  # pylint: disable=no-member
            files_wo_replica.append({"scope": did["scope"], "name": did["name"]})
            file_clause.append(
                and_(
                    models.RSEFileAssociation.scope == did["scope"],
    ...
    if did_clause:
        for scope, name, did_type, constituent in (
            session.query(
            ...
            if resolve_archives and constituent:
                constituent_clause.append(
                    and_(
                        models.ConstituentAssociation.child_scope == scope,
            ...
            if did_type == DIDType.FILE:
                ...
                content_query = content_query.with_hint(
...
def _list_replicas_for_datasets(
    ...
    if updated_after:
        replica_query = replica_query.filter(
            models.RSEFileAssociation.updated_at >= updated_after
        )
...
def _list_replicas_for_constituents(
    ...
    if not ignore_availability:
        constituent_query = constituent_query.filter(
            models.RSE.availability.in_((4, 5, 6, 7))
        )
    ...
    for replica in constituent_query.yield_per(500):
        ...
        {"scope": scope, "name": name} in files_wo_replica and files_wo_replica.remove(
            {"scope": scope, "name": name}
        )
        yield replica


def _list_replicas_for_files(
    file_clause,
    state_clause,
    files_wo_replica,
    rse_clause,
    ...
    for replica_condition in chunks(file_clause, 50):
        ...
        replica_query = (
            ...
            .with_hint(
                models.RSEFileAssociation,
                text="INDEX(REPLICAS REPLICAS_PK)",
                dialect_name="oracle",
            )
        )
        ...
        for (
            ...
            rse_type,
            volatile,
        ) in replica_query.all():
            {
                "scope": scope,
                "name": name,
            } in files_wo_replica and files_wo_replica.remove(
                {"scope": scope, "name": name}
            )
            yield scope, name, None, None, bytes_, md5, adler32, path, state, rse_id, rse, rse_type, volatile
...
def _list_files_wo_replicas(files_wo_replica, session):
    if files_wo_replica:
        ...
        for file in sorted(files_wo_replica, key=lambda f: (f["scope"], f["name"])):
            file_wo_clause.append(
                and_(
                    models.DataIdentifier.scope == file["scope"],
                    models.DataIdentifier.name == file["name"],
                )
            )
        files_wo_replicas_query = (
            session.query(
                models.DataIdentifier.scope,
                models.DataIdentifier.name,
            ...
            .filter(or_(*file_wo_clause))
            .with_hint(
                models.DataIdentifier, text="INDEX(DIDS DIDS_PK)", dialect_name="oracle"
            )
        )
    ...
    if x_caches is NO_VALUE:
        try:
            response = requests.get(
                "{}/serverRanges".format(vp_endpoint), timeout=1, verify=False
            )
            if response.ok:
            ...
            logger(
                logging.WARNING,
                "In get_multi_cache_prefix, could not access {}. Excaption:{}".format(
                    vp_endpoint, re
                ),
            )
            return ""
...
def _sort_replica_file_pfns(file):
    ...
    tmp = sorted(
        [
            (file["pfns"][p]["domain"], file["pfns"][p]["priority"], p)
            for p in file["pfns"]
        ]
    )
    ...
    file["rses"] = {}
    rse_pfns = sorted(
        (file["pfns"][pfn]["rse_id"], file["pfns"][pfn]["priority"], pfn)
...
def _list_replicas(
    ...
    for (
        ...
        if show_pfns and rse_id:
            ...
            rse_info[rse_id]["priority_wan"] = {
                p["scheme"]: p["domains"]["wan"]["read"]
                for p in rse_info[rse_id]["protocols"]
                if p["domains"]["wan"]["read"] > 0
            }
            rse_info[rse_id]["priority_lan"] = {
                p["scheme"]: p["domains"]["lan"]["read"]
                for p in rse_info[rse_id]["protocols"]
                if p["domains"]["lan"]["read"] > 0
            }
            ...
            if rse_id not in tmp_protocols:
                ...
                if not rse_schemes:
                    try:
                        if domain == "all":
                            rse_schemes.append(
                                rsemgr.select_protocol(
                                    rse_settings=rse_info[rse_id],
                                    operation="read",
                                    domain="wan",
                                )["scheme"]
                            )
            ...
            for tmp_protocol in tmp_protocols[rse_id]:
                ...
                if "determinism_type" in protocol.attributes:  # PFN is cachable
                    try:
                        path = pfns_cache[
                            "%s:%s:%s"
                            % (
                                protocol.attributes["determinism_type"],
                                t_scope.internal,
                                t_name,
                            )
                        ]
                ...
                try:
                    ...
                    if sign_urls and protocol.attributes["scheme"] == "https":
                        service = get_rse_attribute(
                            "sign_url", rse_id=rse_id, session=session
                        )
                    ...
                    if (
                        domain == "wan"
                        and protocol.attributes["scheme"] in ["root", "http", "https"]
                        and client_location
                    ):

                        if "site" in client_location and client_location["site"]:
                            # is the RSE site-configured?
                            rse_site_attr = get_rse_attribute(
                                "site", rse_id, session=session
                            )
                            replica_site = [""]
                            ...
                            if client_location["site"] != replica_site:
                                ...
                                if cache_site != "":
                                    ...
                                    # print('filename', name)
                                    selected_prefix = get_multi_cache_prefix(
                                        cache_site, t_name
                                    )
                                    if selected_prefix:
                                        pfn = (
                                    ...
                                    if root_proxy_internal:
                                        ...
                                        if (
                                            "storage.googleapis.com" in pfn
                                            or "atlas-google-cloud.cern.ch" in pfn
                                            or "amazonaws.com" in pfn
                                        ):
                                            pass  # ATLAS HACK
                                        else:
                ...
                if protocol.attributes["scheme"] == "srm":
                    try:
                        file["space_token"] = protocol.attributes[
                            "extended_attributes"
                        ]["space_token"]
                    except KeyError:
        ...
        if (
            "scope" in file
            and "name" in file
            and not (file["scope"] == scope and file["name"] == name)
        ):
        ...
        if resolve_parents and not file.get("parents"):
            file["parents"] = [
                "%s:%s" % (parent["scope"].internal, parent["name"])
                for parent in rucio.core.did.list_all_parent_dids(
    ...
    for scope, name, bytes_, md5, adler32 in _list_files_wo_replicas(
        files_wo_replica, session
    ):
        yield {
            "scope": scope,
            "name": name,
...
def __bulk_add_new_file_dids(files, account, dataset_meta=None, session=None):
    ...
    for file in files:
        ...
        if "meta" in file and file["meta"]:
            rucio.core.did.set_metadata_bulk(
                scope=file["scope"],
                name=file["name"],
                meta=file["meta"],
                recursive=False,
    ...
    try:
        ...
        if (
            match(
                ".*IntegrityError.*02291.*integrity constraint.*DIDS_SCOPE_FK.*violated - parent key not found.*",
                error.args[0],
            )
            ...
            or match(
                '.*IntegrityError.*insert or update on table.*violates foreign key constraint "DIDS_SCOPE_FK".*',
                error.args[0],
            )
            or match(
                ".*ForeignKeyViolation.*insert or update on table.*violates foreign key constraint.*",
                error.args[0],
            )
            or match(
                ".*IntegrityError.*foreign key constraints? failed.*", error.args[0]
            )
        ):
        ...
        if match(
            ".*(DatabaseError).*ORA-14400.*inserted partition key does not map to any partition.*",
            error.args[0],
        ):
            raise exception.ScopeNotFound("Scope not found!")
        ...
        if match(
            ...
            raise exception.DataIdentifierAlreadyExists(
...
def __bulk_add_file_dids(files, account, dataset_meta=None, session=None):
    ...
    for f in files:
        condition.append(
            and_(
                models.DataIdentifier.scope == f["scope"],
    ...
    available_files = [
        dict([(column, getattr(row, column)) for column in row._fields]) for row in q
    ]
    new_files = list()
    for file in files:
        found = False
        for available_file in available_files:
            if (
                file["scope"] == available_file["scope"]
                and file["name"] == available_file["name"]
            ):
                found = True
                break
        if not found:
    ...
    __bulk_add_new_file_dids(
        files=new_files, account=account, dataset_meta=dataset_meta, session=session
    )
...
def __bulk_add_replicas(rse_id, files, account, session=None):
    ...
    for f in files:
        condition.append(
            and_(
                models.RSEFileAssociation.scope == f["scope"],
    ...
    available_replicas = [
        dict([(column, getattr(row, column)) for column in row._fields])
        for row in query
    ]

    default_tombstone_delay = next(
        iter(get_rse_attribute("tombstone_delay", rse_id=rse_id, session=session)), None
    )
    ...
    for file in files:
        ...
        for available_replica in available_replicas:
            if (
                file["scope"] == available_replica["scope"]
                and file["name"] == available_replica["name"]
                and rse_id == available_replica["rse_id"]
            ):
                found = True
                break
        if not found:
    ...
    try:
        new_replicas and session.bulk_insert_mappings(
            models.RSEFileAssociation, new_replicas
        )
        session.flush()
        ...
        if (
            match(
                ".*IntegrityError.*ORA-00001: unique constraint .*REPLICAS_PK.*violated.*",
                error.args[0],
            )
            or match(".*IntegrityError.*1062.*Duplicate entry.*", error.args[0])
            or match(
                ".*IntegrityError.*columns? rse_id.*scope.*name.*not unique.*",
                error.args[0],
            )
            or match(
...
@transactional_session
def add_replicas(
    rse_id, files, account, ignore_availability=True, dataset_meta=None, session=None
):
    """
    ...
    def _expected_pfns(
        lfns, rse_settings, scheme, operation="write", domain="wan", protocol_attr=None
    ):
        p = rsemgr.create_protocol(
    ...
    if replica_rse.volatile is True:
        raise exception.UnsupportedOperation(
            "Cannot add replicas on volatile RSE %s " % (replica_rse.rse)
        )

    if not (replica_rse.availability & 2) and not ignore_availability:
        raise exception.ResourceTemporaryUnavailable(
            "%s is temporary unavailable for writing" % replica_rse.rse
        )
    ...
    for file in files:
        if "pfn" not in file:
            if not replica_rse.deterministic:
                raise exception.UnsupportedOperation(
                    "PFN needed for this (non deterministic) RSE %s "
                    % (replica_rse.rse)
                )

    replicas = __bulk_add_file_dids(
        files=files, account=account, dataset_meta=dataset_meta, session=session
    )
    ...
    if pfns:
        ...
        for scheme in pfns.keys():
            if not replica_rse.deterministic:
                p = rsemgr.create_protocol(
                    rse_settings=rse_settings, operation="write", scheme=scheme
                )
                ...
                lfns = [
                    {"scope": i["scope"].external, "name": i["name"]}
                    for i in files
                    if i["pfn"].startswith(scheme)
                ]
                pfns[scheme] = clean_surls(pfns[scheme])

                for protocol_attr in rsemgr.get_protocols_ordered(
                    rse_settings=rse_settings,
                    operation="write",
                    scheme=scheme,
                    domain="wan",
                ...
                if len(pfns[scheme]) > 0:
                    ...
                    raise exception.InvalidPath(
                        "One of the PFNs provided does not match the Rucio expected PFN : %s (%s)"
                        % (str(pfns[scheme]), str(lfns))
                    )

    nbfiles, bytes_ = __bulk_add_replicas(
        rse_id=rse_id, files=files, account=account, session=session
    )
...
def delete_replicas(rse_id, files, ignore_availability=True, session=None):
    ...
    if use_temp_tables:
        __delete_replicas(
            rse_id, files, ignore_availability=ignore_availability, session=session
        )
    else:
        __delete_replicas_without_temp_tables(
            rse_id, files, ignore_availability=ignore_availability, session=session
        )
...
def __delete_replicas(rse_id, files, ignore_availability=True, session=None):
    ...
    if not (replica_rse.availability & 1) and not ignore_availability:
        raise exception.ResourceTemporaryUnavailable(
            "%s is temporary unavailable" "for deleting" % replica_rse.rse
        )

    scope_name_temp_table = create_scope_name_temp_table(
        "delete_replicas", session=session
    )
    scope_name_temp_table2 = create_scope_name_temp_table(
        "delete_replicas2", session=session
    )
    association_temp_table = create_association_temp_table(
        "delete_replicas_association", session=session
    )

    session.bulk_insert_mappings(
        scope_name_temp_table,
        [{"scope": file["scope"], "name": file["name"]} for file in files],
    )
    ...
    stmt = (
        ...
        .where(
            exists(
                select([1]).where(
                    and_(
                        models.Source.scope == scope_name_temp_table.scope,
    ...
    stmt = select(func.count(), func.sum(models.RSEFileAssociation.bytes),).join_from(
        scope_name_temp_table,
        models.RSEFileAssociation,
        and_(
            models.RSEFileAssociation.scope == scope_name_temp_table.scope,
    ...
    stmt = (
        ...
        .where(
            exists(
                select([1]).where(
                    and_(
                        models.BadReplicas.scope == scope_name_temp_table.scope,
        ...
        .execution_options(synchronize_session=False)
        .values(state=BadFilesStatus.DELETED, updated_at=datetime.utcnow())
    )
    res = session.execute(stmt)

    __cleanup_after_replica_deletion(
        scope_name_temp_table=scope_name_temp_table,
...
def __cleanup_after_replica_deletion(
    ...
    :param session: The database session in use.
    """
    clt_to_update, parents_to_analyze, affected_archives, clt_replicas_to_delete = (
        set(),
        set(),
        set(),
        set(),
        ...
        did_condition.append(
            and_(
                ...
                ~exists(
                    ...
                    and_(
                        models.RSEFileAssociation.scope == file["scope"],
    ...
    session.bulk_insert_mappings(
        scope_name_temp_table, [sn._asdict() for sn in clt_to_update]
    )
    stmt = (
        select(
            models.DataIdentifierAssociation.scope,
        ...
        .join_from(
            ...
            models.DataIdentifierAssociation,
            and_(
                scope_name_temp_table.scope
                == models.DataIdentifierAssociation.child_scope,
                scope_name_temp_table.name
                == models.DataIdentifierAssociation.child_name,
            ),
        )
        .join(
    ...
    while parents_to_analyze:
        ...
        session.query(scope_name_temp_table).delete()
        session.bulk_insert_mappings(
            scope_name_temp_table, [sn._asdict() for sn in parents_to_analyze]
        )
        ...
        stmt = (
            ...
            .where(models.DataIdentifier.scope == null())
            .outerjoin(
                models.RSEFileAssociation,
                and_(
                    models.RSEFileAssociation.scope
            ...
            .where(models.RSEFileAssociation.scope == null())
            .outerjoin(
                models.ConstituentAssociation,
                and_(
                    models.ConstituentAssociation.child_scope
            ...
            .where(models.ConstituentAssociation.child_scope == null())
        )
        ...
        for (
            ...
            did_associations_to_remove.add(
                Association(
                    scope=parent_scope,
                    name=parent_name,
            ...
            clt_to_set_not_archive[-1].add(
                ScopeName(scope=parent_scope, name=parent_name)
            )
            ...
            remove_open_did = config_get(
                "reaper", "remove_open_did", default=False, session=session
            )
            if remove_open_did:
                did_condition.append(
                    and_(
                        models.DataIdentifier.scope == parent_scope,
                        ...
                        ~exists([1]).where(
                            and_(
                                models.DataIdentifierAssociation.scope == parent_scope,
        ...
        if did_associations_to_remove:
            ...
            session.bulk_insert_mappings(
                association_temp_table,
                [a._asdict() for a in did_associations_to_remove],
            )
            ...
            stmt = (
                ...
                .join_from(
                    ...
                    models.DataIdentifier,
                    and_(
                        association_temp_table.scope == models.DataIdentifier.scope,
                        association_temp_table.name == models.DataIdentifier.name,
                    ),
                )
                .where(
                    or_(
                        models.DataIdentifier.complete == true(),
            ...
            for parent_scope, parent_name, parent_did_type in session.execute(stmt):
                ...
                if message not in messages:
                    ...
                    incomplete_dids.append(
                        ScopeName(scope=parent_scope, name=parent_name)
                    )

            content_to_delete_filter = exists(
                select([1]).where(
                    and_(
                        association_temp_table.scope
                        == models.DataIdentifierAssociation.scope,
                        association_temp_table.name
                        == models.DataIdentifierAssociation.name,
                        association_temp_table.child_scope
                        == models.DataIdentifierAssociation.child_scope,
                        association_temp_table.child_name
                        == models.DataIdentifierAssociation.child_name,
                    )
                )
            )

            rucio.core.did.insert_content_history(
                filter_=content_to_delete_filter, did_created_at=None, session=session
            )

            stmt = (
                delete(models.DataIdentifierAssociation)
                .where(
    ...
    session.query(scope_name_temp_table).delete()
    session.bulk_insert_mappings(
        scope_name_temp_table, [sn._asdict() for sn in clt_replicas_to_delete]
    )
    ...
    stmt = (
        ...
        .join_from(
            ...
            models.CollectionReplica,
            and_(
                scope_name_temp_table.scope == models.CollectionReplica.scope,
        ...
        .join(
            models.DataIdentifier,
            and_(
                models.DataIdentifier.scope == models.CollectionReplica.scope,
        ...
        .where(models.DataIdentifierAssociation.scope == null())
    )
    ...
    stmt = (
        ...
        .where(
            exists(
                select([1]).where(
                    and_(
                        models.CollectionReplica.scope == scope_name_temp_table2.scope,
    ...
    session.query(scope_name_temp_table).delete()
    session.bulk_insert_mappings(
        scope_name_temp_table, [sn._asdict() for sn in incomplete_dids]
    )
    stmt = (
        ...
        .where(
            exists(
                select([1]).where(
                    and_(
                        models.DataIdentifier.scope == scope_name_temp_table.scope,
                ...
                messages.append(
                    {
                        "event_type": "ERASE",
                        "payload": dumps(
                            {"scope": scope.external, "name": name, "account": "root"}
                        ),
                    }
                )
    ...
    session.query(scope_name_temp_table).delete()
    session.bulk_insert_mappings(
        scope_name_temp_table, [sn._asdict() for sn in affected_archives]
    )

    stmt = (
        select(models.ConstituentAssociation)
        .distinct()
        .join_from(
            scope_name_temp_table,
            models.ConstituentAssociation,
            and_(
                scope_name_temp_table.scope == models.ConstituentAssociation.scope,
        ...
        .outerjoin(
            models.RSEFileAssociation,
            and_(
                models.RSEFileAssociation.scope == models.ConstituentAssociation.scope,
    ...
    for constituent in session.execute(stmt).scalars().all():
        constituent_associations_to_delete.add(
            Association(
    ...
    if constituent_associations_to_delete:
        ...
        session.bulk_insert_mappings(
            association_temp_table,
            [a._asdict() for a in constituent_associations_to_delete],
        )
        stmt = (
            delete(models.ConstituentAssociation)
            .where(
                exists(
                    select([1]).where(
                        and_(
        ...
        removed_constituents = {
            ScopeName(scope=c.child_scope, name=c.child_name)
            for c in constituent_associations_to_delete
        }
        for chunk in chunks(removed_constituents, 200):
            __cleanup_after_replica_deletion(
                ...
                association_temp_table=association_temp_table,
                rse_id=rse_id,
                files=[sn._asdict() for sn in chunk],
                session=session,
            )

    session.query(scope_name_temp_table).delete()
    session.bulk_insert_mappings(
        scope_name_temp_table, [sn._asdict() for sn in dids_to_delete]
    )
    ...
    stmt = (
        ...
        .where(
            exists(
                select([1]).where(
                    and_(
                        models.ReplicationRule.scope == scope_name_temp_table.scope,
        ...
        .where(
            models.ReplicationRule.state.in_(
                (RuleState.SUSPENDED, RuleState.WAITING_APPROVAL)
            )
        )
        .execution_options(synchronize_session=False)
    )
        ...
        stmt = (
            ...
            .where(
                exists(
                    select([1]).where(
                        and_(
                            models.DidMeta.scope == scope_name_temp_table.scope,
    ...
    archive_dids = config_core.get(
        "deletion", "archive_dids", default=False, session=session
    )
    if archive_dids:
        rucio.core.did.insert_deleted_dids(
            filter_=dids_to_delete_filter, session=session
        )
    stmt = (
        delete(
            models.DataIdentifier,
    ...
    while clt_to_set_not_archive:
        session.query(scope_name_temp_table).delete()
        session.bulk_insert_mappings(
            scope_name_temp_table, [sn._asdict() for sn in clt_to_set_not_archive[0]]
        )
        session.query(scope_name_temp_table2).delete()
        ...
        stmt = (
            ...
            .distinct()
            .where(models.DataIdentifier.is_archive == true())
            .join_from(
                scope_name_temp_table,
                models.DataIdentifier,
                and_(
                    scope_name_temp_table.scope == models.DataIdentifier.scope,
            ...
            .outerjoin(
                ...
                and_(
                    data_identifier_alias.scope
                    == models.DataIdentifierAssociation.child_scope,
                    data_identifier_alias.name
                    == models.DataIdentifierAssociation.child_name,
                    data_identifier_alias.is_archive == true(),
                ),
            )
            .where(data_identifier_alias.scope == null())
        )
        session.execute(
        ...
        stmt = (
            ...
            .where(
                exists(
                    select([1]).where(
                        and_(
                            models.DataIdentifier.scope == scope_name_temp_table2.scope,
...
@transactional_session
def __delete_replicas_without_temp_tables(
    rse_id, files, ignore_availability=True, session=None
):
    """
    Delete file replicas.
    ...
    for file in files:
        ...
        src_condition.append(
            and_(
                models.Source.scope == file["scope"],
        ...
        bad_replica_condition.append(
            and_(
                models.BadReplicas.scope == file["scope"],
    ...
    for chunk in chunks(replica_condition, 10):
        for (scope, name, rid, replica_bytes) in (
            session.query(
                models.RSEFileAssociation.scope,
                models.RSEFileAssociation.name,
                models.RSEFileAssociation.rse_id,
        ...
        stmt = (
            delete(models.RSEFileAssociation)
    ...
    __cleanup_after_replica_deletion_without_temp_table(
...
def __cleanup_after_replica_deletion_without_temp_table(rse_id, files, session=None):
    ...
    (
        ...
        messages,
        clt_is_not_archive_condition,
        archive_contents_condition,
    ) = ([], [], [], [])
    for file in files:
        ...
        dst_replica_condition.append(
            and_(
                ...
                exists(
                    select([1]).prefix_with(
                        "/*+ INDEX(COLLECTION_REPLICAS COLLECTION_REPLICAS_PK) */",
                        dialect="oracle",
                    )
                ).where(
                    and_(
        ...
        archive_contents_condition.append(
            and_(
                models.ConstituentAssociation.scope == file["scope"],
    ...
    while parent_condition:
        ...
        for chunk in chunks(parent_condition, 10):
            ...
            for (
                ...
                clt_replica_condition.append(
                    and_(
                        models.CollectionReplica.scope == parent_scope,
                        ...
                        ~exists(
                            select([1]).prefix_with(
                                "/*+ INDEX(CONTENTS CONTENTS_PK) */", dialect="oracle"
                            )
                        ).where(
                            and_(
                                models.DataIdentifierAssociation.scope == parent_scope,
                ...
                tmp_parent_condition.append(
                    and_(
                        models.DataIdentifierAssociation.child_scope == parent_scope,
        ...
        if child_did_condition:
            ...
            for chunk in chunks(child_did_condition, 10):
                rucio.core.did.insert_content_history(
                    filter_=or_(*chunk), did_created_at=None, session=session
                )
    ...
    for chunk in chunks(clt_replica_condition, 10):
        session.query(models.CollectionReplica).filter(or_(*chunk)).delete(
    ...
    for chunk in chunks(did_condition, 10):
        ...
        for scope, name, did_type in query:
            ...
            if session.bind.dialect.name == "oracle":
                oracle_version = int(
                    session.connection().connection.version.split(".")[0]
                )
                if oracle_version >= 12:
                    deleted_did_meta.append(
    ...
    for chunk in chunks(archive_contents_condition, 30):
        query = (
            ...
            .with_hint(
                models.ConstituentAssociation,
                "INDEX(ARCHIVE_CONTENTS ARCH_CONTENTS_CHILD_IDX)",
                "oracle",
            )
            .filter(or_(*chunk))
        )
        for constituent in query:
            removed_constituents.append(
                {"scope": constituent.child_scope, "name": constituent.child_name}
            )
            constituents_to_delete_condition.append(
                and_(
                    models.ConstituentAssociation.scope == constituent.scope,
                    models.ConstituentAssociation.name == constituent.name,
                    models.ConstituentAssociation.child_scope
                    == constituent.child_scope,
    ...
    if constituents_to_delete_condition:
        stmt = (
            delete(models.ConstituentAssociation)
            .prefix_with(
                "/*+ INDEX(ARCHIVE_CONTENTS ARCH_CONTENTS_PK) */", dialect="oracle"
            )
            .where(or_(*constituents_to_delete_condition))
            .execution_options(synchronize_session=False)
        )
    ...
    for chunk in chunks(deleted_did_meta, 100):
        session.query(models.DidMeta).filter(or_(*chunk)).delete(
            synchronize_session=False
        )
...
def get_replica(rse_id, scope, name, session=None):
    ...
    try:
        ...
        raise exception.ReplicaNotFound(
            "No row found for scope: %s name: %s rse: %s"
            % (scope, name, get_rse_name(rse_id=rse_id, session=session))
        )
...
def list_and_mark_unlocked_replicas(
    ...
    temp_table_cls = create_scope_name_temp_table(
        "list_and_mark_unlocked_replicas", session=session
    )
    ...
    stmt = (
        ...
        .with_hint(
            models.RSEFileAssociation,
            "INDEX_RS_ASC(replicas REPLICAS_TOMBSTONE_IDX)  NO_INDEX_FFS(replicas REPLICAS_TOMBSTONE_IDX)",
            "oracle",
        )
        .where(
            ...
            else models.RSEFileAssociation.tombstone < datetime.utcnow(),
        )
        .where(
            or_(
        ...
        .outerjoin(
            models.Source,
            and_(
                models.RSEFileAssociation.scope == models.Source.scope,
                models.RSEFileAssociation.name == models.Source.name,
                models.RSEFileAssociation.rse_id == models.Source.rse_id,
            ),
        )
        .where(
            models.Source.scope.is_(
                None
            )  # Only try to delete replicas if they are not used as sources in any transfers
        )
    ...
    for chunk in chunks(
        session.execute(stmt).yield_per(2 * limit), math.ceil(1.25 * limit)
    ):
        session.query(temp_table_cls).delete()
        session.bulk_insert_mappings(
            temp_table_cls, [{"scope": scope, "name": name} for scope, name in chunk]
        )
        ...
        stmt = (
            ...
            .join_from(
                ...
                models.RSEFileAssociation,
                and_(
                    models.RSEFileAssociation.scope == temp_table_cls.scope,
                    models.RSEFileAssociation.name == temp_table_cls.name,
                    models.RSEFileAssociation.rse_id == rse_id,
                ),
            )
            .with_hint(replicas_alias, "index(%(name)s REPLICAS_PK)", "oracle")
            .outerjoin(
                replicas_alias,
                and_(
                    models.RSEFileAssociation.scope == replicas_alias.scope,
            ...
            .outerjoin(
                models.Request,
                and_(
                    models.RSEFileAssociation.scope == models.Request.scope,
                    models.RSEFileAssociation.name == models.Request.name,
                ),
            )
            .group_by(models.RSEFileAssociation)
            .having(
                case(
                    [
                        (
                            func.count(replicas_alias.scope) > 0,
                            True,
                        ),  # Can delete this replica if it's not the last replica
                        (func.count(models.Request.scope) == 0, True),
                    ],  # If it's the last replica, only can delete if there are no requests using it
                    else_=False,
                ).label("can_delete"),
            )
            .order_by(models.RSEFileAssociation.tombstone)
        ...
        for scope, name, path, bytes_, tombstone, state in session.execute(stmt):
            if len(rows) >= limit or (
                needed_space is not None and total_bytes > needed_space
            ):
                break
    ...
    if rows:
        session.query(temp_table_cls).delete()
        session.bulk_insert_mappings(
            temp_table_cls, [{"scope": r["scope"], "name": r["name"]} for r in rows]
        )
        stmt = (
            update(models.RSEFileAssociation)
            .where(
                exists(
                    select([1])
                    .prefix_with("/*+ INDEX(REPLICAS REPLICAS_PK) */", dialect="oracle")
...
@transactional_session
def list_and_mark_unlocked_replicas_no_temp_table(
    limit,
    bytes_=None,
    rse_id=None,
    delay_seconds=600,
    ...
    query = (
        ...
        .filter(
            ~exists(
                select([1])
                .prefix_with(
                    "/*+ INDEX(SOURCES SOURCES_SC_NM_DST_IDX) */", dialect="oracle"
                )
                .where(
                    and_(
                        models.RSEFileAssociation.scope == models.Source.scope,
    ...
    for (scope, name, path, bytes_, tombstone, state) in query.yield_per(1000):
        ...
        replica_cnt = (
            session.query(func.count(models.RSEFileAssociation.scope))
            .with_hint(
                models.RSEFileAssociation, "index(REPLICAS REPLICAS_PK)", "oracle"
            )
            .filter(
        ...
        if replica_cnt[0] > 1:
            ...
            request_cnt = (
                ...
                .filter(
                    and_(models.Request.scope == scope, models.Request.name == name)
                )
                .one()
            )
...
def update_replicas_states(replicas, nowait=False, session=None):
    ...
    for replica in replicas:
        query = session.query(models.RSEFileAssociation).filter_by(
            rse_id=replica["rse_id"], scope=replica["scope"], name=replica["name"]
        )
        try:
            if nowait:
            ...
            raise exception.ReplicaNotFound(
                ...
                % (
                    replica["scope"],
                    replica["name"],
                    get_rse_name(replica["rse_id"], session=session),
                )
            )
        ...
        if replica["state"] == ReplicaState.BEING_DELETED:
            ...
            rucio.core.lock.successful_transfer(
                scope=replica["scope"],
                name=replica["name"],
            ...
            query1 = session.query(models.BadReplicas).filter_by(
                state=BadFilesStatus.BAD,
                rse_id=replica["rse_id"],
                scope=replica["scope"],
                name=replica["name"],
            ...
            if query1.count():
                query1.update(
                    {
                        "state": BadFilesStatus.RECOVERED,
                        "updated_at": datetime.utcnow(),
                    },
                    synchronize_session=False,
                )
        elif replica["state"] == ReplicaState.UNAVAILABLE:
            rucio.core.lock.failed_transfer(
                scope=replica["scope"],
                name=replica["name"],
                rse_id=replica["rse_id"],
        ...
        if not query.update(values, synchronize_session=False):
            ...
            raise exception.UnsupportedOperation(
                "State %(state)s for replica %(scope)s:%(name)s on %(rse)s cannot be updated"
                % replica
            )
    return True
...
def touch_replica(replica, session=None):
    ...
    try:
        ...
        session.query(models.RSEFileAssociation).filter_by(
            rse_id=replica["rse_id"], scope=replica["scope"], name=replica["name"]
        ).with_hint(
            models.RSEFileAssociation, "index(REPLICAS REPLICAS_PK)", "oracle"
        ).with_for_update(
        ...
        session.query(models.DataIdentifier).filter_by(
            scope=replica["scope"], name=replica["name"], did_type=DIDType.FILE
        ).with_hint(
...
def update_replica_state(rse_id, scope, name, state, session=None):
    ...
    :param session: The database session in use.
    """
    return update_replicas_states(
        replicas=[{"scope": scope, "name": name, "state": state, "rse_id": rse_id}],
        session=session,
    )
...
def get_and_lock_file_replicas(
    scope, name, nowait=False, restrict_rses=None, session=None
):
    """
...
def get_source_replicas(scope, name, source_rses=None, session=None):
    ...
    query = (
        session.query(models.RSEFileAssociation.rse_id)
        .filter_by(scope=scope, name=name)
...
def get_and_lock_file_replicas_for_dataset(
    ...
    if session.bind.dialect.name == "postgresql":
        ...
        if total_threads and total_threads > 1:
            content_query = filter_thread_work(
                session=session,
                query=content_query,
                total_threads=total_threads,
                thread_id=thread_id,
        ...
        for child_scope, child_name, bytes_, md5, adler32 in content_query.yield_per(
            1000
        ):
            files[(child_scope, child_name)] = {
                "scope": child_scope,
                "name": child_name,
                "bytes": bytes_,
                "md5": md5,
...
def get_source_replicas_for_dataset(
    scope, name, source_rses=None, total_threads=None, thread_id=None, session=None
):
    """
    Get file replicas for all files of a dataset.
...
def get_replica_atime(replica, session=None):
    ...
    """
    return (
        session.query(models.RSEFileAssociation.accessed_at)
        .filter_by(
            scope=replica["scope"], name=replica["name"], rse_id=replica["rse_id"]
        )
        .with_hint(
            models.RSEFileAssociation,
            text="INDEX(REPLICAS REPLICAS_PK)",
...
def touch_collection_replicas(collection_replicas, session=None):
    ...
    for collection_replica in collection_replicas:
        try:
            session.query(models.CollectionReplica).filter_by(
                scope=collection_replica["scope"],
                name=collection_replica["name"],
                rse_id=collection_replica["rse_id"],
            ).update(
                {"accessed_at": collection_replica.get("accessed_at") or now},
                synchronize_session=False,
            )
        except DatabaseError:
            return False
    ...
    if not deep:
        query = (
            session.query(
                ...
                models.CollectionReplica.available_replicas_cnt.label(
                    "available_length"
                ),
        ...
        # count the metrics
        group_query_archives = (
            session.query(
                sub_query_archives.c.dataset_scope,
        ...
        full_query_archives = session.query(
            group_query_archives.c.dataset_scope.label("scope"),
            ...
            group_query_archives.c.dataset_scope,
...
def list_dataset_replicas_vp(scope, name, deep=False, session=None, logger=logging.log):
    ...
    try:
        vp_replies = requests.get(
            "{}/ds/{}/{}:{}".format(vp_endpoint, nr_replies, scope, name),
            verify=False,
            timeout=1,
        )
        ...
        logger(
            logging.ERROR,
            "In list_dataset_replicas_vp, could not access {}. Error:{}".format(
                vp_endpoint, re
            ),
        )
        vp_replies = ["other"]
    ...
    if vp_replies != ["other"]:
        ...
        for reply in list_dataset_replicas(
            scope=scope, name=name, deep=deep, session=session
        ):
            rse_info = rsemgr.get_rse_info(
                rse=reply["rse"], vo=scope.vo, session=session
            )
    ...
    for (k, v) in filters and filters.items() or []:
        if k == "name" or k == "scope":
            ...
            if "*" in v_str or "%" in v_str:
                ...
                ):  # PostgreSQL escapes automatically
                    query = query.filter(
                        getattr(models.CollectionReplica, k).like(
                            v_str.replace("*", "%")
                        )
                    )
                else:
...
def get_cleaned_updated_collection_replicas(
    total_workers, worker_number, limit=None, session=None
):
    """
    ...
    session.query(models.UpdatedCollectionReplica).filter(
        models.UpdatedCollectionReplica.rse_id.is_(None)
        & ~exists().where(
            and_(
                models.CollectionReplica.name
                == models.UpdatedCollectionReplica.name,  # NOQA: W503
                models.CollectionReplica.scope == models.UpdatedCollectionReplica.scope,
            )
        )
    ).delete(synchronize_session=False)
    session.query(models.UpdatedCollectionReplica).filter(
        models.UpdatedCollectionReplica.rse_id.isnot(None)
        & ~exists().where(
            and_(
                models.CollectionReplica.name
                == models.UpdatedCollectionReplica.name,  # NOQA: W503
    ...
    if session.bind.dialect.name == "oracle":
        ...
        session.execute(
            "DELETE FROM {schema}updated_col_rep A WHERE A.rowid > ANY (SELECT B.rowid FROM {schema}updated_col_rep B WHERE A.scope = B.scope AND A.name=B.name AND A.did_type=B.did_type AND (A.rse_id=B.rse_id OR (A.rse_id IS NULL and B.rse_id IS NULL)))".format(
                schema=schema
            )
        )  # NOQA: E501
    elif session.bind.dialect.name == "mysql":
        subquery1 = (
            session.query(func.max(models.UpdatedCollectionReplica.id).label("max_id"))
            .group_by(
        ...
        session.query(models.UpdatedCollectionReplica).filter(
            models.UpdatedCollectionReplica.id.notin_(subquery2)
        ).delete(synchronize_session=False)
    else:
...
def update_collection_replica(update_request, session=None):
    ...
    if update_request["rse_id"] is not None:
        ...
        try:
            file_replica = (
                session.query(
                    models.RSEFileAssociation, models.DataIdentifierAssociation
                )
                .filter(
                    models.RSEFileAssociation.scope
                    == models.DataIdentifierAssociation.child_scope,
                ...
                .with_entities(
                    label(
                        "ds_available_bytes", func.sum(models.RSEFileAssociation.bytes)
                    ),
        ...
        file_replicas = (
            session.query(models.RSEFileAssociation, models.DataIdentifierAssociation)
            .filter(
                models.RSEFileAssociation.scope
                == models.DataIdentifierAssociation.child_scope,
                models.RSEFileAssociation.name
        ...
        for file_replica in file_replicas:
            ...
            collection_replica = (
                ...
                .filter_by(
                    scope=update_request["scope"],
                    name=update_request["name"],
                    rse_id=file_replica.rse_id,
                )
                .first()
            )
            if collection_replica:
                collection_replica.state = ds_replica_state
                collection_replica.available_replicas_cnt = (
                    file_replica.available_replicas
                )
                collection_replica.available_bytes = file_replica.ds_available_bytes
    session.query(models.UpdatedCollectionReplica).filter_by(
...
def bulk_add_bad_replicas(
    ...
    for replica in replicas:
        ...
        if state == BadFilesStatus.TEMPORARY_UNAVAILABLE:
            query = session.query(models.BadReplicas).filter_by(
                scope=replica["scope"],
                name=replica["name"],
                rse_id=replica["rse_id"],
                state=state,
...
def bulk_delete_bad_replicas(bad_replicas, session=None):
    ...
    for chunk in chunks(replica_clause, 100):
        session.query(models.BadReplicas).filter(or_(*chunk)).delete(
            synchronize_session=False
        )
    return True
...
def add_bad_pfns(pfns, account, state, reason=None, expires_at=None, session=None):
    ...
    if rep_state == BadPFNStatus.TEMPORARY_UNAVAILABLE and expires_at is None:
        raise exception.InputValidationError(
            "When adding a TEMPORARY UNAVAILABLE pfn the expires_at value should be set."
        )
    elif rep_state == BadPFNStatus.BAD and expires_at is not None:
        raise exception.InputValidationError(
            "When adding a BAD pfn the expires_at value shouldn't be set."
        )

    pfns = clean_surls(pfns)
    for pfn in pfns:
        new_pfn = models.BadPFNs(
            path=str(pfn),
            account=account,
            state=rep_state,
            reason=reason,
...
def list_expired_temporary_unavailable_replicas(
    total_workers, worker_number, limit=10000, session=None
):
    """
    ...
    query = (
        session.query(
            models.BadReplicas.scope, models.BadReplicas.name, models.BadReplicas.rse_id
        )
...
def get_replicas_state(scope=None, name=None, session=None):
    ...
    query = session.query(
        models.RSEFileAssociation.rse_id, models.RSEFileAssociation.state
    ).filter_by(scope=scope, name=name)
    states = {}
...
def get_suspicious_files(
    ...
    if available_elsewhere not in [
        SuspiciousAvailability["ALL"].value,
        SuspiciousAvailability["EXIST_COPIES"].value,
        SuspiciousAvailability["LAST_COPY"].value,
    ]:
        logger(
            logging.WARNING,
            """ERROR, available_elsewhere must be set to one of the following:
    ...
    other_states_present = exists(
        select([1]).where(
            and_(
                models.BadReplicas.scope == bad_replicas_alias.scope,
...
def get_suspicious_reason(
    rse_id, scope, name, nattempts=0, logger=logging.log, session=None
):
    """
    ...
    query = session.query(
        ...
        bad_replicas_alias.rse_id == rse_id,
...
def set_tombstone(rse_id, scope, name, tombstone=OBSOLETE, session=None):
    ...
    if result.rowcount == 0:
        try:
            ...
            raise exception.ReplicaIsLocked(
                "Replica %s:%s on RSE %s is locked."
                % (scope, name, get_rse_name(rse_id=rse_id, session=session))
            )
        except NoResultFound:
            raise exception.ReplicaNotFound(
                "Replica %s:%s on RSE %s could not be found."
                % (scope, name, get_rse_name(rse_id=rse_id, session=session))
            )
