...
from sqlalchemy.sql.expression import case, false, literal, literal_column, null, select, true
...
from rucio.db.sqla.session import DEFAULT_SCHEMA_NAME, read_session, stream_session, transactional_session
        ...
        if clause:
            for chunk in chunks(clause, 10):
                stmt = select(
                    ...
                    func.max(
                        case((models.BadReplicas.state == BadFilesStatus.SUSPICIOUS, 0),
...
def get_cleaned_updated_collection_replicas(total_workers, worker_number, limit=None, *, session: "Session"):
    ...
    # Delete duplicates
    dialects_fn_map = {'mysql': models.UpdatedCollectionReplica,
                       'oracle': func.nvl(models.UpdatedCollectionReplica.rse_id, 'rse_id_nvl')}

    if session.bind.dialect.name in dialects_fn_map:
        stmt_1 = select(
            ...
            dialects_fn_map[session.bind.dialect.name]
        )

        stmt_del = delete(
            models.UpdatedCollectionReplica
        ).where(
            models.UpdatedCollectionReplica.id.notin_(stmt_1)
        ).execution_options(
            synchronize_session=False
        )
