...
def _get_local_account_usage_unique(account: "InternalAccount", rse_id: Optional[str] = None, *, session: "Session") -> list["RSELocalAccountUsageDict"]:
    ...
    # Create a subquery to get distinct replicas that have locks for this account
    subq = select(
        ...
        and_(
            ...
            models.ReplicaLock.rse_id == models.RSEFileAssociation.rse_id
        )
    ).where(
    ...
    if rse_id:
        subq = subq.where(models.RSEFileAssociation.rse_id == rse_id)
    ...
    for rse_id in all_rse_ids:
        ...
        if usage['bytes'] > 0 or usage['files'] > 0 or limit > 0:
            result_list.append({
                ...
                'files': usage['files'],
                'bytes_limit': limit,
                'bytes_remaining': limit - usage['bytes'] if limit > 0 else 0,
            })
