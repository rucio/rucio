...
def get_rse_thread_number(rse, total_threads, thread_number, session=None):
    ...
    return {'total_threads_per_rse': len(threads),
            'rse_thread_number': threads.index(thread_number)}


@read_session
def list_rses_for_thread_hash(thread_number, total_threads, dynamic=True, session=None):
    """
    ...
    :returns: a list of RSES, partitions
    """
    false_value = False
...
#        filter(RSE.deleted == false_value)
    ...
    rses = [{'id': id, 'rse': rse} for id, rse in query]
    ...
    partitions = {}
    for rse in rses:
        partitions[rse['rse']] = get_rse_thread_number(
            rse=rse['rse'],
            total_threads=total_threads,
            thread_number=thread_number)

    return rses, partitions


@read_session
def list_rses_for_thread_sequential(thread_number, total_threads, dynamic=True, session=None):
    """
    List RSES assigned dynacicaly to a reaper --auto thread.
...
#        filter(RSE.deleted == false_value)
    ...
    threads, partitions, indice = {}, {}, 0
    for _ in range(concurrency):
        for rse in rses:

            if rse['rse'] not in partitions:
                partitions[rse['rse']] = []

            if indice % total_threads not in threads:
                threads[indice % total_threads] = []

            # Thread number - RSEs
            threads[indice % total_threads].append(rse)

            # Reverse lookup RSE-threads
            partitions[rse['rse']].append(indice % total_threads)

            indice += 1

    for rse in threads[thread_number]:
        partitions[rse['rse']].sort()
        partitions[rse['rse']] = {
            'total_threads_per_rse': len(partitions[rse['rse']]),
            'rse_thread_number': partitions[rse['rse']].index(thread_number)}

    return threads[thread_number], partitions
