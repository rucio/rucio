...
def get_signed_url(
    ...
    if service == 'gcs':
        if not CREDS_GCS:
            account_file = config_get('credentials', 'gcs', raise_exception=False, default='/opt/rucio/etc/google-cloud-storage-test.json')  # doc: Path of the Google Cloud Storage credentials.
