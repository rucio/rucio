...
def show(ctx: click.Context, did: str) -> None:
    ...
    meta_plugins = config_get_list("metadata", "plugins", default=["DID_COLUMN"])
    ...
    if ctx.obj.use_rich:
        ...
        for plugin, meta in metadata.items():
            if len(meta) != 0:
                output.append(Text(plugin, style=RichCLITheme.SUBHEADER_HIGHLIGHT))
        ...
        table_data = [[
            ...
            Text(str(parent["type"]), style=RichCLITheme.DID_TYPE.get(parent["type"], 'default'))
        ] for parent in parents]
        output.append(RichUtils.generate_table(table_data, row_styles=['none'], col_alignments=['left', 'left'], headers=['SCOPE:NAME', '[DID TYPE]']))
