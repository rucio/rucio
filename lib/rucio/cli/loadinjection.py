...
def list_(
    ...
    if cli_config == 'rich':
        table = RichUtils.generate_table(table_data, headers=headers)
...
@_DEST_RSE_OPTION
@click.pass_context
def remove(ctx: click.Context, src_rse: str, dest_rse: str) -> None:
...
def update(
    ...
    src_rse: str,
    dest_rse: str,
    inject_rate: Optional[int],
    start_time: Optional[str],
    end_time: Optional[str],
    interval: Optional[int],
    fudge: Optional[float],
    max_injection: Optional[float],
    expiration_delay: Optional[int],
    rule_lifetime: Optional[int],
    big_first: Optional[bool],
...
@_DEST_RSE_OPTION
@click.pass_context
def kill(ctx: click.Context, src_rse: str, dest_rse: str) -> None:
