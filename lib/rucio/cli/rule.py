...
def list_(ctx, did, traverse, csv, file, account, subscription):
    ...
    args = Arguments({"did": did, "rule_id": None, "traverse": traverse, "csv": csv, "file": file, "subscription": (account if account is not None else ctx.obj.client.account, subscription)})
