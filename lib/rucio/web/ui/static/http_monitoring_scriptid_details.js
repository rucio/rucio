...
function fillTable(csv,date) {
  ...
  for(var i=0; i < split.length; i++) {
    ...
    if((cols.length < 7) || (isNaN(cols[4])) || (isNaN(cols[5])) || (isNaN(cols[6])))
      continue;
    tbl_data.push([
      $('<div>').append($('<a></a>', {'href': '/webstats/accounts/'+cols[1]+'?date='+date}).text(cols[1])).html(),
      $('<div>').append($('<a></a>', {'href': '/webstats/apiclasses/'+cols[2]+'?date='+date}).text(cols[2])).html(),
      Number(cols[4]),
...
function fillTableScriptList(csv, date) {
  ...
  for(var scriptID in aggregatedData) {
    tblData.push([$('<div>').append($('<a></a>', {'href': '/webstats/scriptids/'+scriptID+'?date='+date}).text(scriptID)).html(),
