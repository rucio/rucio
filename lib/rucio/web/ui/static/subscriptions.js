...
display_data = function(data, chosen_account, date, hour, minutes) {
    ...
    $.each(data, function(index, value) {
        ...
        if (state == 'OK') {
            if (count > 0){
                tmp[name][0] = $('<div>').append($('<a></a>', {'href': '/subscriptions/rules?name=' + name + '&state=O&account=' + chosen_account}).text(count)).html();
            }
        } else if (state == 'REPLICATING') {
            if (count > 0){
                tmp[name][1] = $('<div>').append($('<a></a>', {'href': '/subscriptions/rules?name=' + name + '&state=R&account=' + chosen_account}).text(count)).html();
            }
        } else if (state == 'STUCK'){
            if (count > 0){
                tmp[name][2] = $('<div>').append($('<a></a>', {'href': '/subscriptions/rules?name=' + name + '&state=S&account=' + chosen_account}).text(count)).html();
            }
        } else if (state == 'SUSPENDED'){
            if (count > 0){
                tmp[name][3] = $('<div>').append($('<a></a>', {'href': '/subscriptions/rules?name=' + name + '&state=U&account=' + chosen_account}).text(count)).html();
            }
        }
    });
    data = [];
    $.each(tmp, function(key, values) {
        data.push({
            'name': $('<div>').append($('<a></a>', {'href': '/subscription?name=' + key + '&account=' + chosen_account}).text(key)).html(),
            'ok': values[0],
            'rep': values[1],
            'stuck': values[2],
            'suspended': values[3],
    ...
    if (data.length == 0) {
        $('#loader').text('No subscriptions found for the chosen account (' + chosen_account + ')');
        $('#resulttable').html('');
        return;
    }
    ...
    dt.order([0, 'asc']).draw();
    $('#loader').html('');
    $('#last_update').empty().append($('<font color="orange"></font>').text('Last Update: ' + date + ' ' + hour + ':' + minutes));
}
...
$(document).ready(function(){
    ...
    $('#subbar-details').text('[' + chosen_account + ']');
