    ...
    r.list_subscription_rules_state({
        ...
        success: function(data) {
            ...
            $.each(data, function(index, value) {
                ...
                if (state == 'OK') {
                    if (count > 0){
                        tmp[name][0] = '<a href="/subscriptions/rules?name=' + name + '&state=' + state + '&account=' + chosen_account + '">' + count + '</a>';
                    }
            ...
            $.each(tmp, function(key, values) {
                    data.push({'name': '<a href="/subscription?name=' + name + '&account=' + chosen_account + '">' + name + '</a>', 'ok': values[0], 'rep': values[1], 'stuck': values[2], 'suspended': values[3]});
            });
