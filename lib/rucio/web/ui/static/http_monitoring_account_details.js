...
function fillTable(csv,date) {
  ...
  for(var i=0; i < split.length; i++) {
    cols = split[i].split('\t');
    if(cols[0] == "") continue;
    let caption_elem = $('<a></a>').text(cols[2]);
    if (/^\w+\s+\/(dids|replicas)\/\S+?\/.+$/.test(cols[2])) {
      caption_elem.attr('target', '_blank').attr('href', '/search?scope='+cols[2].match(/^\w+\s+\/(dids|replicas)\/(.*?)\/.*$/)[2]+'&name='+cols[2].match(/^\w+\s+\/(dids|replicas)\/.*?\/(.*?)(\?.*)?$/)[2]);
    } else if (/^PUT\s\/rules\/\w+$/.test(cols[2])) {
      caption_elem.attr('target', '_blank').attr('href', '/rule?rule_id='+cols[2].match(/^PUT\s\/rules\/(\w+)$/)[2]);
    } else {
      caption_elem = caption_elem.html(); // remove link element
    }
    tbl_data.push([$('<div>').append(caption_elem).html(),
      Number(cols[3]),
...
function fillTableAccountList(csv, date) {
  ...
  for(var i in data) {
    tblData.push([$('<div>').append($('<a></a>', {'href': '/webstats/accounts/'+data[i][1]+'?date='+date}).text(data[i][1])).html(),
