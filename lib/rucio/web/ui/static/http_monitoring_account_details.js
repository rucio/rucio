...
function loadData(date, account) {
  ...
  $.ajax({
    ...
    crossDomain: true,
    success: function(csv) {
      drawChartH('#scriptids1 > .chart.hits', "Requests per SrciptID", csv2chart(csv, 3, 4, undefined, true));
  ...
  $.ajax({
    ...
    success: function(csv) {
      fillTable(csv,date);
      drawChartV('#resources > .chart.hits', undefined, csv2chart(csv, 2, 3, 20));
      drawChartV('#resources > .chart.mb', undefined, csv2chart(csv, 2, 4, 20, undefined, (1/(1024*1024))));
      drawChartV('#resources > .chart.sec', undefined, csv2chart(csv, 2, 5, 20, undefined, (1/1000000)));
    },
    error: function(jqXHR, textStatus, errorThrown) {
...
function fillTable(csv,date) {
  ...
  for(var i=1; i < splitted.length; i++) {
    ...
    if(cols[0] == "") continue;
    var caption = '';
    if (/^\w+\s+\/dids\/\S+?\/.+$/.test(cols[2])) {
      caption = '<a target="_blank" href="/search?scope='+cols[2].match(/^\w+\s+\/dids\/(.*?)\/.*$/)[2]+'&name='+cols[2].match(/^\w+\s+\/dids\/.*?\/(.*)$/)[2]+'">'+cols[2]+'</a>';
