...
change_attrs = function() {
    ...
    if (err) {
        if (err.status == 401) {
            ...
            $('#rse_options_message').empty().append($('<div class="columns large-12"></div>').append($('<font color="red"></font>').text(err.responseText)));
        }
        return;
    }
...
load_data = function(rse) {
    ...
    $("#rse_input").removeAttr('data-storage-total')
    ...
    r.get_rse_usage({
        ...
        success: function(data) {
            $(data).each(function(index, element) {
                if (element.source == 'storage') $("#rse_input").attr('data-storage-total', element.total);
            });
            r.get_rse_account_usage({
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    $("#results").append($('<font color="red"></font>').text(String(jqXHR.responseText)));
                }
            });
        },
        error: function(jqXHR, textStatus, errorThrown) {
...
function renderBytesWithPct(data, type, row) {
  ...
  if (type === 'display' || type === 'filter') {
    switch(data) {
      ...
      default: return button + '<span/>' + filesize(data, {'base': 10}) + ' (' + Math.floor(100 / $("#rse_input").attr('data-storage-total') * data) + '%)</span>';
    }
  } else {
    return data;
  }
...
function parse_quota_input(quota_str) {
    ...
    switch(parts[2].toLowerCase()) {
        case '%': return (Number(parts[1])/100.0*Number($("#rse_input").attr('data-storage-total')));
