        ...
        success: function(data) { 
            ...
            r.get_rse_account_usage({
                ...
                success: function(data) {
                    ...
                    dt.order([0, 'asc']).draw();
                    $('#bulk_edit').html('<a href="#" class="button small" data-reveal-id="bulk_editor">Bulk Edit</a>')
...
$(document).ready(function(){
    ...
    $(document).on('click', '#resulttable .button.reset', function() {
        var container = $(this).closest('tbody .editable');
    ...
    $('#bulk_apply').click(function() {
      ...
      $('tbody .editable').each( function(index, element) {
        ...
        if (bytes != $(element).attr('data-bytes')) $(element).addClass('updated')
    ...
    $('#bulk_edit').click(function(event) { $('#bulk_editor input[type="text"]').select().focus(); });
