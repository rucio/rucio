...
value_to_color_safe = function(value) {
    if (value < 0) {
        return $('<div>').append($('<font color="red"></font>').text(String(value.toFixed(2)))).html();
    } else if (value > 0) {
        return $('<div>').append($('<font color="green"></font>').text(String(value.toFixed(2)))).html();
    } else {
        return $('<div>').text(String(value.toFixed(2))).html();
    }
}
    ...
    r.get_account_usage_from_dumps({
        success: function(ret_data) {
            ...
            $.each(str_data, function(index, value) {
                ...
                data.push({
                    'account': $('<div>').text(String(tmp['account'])).html(),
                    'rse': $('<div>').text(String(tmp['rse'])).html(),
                    'quota': $('<div>').text(String(tmp['quota'].toFixed(2))).html(),
                    'usage': $('<div>').text(String(tmp['usage'].toFixed(2))).html(),
                    'difference': value_to_color_safe(tmp['difference']),
                });
            ...
            $.each(disk_overview, function(key, value) {
                data_overview.push({
                    'usage': $('<div>').text(String(value['usage'].toFixed(2))).html(),
                    'account': $('<div>').text(String(value['account'])).html(),
                    'quota': $('<div>').text(String(value['quota'].toFixed(2))).html(),
                    'difference': value_to_color_safe(value['difference']),
                });
            ...
            var dt = $('#resulttable_details').DataTable( {
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    $(api.column(2).footer()).html(total_quota.toFixed(2));
                    $(api.column(3).footer()).html(total_used.toFixed(2));
                    $(api.column(4).footer()).html(total_difference.toFixed(2));
                }
            });
            dt.order([0, 'asc']).draw();
            $('#loader').html('');
