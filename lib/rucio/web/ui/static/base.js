...
$(document).ready(function() {
    $('#rucio_webui_version').text(get_version());
    r.ping({
        success: function(data) {
            $('#rucio_server_version').text(data.version);
        }, error: function(jqXHR, textStatus, errorThrown) {
    ...
    available_accounts.forEach(function(acct) {
        let safe_acct = String(acct).replaceAll('"', '');
        $('#accountselecter').append($('<li></li>').append($('<a></a>').attr('onClick', 'set_account("' + safe_acct + '")').text(acct)));
    });
