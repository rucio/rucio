    ...
    $("#submit_button").click(function(){
        ...
        r.get_bad_replicas_summary({rse_expression: url_param('rse_expression'),
             ...
             for (var k in dict){
                 tmp = k.split('|');
                 tmp_dict = {};
                 tmp_dict['rse'] = $('<div>').text(tmp[0]).html();
                 var tmp_date = tmp[1].split(' ');
                 tmp_dict['created_at'] = $('<div>').text(tmp_date[1] + ' ' + tmp_date[2] + ' ' + tmp_date[3]).html();
                 var isURL = tmp[2].indexOf("http") == 0;
                 if (isURL){
                     tmp_dict['reason'] = $('<div>').append($('<a></a>', {'href': tmp[2]}).text(tmp[2])).html();
                 } else {
                     tmp_dict['reason'] = $('<div>').text(tmp[2]).html();
                 }
                 tmp_dict['BAD'] = $('<div>').text(dict[k]['BAD']).html();
                 if  (dict[k]['BAD'] != '0'){
                     tmp_dict['BAD'] = $('<div>').append($('<a></a>', {'href': '/bad_replicas?rse=' + tmp[0] + '&state=BAD'}).text(dict[k]['BAD'])).html();
                 }
                 tmp_dict['DELETED'] = $('<div>').text(dict[k]['DELETED']).html();
                 if  (dict[k]['DELETED'] != '0'){
                     tmp_dict['DELETED'] = $('<div>').append($('<a></a>', {'href': '/bad_replicas?rse=' + tmp[0] + '&state=DELETED'}).text(dict[k]['DELETED'])).html();
                 }
                 tmp_dict['LOST'] = $('<div>').text(dict[k]['LOST']).html();
                 if  (dict[k]['LOST'] != '0'){
                     tmp_dict['LOST'] = $('<div>').append($('<a></a>', {'href': '/bad_replicas?rse=' + tmp[0] + '&state=LOST'}).text(dict[k]['LOST'])).html();
                 }
                 tmp_dict['RECOVERED'] = $('<div>').text(dict[k]['RECOVERED']).html();
                 if  (dict[k]['RECOVERED'] != '0'){
                     tmp_dict['RECOVERED'] = $('<div>').append($('<a></a>', {'href': '/bad_replicas?rse=' + tmp[0] + '&state=RECOVERED'}).text(dict[k]['RECOVERED'])).html();
                 }
                 tmp_dict['SUSPICIOUS'] = $('<div>').text(dict[k]['SUSPICIOUS']).html();
                 if  (show_suspicious && dict[k]['SUSPICIOUS'] != '0'){
                     tmp_dict['SUSPICIOUS'] = $('<div>').append($('<a></a>', {'href': '/bad_replicas?rse=' + tmp[0] + '&state=SUSPICIOUS'}).text(dict[k]['SUSPICIOUS'])).html();
                 }
                 var res_list = [];
             ...
             $('#downloader').empty().append($('<a></a>', {'href': 'data:application/octet-stream;base64,' + btoa(JSON.stringify(res)), 'download': 'dump.json'}).text("download as JSON"));
             dt = $('#badreplicasummary').DataTable( {
                 retrieve: true,
                 data: list_result,
