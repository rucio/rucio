        ...
        success: function(response) {
            ...
            $.each(data, function(did, meta) {
                ...
                if (open) {
                    ...
                    size =  parseInt(meta['bytes'] / 1024);
                }
                did_link = '<a href="/did?scope=' + scope_name[0] + '&name=' + scope_name[1] + '">' + did + '</a>'
                entry = {'name': did_link, 'creation_date': meta['creation_date'].replace('T', ' '), 'size': size, 'CERN': '', 'CA': '', 'DE': '', 'ES': '', 'FR': '', 'IT': '', 'ND': '', 'NL': '', 'RU': '', 'TW': '', 'UK': '', 'US': ''};
                rses = meta['rses'];
                max_rows = 1;
                ...
                $.each(rses, function(cloud, locks) {
                    ...
                    $.each(locks, function(index, lock) {
                        if (lock[2] == 'O') {
                            if (open) {
                                entry[cloud] += '<div style="background-color:#00FF00"><a title="' + lock[0] + '" style="color: black" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + ' (open)</center></a></div>';
                            } else {
                            ...
                            if (open) {
                                entry[cloud] == '<div style="background-color:orange"><a style="color: black" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + ' (open)</center></a></div>';
                            } else {
                            ...
                            if (open) {
                                entry[cloud] += '<div style="background-color:red"><a style="color: white" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + ' (open)</center></a></div></center>';
                            } else {
