...
get_rules = function(account, rse, activity, state, created_before, created_after) {
    ...
    r.list_rules({
        ...
        success: function(data) {
            ...
            $.each(data, function(index, value) {
                ...
                if (value.expires_at != undefined) {
                    ...
                    lifetime = $('<div>').append($('<span></span>', {'data-tooltip': '', 'aria-haspopup': 'true', 'class': 'has-tip', 'title': 'Expires at: ' + value.expires_at}).text(format_time(time_to_expire))).html();
                }
                new_name = "";
                name_split = $('<div>').text(value.name).html().split('.');
                ...
                let scope_name_html = $('<div>').text(value.scope).html() + ':' + new_name;

                let link_container = $('<div>').append($('<a></a>', {'href': '/rule?rule_id=' + value.id}).html(scope_name_html));
                if (time_to_expire < 3600) {
                    link_container.prepend($('<i></i>', {'title': 'This rule will expire in less than an hour', 'class': 'step fi-alert size-18'}), ' ');
                }
                const locks_ok_cnt_elem = $('<font></font>').text(value.locks_ok_cnt);
                let locks_ok_cnt;
                if (value.locks_ok_cnt > 0) {
                    locks_ok_cnt = $('<div>').append(locks_ok_cnt_elem.attr('color', 'green')).html();
                } else {
                    locks_ok_cnt = locks_ok_cnt_elem.html();
                }
                const locks_replicating_cnt_elem = $('<a></a>').text(value.locks_replicating_cnt);
                let locks_replicating_cnt;
                if (value.locks_replicating_cnt > 0) {
                    locks_replicating_cnt = $('<div>').append(locks_replicating_cnt_elem.attr('href', '/rule?rule_id=' + value.id + '&show_locks=true&lock_state=replicating').attr('color', 'orange')).html();
                } else {
                    locks_replicating_cnt = locks_replicating_cnt_elem.html();
                }
                const locks_stuck_cnt_elem = $('<a></a>').text(value.locks_stuck_cnt);
                let locks_stuck_cnt;
                if (value.locks_stuck_cnt > 0) {
                    locks_stuck_cnt = $('<div>').append(locks_stuck_cnt_elem.attr('href', '/rule?rule_id=' + value.id + '&show_locks=true&lock_state=stuck').attr('color', 'red')).html();
                } else {
                    locks_stuck_cnt = locks_stuck_cnt_elem.html();
                }
                let state = $('<font></font>').text(value.state);
                if (value.state == 'OK') {
                    state.attr('color', 'green');
                } else if (value.state == 'REPLICATING') {
                    state.attr('color', 'orange');
                } else if (value.state == 'STUCK') {
                    state.attr('color', 'red');
                } else if (value.state == 'WAITING_APPROVAL') {
                    state.attr('color', 'purple');
                } else if (value.state == 'INJECT') {
                    state.attr('color', 'pink');
                } else {
                    state = state.html(); // remove font element
                }
                filtered_data.push({
                    '$id': value.id,
                    'link': link_container.html(),
                    'account': $('<div>').text(value.account).html(),
                    'rse_expression': $('<div>').text(value.rse_expression).html(),
                    'created_at': $('<div>').text(new Date(value.created_at).toISOString()).html(),
                    'lifetime': lifetime,
                    'state': $('<div>').append(state).html(),
                    'size': $('<div>').text(value.bytes != null ? filesize(value.bytes, {'base': 10}) : 'N/A').html(),
                    'locks_ok_cnt': locks_ok_cnt,
                    'locks_replicating_cnt': locks_replicating_cnt,
                    'locks_stuck_cnt': locks_stuck_cnt,
                });
            });

            $('#downloader').empty().append($('<a></a>', {'href': 'data:application/octet-stream;base64,' + btoa(JSON.stringify(filtered_data)), 'download': download_name}).text("download as JSON"));
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            loader.append($('<text color="red"></text>').text(String(jqXHR.responseText).split(':')[1]));
            loader.append('<br/>');
        }
    });
};
