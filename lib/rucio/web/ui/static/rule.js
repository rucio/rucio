    ...
    r.get_request_by_did({'scope': scope,
                          ...
                          success: function(data){
                              ...
                              if (['DONE', 'FAILED', 'SUBMITTED'].includes(data['state'])) {
                                  ...
                                  $("#" + e.target.id).empty().append($('<a></a>', {'href': url}).text(url.split('#')[0] + '...'));
                              } else {
...
function link_to_new_atlas_dashboard(file, scope, rse) {
    ...
    return $('<div>').append($('<a></a>', {'href': link, 'target': '_blank'}).text('link')).html();
}
...
function show_details(file_data) {
    ...
    $.each(file_data, function(file, rses) {
        ...
        let ftsmon_container_elem = $('<div>');

        $.each(sorted_rses, function(index, rse) {
            var state = rses[rse];
            let rse_elem = $('<font></font>').text(rse);
            dashboard += "<div>" + link_to_new_atlas_dashboard(name, scope, rse) + "</div>";
            if (state == 'OK') {
                rse_elem.attr('color', 'green');
                ftsmon_container_elem.append('<div style="visibility: hidden;">.</div>');
                str_state += 'Ok ';
            } else if (state == 'REPLICATING') {
                rse_elem.attr('color', 'orange');
                value = scope + ',' + name + ',' + rse;
                id = 'ftsmon' + i;
                ftsmon_container_elem.append($('<div>', {'id': id, 'value': value}).text('click to generate link'));
                ids.push(id);
                str_state += 'Replicating ';
            } else if (state == 'STUCK') {
                rse_elem.attr('color', 'red');
                value = scope + ',' + name + ',' + rse;
                id = 'ftsmon' + i;
                ftsmon_container_elem.append($('<div>', {'id': id, 'value': value}).text('click to generate link'));
                ids.push(id);
            ...
            rses_container_elem.append(rse_elem, '<br/>')
            i += 1;
        });
        row.rses = rses_container_elem.html();
        row.ftsmon = ftsmon_container_elem.html();
        row.state = str_state;
        if (policy == 'atlas') {
...
function load_locks() {
    ...
    r.get_replica_lock_for_rule_id({
        ...
        success: function(data) {
            ...
            if (policy == 'atlas') {
                $("#locks_details").append("<table id=\"dt_files\" class=\"compact stripe order-column cell-border\" style=\"word-wrap: break-word;\"><thead><th>Filename</th><th>RSEs</th><th>DDM Dashboard</th><th>FTS Monitoring</th></thead><tfoot><th>Filename</th><th>RSEs</th><th>DDM Dashboard</th><th>FTS Monitoring</th></tfoot></table>");
            } else {
                $("#locks_details").append("<table id=\"dt_files\" class=\"compact stripe order-column cell-border\" style=\"word-wrap: break-word;\"><thead><th>Filename</th><th>RSEs</th><th>FTS Monitoring</th></thead><tfoot><th>Filename</th><th>RSEs</th><th>FTS Monitoring</th></tfoot></table>");
            }
        ...
        r.update_replication_rule({
            ...
            error: function(jqXHR, textStatus, errorThrown)
            {
                if (jqXHR.status == 500) {
                    ...
                    $('#boost_message').empty().append($('<font color="red"></font>').text(except['ExceptionMessage']));
                }
            }
        })
    });
    ...
    r.examine_rule({
        ...
        success: function(data) {
            ...
            $.each(data['transfers'], function(index, lock) {
                ...
                available_srcs = available_srcs.slice(0, -2);
                blocklisted_srcs = blocklisted_srcs.slice(0, -2);
                dt.row.add({
                    'name': $('<div>').text(lock['name']).html(),
                    'rse': $('<div>').text(lock['rse']).html(),
                    'attempts': $('<div>').text(lock['attempts']).html(),
                    'last_time': $('<div>').text(lock['last_time']).html(),
                    'last_error': $('<div>').text(lock['last_error']).html(),
                    'last_source': $('<div>').text(lock['last_source']).html(),
                    'available_srcs': $('<div>').text(available_srcs).html(),
                    'blocklisted_srcs': $('<div>').text(blocklisted_srcs).html(),
                });
            });
            dt.order([0, 'asc']).draw();
            $("#show_examine").html("");
...
$(document).ready(function(){
    $('#subbar-details').text('[' + url_param('rule_id') + ':' + url_param('name') + ']');
    ...
    r.list_replication_rule({
        ...
        success: function(data) {
            $("#loading").html("");
            if (data == '') {
                $('#result').text('Could not find rule ' + url_param('rule_id'));
            } else {
                $("#locks").attr('class', 'columns panel');
                $("#locks").html("<h4 id=\"locks_title\">Lock States per File</h4><div id=\"show_locks\">Please click to show locks</div></div><div id=\"locks_details\"></div>");
                ...
                for(var i=0; i<sorted_keys.length; ++i) {
                    if (data[sorted_keys[i]] != undefined) {
                        let row = $('<tr></tr>');
                        row.append($('<th></th>', {'id': 'row_' + sorted_keys[i]}).text(sorted_keys[i]));
                        ...
                        if (typeof data[sorted_keys[i]] === 'boolean'){
                        ...
                        } else if (sorted_keys[i] == 'name') {
                            row.append($('<td></td>').append($('<a></a>', {'href': '/did?scope=' + data['scope'] + '&name=' + data['name']}).text(data['name'])));
                        } else if (sorted_keys[i] == 'scope') {
                            row.append($('<td></td>').append($('<a></a>', {'href': '/search?scope=' + data['scope'] + '&name=undefined'}).text(data['scope'])));
                        } else if (sorted_keys[i] == 'state') {
                            let color = "black";
                            if (data['state'] == 'OK') {
                                color = "green";
                            ...
                            row.append($('<td></td>').append($('<font></font>', {'color': color}).text(data['state'])));
                        } else if (sorted_keys[i] == 'subscription_id') {
                            row.append($('<td></td>').append($('<a></a>', {'href': '/subscription?id=' + data['subscription_id']}).text(data['subscription_id'])));
                        } else if (sorted_keys[i] == 'expires_at') {
                            let col = $('<td></td>');
                            col.text(data['expires_at']);
                            col.append('<i style="visibility:hidden;" class="step fi-plus"></i><a><i title="Extend expiration date" id="change_date" class="step fi-plus size-18"></i></a><i style="visibility:hidden;" class="step fi-plus"></i><a><i title="Remove expiration date" id="clear_date" class="step fi-x size-18"></i></a>');
                            row.append(col);
                        ...
                        } else if (sorted_keys[i] == 'child_rule_id') {
                            row.append($('<td></td>').append($('<a></a>', {'href': '/rule?rule_id=' + data['child_rule_id']}).text(data['child_rule_id'])));
                        } else {
