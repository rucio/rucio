...
function fillTable(csv,date) {
  ...
  for(var i=0; i < split.length; i++) {
    ...
    if((cols.length < 5) || (isNaN(cols[2])) || (isNaN(cols[3])) || (isNaN(cols[4])))
      continue
    let caption_elem = $('<a></a>').text(cols[1]);
    if (/^\w+\s+\/(dids|replicas)\/\S+?\/.+$/.test(cols[1])) {
      caption_elem.attr('target', '_blank').attr('href', '/search?scope='+cols[1].match(/^\w+\s+\/(dids|replicas)\/(.*?)\/.*$/)[2]+'&name='+cols[1].match(/^\w+\s+\/(dids|replicas)\/.*?\/(.*?)(\?.*)?$/)[2]);
    } else if (/^PUT\s\/rules\/\w+$/.test(cols[1])) {
      caption_elem.attr('target', '_blank').attr('href', '/rule?rule_id='+cols[1].match(/^PUT\s\/rules\/(\w+)$/)[2]);
    } else {
