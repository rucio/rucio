...
get_color_state_safe = function(locks_ok, locks_rep, locks_stuck, show_state) {
    const elem = $('<font></font>');
    var state = get_state(locks_ok, locks_rep, locks_stuck);
    if (state == 'done') {
        elem.attr('color', 'green').text('Done');
    } else if (state == '90% transfers done') {
        elem.attr('color', 'green').text('90% transfers done');
    } else if (state == 'transferring') {
        elem.attr('color', 'orange').text('Transferring');
    } else if (state == 'initialised') {
        elem.attr('color', 'black').text('Initialised');
    } else if (state == 'subscribed') {
        elem.attr('color', 'pink').text('Subscribed');
    } else if (state == 'stuck') {
        elem.attr('color', 'red').text('Stuck');
    }
    ...
    r.list_rules({
        ...
        success: function(data) {
            ...
            $.each(data, function(index, value) {
                ...
                filtered_data.push({
                    'link': $('<div>').append($('<a></a>', {'href': '/rule?rule_id=' + value.id}).text(String(value.name))).html(),
                    'rse_expression': $('<div>').text(String(value.rse_expression)).html(),
                    'state': get_color_state_safe(value.locks_ok_cnt, value.locks_replicating_cnt, value.locks_stuck_cnt, state),
                    'created_at': $('<div>').text(new Date(value.created_at).toISOString()).html(),
                    'datatype': $('<div>').text(String(value.datatype)).html(),
                    'project': $('<div>').text(String(value.project)).html(),
                    'stream': $('<div>').text(String(value.stream)).html(),
                    'version': $('<div>').text(String(value.version)).html(),
