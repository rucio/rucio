...
function load_info() {
    r.get_account_info({
        ...
        success: function(data) {
            $("#loading").html("");
            if (data == '') {
                $('#result').text('Could not find account ' + url_param('account'));
            } else {
        ...
        success: function(data) {
            ...
            if (data != '') {
                ...
                $.each(data, function(index, pair) {
                    sanitisedData[index] = {
                        '$key': pair.key, // not displayed, but used in API calls
                        'key': $('<div>').text(String(pair.key)).html(),
                        'value': $('<div>').text(String(pair.value)).html(),
                    };
                });
...
function add_attr() {
    ...
    r.add_account_attribute({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            $("#attr_notify").append($('<font color="red"></font>').text('Could not add attribute: ' + String(jqXHR.responseText)));
        }
    });
};
    ...
    r.list_identities({
        ...
        success: function(data) {
            ...
            if (data == '') {
                ...
                $.each(data, function(index, pair) {
                    sanitisedData[index] = {
                        '$identity': pair.identity, // not displayed, but used in API calls
                        '$type': pair.type, // not displayed, but used in API calls
                        'identity': $('<div>').text(String(pair.identity)).html(),
                        'type': $('<div>').text(String(pair.type)).html(),
                    };
                });
...
function add_identity() {
    ...
    r.add_identity({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            console.log(jqXHR.responseText);
            $("#identity_notify").empty();
            // sanitize jqXHR.responseText as it could contain HTML
            $("#identity_notify").append($('<font color="red"></font>').text('Could not add identity: ' + String(jqXHR.responseText)));
        }
    });
...
$(document).ready(function(){
    $('#subbar-details').text('[' + url_param('account') + ']');
