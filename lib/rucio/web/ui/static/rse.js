...
create_info_table = function(data, tableIdentifier, counter) {
    ...
    for (var i = 0; i < sorted_keys.length; ++i) {
        ...
        let row = $('<tr></tr>');
        $(tableIdentifier).append(row);
        if(typeof(data[sorted_keys[i]]) == 'object' && data[sorted_keys[i]]) {
            var newTableIdentifier = 'subtable_' + sorted_keys[i] + counter + i;
            row.append($('<th></th>', {'width': '200'}).text(sorted_keys[i]));
            row.append($('<td></td>').append($('<table></table>', {'id': newTableIdentifier})));
            counter++;
...
create_attributes_table = function(data) {
    ...
    $.each(keys,function(index, item) {
        ...
        row.append($('<td></td>').text(data[0][item]));
        row.append($('<td></td>').append($('<a></a>', {'class': 'button deleteAttribute postfix', 'style': 'margin: 0', 'data-key': item}).text("Delete")));
...
create_protocol_table = function(data) {
    $.each(data,function(index, item) {
        let row = $('<tr></tr>');
        row.append($('<th></th>').text(item['scheme']));
        row.append($('<td></td>').text(item['hostname']));
        row.append($('<td></td>').text(item['port']));
        row.append($('<td></td>').append($('<a></a>', {'class': 'button deleteProtocol postfix', 'style': 'margin: 0', 'data-port': item['port'], 'data-hostname': item['hostname'], 'data-scheme': item['scheme']}).text("Delete")));
        $('#t_rse_protocols').append(row);
    })
}
...
get_rse_attributes = function(rse) {
    r.list_rse_attributes({
        ...
        success: function (data) {
            ...
            if (data == '') {
                $('#errorLoadingAttributes').text('Could not find RSE' + rse);
            } else {
...
get_rse_info = function (rse) {
    r.get_rse({
        ...
        success: function (data) {
            ...
            if (data == '') {
                $('#errorLoadingInfo').text('Could not find RSE ' + rse);
            } else {
