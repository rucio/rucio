    ...
    r.get_dbreleases_from_dumps({
        success: function(response) {
            ...
            $.each(data, function(did, meta) {
                did_link = '<a href="/did?scope=ddo&name=' + did + '">' + did + '</a>'
                ...
                $.each(rses, function(rse, locks) {
                    if (rse == 'T0') {
                        $.each(locks, function(index, lock) {
                            rse = lock[0].split('_')[1];
                            if (lock[2] == 'O') {
                                entry[rse] = '<div style="background-color:#00FF00"><a style="color: black" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + '/' + meta['length'] + '</center></a></div>';
                                
                            } else if (locks[2] == 'R') {
                                entry[rse] = '<div style="background-color:orange"><a style="color: black" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + '/' + meta['length'] + '</center></a></div>';
                            } else {
                                console.log(lock[3]);
                                entry[rse] = '<div style="background-color:red"><a style="color: white" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + '/' + meta['length'] + '</center></a></div></center>';
                            }
                        });
                    } else {
                    if (locks[2] == 'O') {
