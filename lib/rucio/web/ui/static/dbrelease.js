    ...
    r.get_dbreleases_from_dumps({
        success: function(response) {
            ...
            $.each(data, function(did, meta) {
                ...
                entry = {'name': did_link, 'creation_date': meta['creation_date'].replace('T', ' '), 'size': parseInt(meta['bytes'] / 1024 / 1024), 'HOTDISK': '', 'DATADISK': '', 'DATATAPE': '', 'CA': '', 'DE': '', 'ES': '', 'FR': '', 'IT': '', 'ND': '', 'NL': '', 'RU': '', 'TW': '', 'UK': '', 'US': ''};
                rses = meta['rses'];
                max_rows = 1;
                ...
                $.each(rses, function(cloud, locks) {
                    ...
                    $.each(locks, function(index, lock) {
                        if (lock[2] == 'O') {
                            entry[cloud] += '<div style="background-color:#00FF00"><a title="' + lock[0] + '" style="color: black" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + '/' + meta['length'] + '</center></a></div>';
                        } else if (locks[2] == 'R') {
                            ...
                            console.log(lock[3]);
                            entry[cloud] += '<div style="background-color:red"><a style="color: white" href="/rule?rule_id=' + lock[1] +'"><center>' + lock[3] + '/' + meta['length'] + '</center></a></div></center>';
                        }
                    });
                    entry[cloud] += '</div>'
                });
