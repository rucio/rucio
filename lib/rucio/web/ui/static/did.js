                                ...
                                try{
                                    if (browser_enabled_rses.indexOf(rse) != -1){
                                        ...
                                        if (link.includes(".log")) {
                                            str_rses += "<a href=\"#\" onclick=\"loadLogfile('" + link  +"');return false;\"><i title=\"Click to preview " + lfn + " from " + rse  + ".\" class=\"fi-eye size-18\"></i></a> "; 
                                        }
...
loadLogfile = function(input){ 
    ...
    $.each(res, function(key, value) {
        var tmp = {};
        tmp['file'] = "<a href=\"#\" onclick=\"extractLogfile('" + input  + "', '" + key + "');return false;\">" + key  + "</a>";
...
extractLogfile = function(link, filename) {
    ...
    var jqXHR = $.ajax({
        type: "GET",
        url: "/ui/extractLogfile",
        async: false,
        data: {file_location: link, file_name: filename},
        dataType: "json",
        success: function(res) {
            var str = '';
            if(parseInt(res.size) >= 20971520){
