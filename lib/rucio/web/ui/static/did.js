            ...
            if (reference_length <= 500) {              
                        r.list_replicas({
                            ...
                            success: function(ret_meta) {            
                                $('#meta_button').html('<a href="data:application/octet-stream,' + encodeURIComponent(ret_meta.replace(new RegExp('davs', 'g'), 'https')) + '" target="_blank"  download="metalink_' + scope + ':' + name  + '.meta4"><i title="Click to download metalink for this dataset." class="fi-download size-24"></i></a>');                       
                            }});                
