            ...
            $.each(replicas, function(index, replica) {
                ...
                $.each(sorted_rses, function(index, rse) {
                    ...
                    if (browser_enabled_rses.indexOf(rse) != -1){
                        str_rses += rse + "<a href=\"https" + replicas[0]['rses'][rse][0].substring(4) + "\" target=\"blank\">  <i title=\"Click to download this file from " + rse  + ".\" class=\"fi-download size-18\"></i></a>";
    		        } else {
                        str_rses += rse;
			}
...
$(document).ready(function(){
    ...
    if ((storage.isSet('expiration') && Date.now() - storage.get('expiration') > 300000) || !storage.isSet('expiration')) {
        //5 min in millisecs
        storage.set('expiration', Date.now());
        console.log('renewed storage, loading rse info...');
	r.list_rses({
        expression: 'browser_enabled=1',
        success: function(rses) {
            browser_enabled_rses = [];
	    for(i = 0; i < rses.length; i++){
                browser_enabled_rses[i] = rses[i]['rse'];
	    }
        console.log(browser_enabled_rses);
