...
html_dataset_replicas_base = '<div id="t_dataset_replicas" class="columns panel">' +
    '<h4>Dataset Replicas <a href="#" data-reveal-id="infomodalmeta"><i title="Info" class="step fi-info size-24"></i></a></h4>' +
                    ...
                    $.each(replicas, function(index, replica) {
                        ...
                        $.each(sorted_rses, function(index, rse) {
                            ...
                            if (state == 'AVAILABLE') {
                                str_rses += "green>";
                                try{
                                    if (browser_enabled_rses.indexOf(rse) != -1){
                                        ...
                                        str_rses += "<a href=\"" + link + "\" target=\"blank\">  <i title=\"Click to download " + lfn  + " from " + rse  + ".\" class=\"fi-download size-18\"></i></a>";
    		                    } else {
                                        str_rses += rse;
			            }
                                } catch(err) {
...
create_metalink = function(rse, links) {
    var xmlDoc = '<?xml version="1.0" encoding="UTF-8"?>' +
        '<metalink xmlns="urn:ietf:params:xml:ns:metalink">';
    for(i = 0; i < links.length; i++){
        xmlDoc += '<file name="' + links[i]['name']  + '">';
        xmlDoc += '<identity>' + links[i]['identity'] + '</identity>';
        xmlDoc += '<hash type="adler32">' + links[i]['adler32'] + '</hash>';
        xmlDoc += '<hash type="md5">' + links[i]['md5'] + '</hash>';
        xmlDoc += '<size>' + links[i]['size'] + '</size>';
        xmlDoc += '<url priority="' + (i+1) + '">' + links[i]['link'] + '</url></file>';
    }
    xmlDoc += '</metalink>';
    return xmlDoc;
};
        ...
        success: function(replicas) {
            ...
            if (reference_length <= 500) {
                r.list_replicas({
                    'scope': scope,
                    'name': name,
                    success: function(file_replicas) {
                        r.list_replicas({
                            'scope': scope,
                            'name': name,
                            'schemes': ['davs'],
                            success: function(ret_dav_replicas) {
                                var dav_replicas = ret_dav_replicas;
                                temp = {};
                                $.each(dav_replicas, function(index, dav_replica) {
                                    var key = dav_replica['name'];
                                    temp[key] = dav_replica['rses'];
                                ...
                                $.each(file_replicas, function(index, f_replica) {                  
                                    var sorted_rses = Object.keys(f_replica['states']).sort();
                                    $.each(sorted_rses, function(index, rse) {
                                        var state = f_replica['states'][rse];
                                        if(state == 'AVAILABLE'){
                                            if (browser_enabled_rses.indexOf(rse) != -1){
                                                if(available_dataset_rses.hasOwnProperty(rse)){
                                                    available_dataset_rses[rse]['rses'].push({name: f_replica['name'], identity: f_replica['scope'] + ':' + f_replica['name'] , link: temp[f_replica['name']][rse][0].replace("davs", "https"), size: f_replica['bytes'], adler32: f_replica['adler32'], md5: f_replica['md5']});
                                                }
                                            }
                                        }
                                    });
                                });
                                var rowData = dt.rows().every(function(rowIdx) {
                                    var colIdx = 0;
                                    var te = this.data().rse.split(" ");
                                    rse_id = te[te.length - 1];
                                    if(available_dataset_rses.hasOwnProperty(rse_id)){
                                        dt.cell( rowIdx, colIdx).data(rse_id + ' <a href="data:application/octet-stream,' + encodeURIComponent(create_metalink(rse_id, available_dataset_rses[rse_id]['rses'])) + '" target="_blank"  download="metalink_' + rse_id  + '.meta4"><i title="Click to download metalink for this dataset from ' + rse_id  + '." class="fi-download size-18"></i></a>').draw(false);
                                    }
                                });
                            }});
                    }
                });
            } else { console.log('Dataset contains more than 500 files, no metalinks created.');}
        },
        error: function(jqXHR, textStatus, errorThrown) {
            $('#loading_rules').html(errorThrown);
        }
