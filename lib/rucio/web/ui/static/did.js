...
html_replicas_table = '<table id="dt_replicas" class="compact stripe order-column" style="word-wrap: break-word;">' +
    ...
    '<font color=blue>TEMPORARY_UNAVAILABLE</font> ' +
    '</div>';
    ...
    r.list_parent_dids({
        ...
        success: function(dids) {
            ...
            $.each(dids, function(index, did) {
                sanitisedData[index] = {
                    'link': $('<div>').append($('<a></a>', {'href': '/did?scope=' + did['scope'] + '&name=' + did['name']}).text(did['scope'] + ':' + did['name'])).html(),
                    'type': $('<div>').text(String(did.type)).html(),
                };
            });
                ...
                success: function(ret_dav_replicas) {
                    ...
                    $.each(replicas, function(index, replica) {
                        ...
                        $.each(sorted_rses, function(index, rse) {
                            ...
                            if (blocklisted_rses.indexOf(rse) != -1) {
                                rses_container_elem.append($('<i></i>', {'title': 'This RSE is blocklisted for reading', 'class': 'step fi-alert size-18'}));
                                blocklisted = true;
                            }
                            let font_elem = $('<font></font>').text(rse);
                            if (state == 'AVAILABLE') {
                                font_elem.attr('color', 'green');
                                try{
                                    let replica_scope = String(replica['scope']).replaceAll("'", '');
                                    let replica_name = String(replica['name']).replaceAll("'", '');
                                    let rse_escaped = String(rse).replaceAll("'", '');
                                    let replica_bytes = String(replica['bytes']).replaceAll("'", '');
                                    let did_scope = String(scope).replaceAll("'", '');
                                    let did_name = String(name).replaceAll("'", '');
                                    if (browser_enabled_rses.indexOf(rse) != -1 && !blocklisted){
                                        var link = temp[replica['name']][rse][0].replace("davs", "https").replaceAll("'", '');
                                        let download_link_elem = $('<a></a>', {'href': '#', 'onclick': "download_file('" + link + "', '" + replica_scope + "', '" + replica_name + "', '" + rse_escaped + "', '" + replica_bytes  + "', '" + did_scope  + "', '" + did_name + "');return false;"});
                                        download_link_elem.append($('<i></i>', {'title': 'Click to download ' + lfn + ' from ' + rse + '.', 'class': 'fi-download size-18'}));
                                        font_elem.prepend(download_link_elem, ' ');
                                        if (link.includes(".log")) {
                                            let load_log_link_elem = $('<a></a>', {'href': '#', 'onclick': "load_logfile('" + link + "');return false;"});
                                            load_log_link_elem.append($('<i></i>', {'title': 'Click to preview ' + lfn + ' from ' + rse + '.', 'class': 'fi-eye size-18'}));
                                            font_elem.prepend(load_log_link_elem, ' ');
                                        }
                                        browser_enabled_rse_exists = true;
    		                        } else if(!browser_enabled_rse_exists) {
                                        let move_link_elem = $('<a></a>', {'href': '#', 'onclick': "move_did('" + replica_scope + "','" + replica_name + "',false,1);return false;", 'style': 'color: rgb(128,128,128)'});
                                        move_link_elem.append($('<i></i>', {'title': 'This RSE is not WebDAV-enabled. Click here to move the file to a WebDAV-enabled RSE.', 'class': 'fi-download size-18'}));
                                        font_elem.append(' ', $('<span></span>', {'class': 'non_browser'}).append(move_link_elem));
                                    }
                                } catch(err) {
                                    console.log("An error occurred for RSE " + rse + " : " + err);
                                }
                            } else if (state == 'UNAVAILABLE') {
                                font_elem.attr('color', 'red');
                            } else if (state == 'COPYING') {
                                font_elem.attr('color', 'orange');
                            } else if (state == 'BEING_DELETED') {
                                font_elem.attr('color', 'black');
                            } else if (state == 'BAD') {
                                font_elem.attr('color', 'pink');
                            } else if (state == 'TEMPORARY_UNAVAILABLE') {
                                font_elem.attr('color', 'blue');
                            }
                            rses_container_elem.append(font_elem, '<br>');
                        });
                        if(browser_enabled_rse_exists) {
                            rses_container_elem.children('.non_browser').remove();
                        }
                        browser_enabled_rse_exists = false;
                        dt2.row.add({
                            'name': $('<div>').text(lfn).html(),
                            'rses': rses_container_elem.html(),
                        });
                    });
...
load_logfile = function(input){
    ...
    $.each(res, function(key, value) {
        let escaped_input = String(input).replaceAll("'", '');
        let escaped_key = String(key).replaceAll("'", '');
        data.push({
            'file': $('<div>').append($('<a></a>', {'href': '#', 'onclick': "extract_logfile('" + escaped_input  + "', '" + escaped_key + "');return false;"}).text(key)).html(),
            'size': $('<div>').text(filesize(value, {'base': 10})).html(),
        });
    });
...
extract_logfile = function(link, filename) {
    ...
    var jqXHR = $.ajax({
        ...
        success: function(res) {
            ...
            body.empty();
            body.append($('<h1></h1>').text('Content of ' + filename));
            body.append($('<div>').text(str).html());
            body.append($('<textarea></textarea>', {'style': 'width:100%;height:40vw;resize:none;'}).text(res.content));
        },
        error: function(jqXHR, textStatus, errorThrown) {
            w.close();
...
load_rules = function(scope, name) {
    ...
    r.did_get_rules({
                    ...
                    if (rule.subscription_id != null) {
                        r.get_subscription_by_id({
                            ...
                            success: function(subscription) {
                                tmp_sub = $('<div>').append($('<a></a>', {'href': '/subscription?name=' + subscription.name + '&account=' + subscription.account}).text(subscription.name)).html();
                            },
                            error: function(jqXHR, textStatus, errorThrown) {
                                console.log(textStatus);
                            }});
                    }
                    let state_elem = $('<font></font>').text(rule.state);
                    if (rule.state == 'OK') {
                        state_elem.attr('color', 'green');
                    } else if (rule.state == 'REPLICATING') {
                        state_elem.attr('color', 'orange');
                    } else if (rule.state == 'STUCK') {
                        state_elem.attr('color', 'red');
                    } else {
                        state_elem = state_elem.html(); // remove font element
                    }
                    data.push({
                        'rule': $('<div>').append($('<a></a>', {'href': '/rule?rule_id=' + rule.id}).text(rule.rse_expression)).html(),
                        'state': $('<div>').append(state_elem).html(),
                        'account': $('<div>').text(rule.account).html(),
                        'subscription': tmp_sub,
                        'updated_at': $('<div>').text(rule.updated_at).html(),
                    });
                });
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            $('#loading_rules').text(errorThrown);
        }
    });
};

load_blocklisting = function(rses) {
    $.each(rses, function(index, rse) {
        r.get_rse({
            'rse': rse,
            success: function(info) {
                if (info['availability_read'] == false) {
                    blocklisted_rses.push(rse);
...
load_dataset_replicas = function(scope, name) {
    ...
    r.list_dataset_replicas({
        ...
        success: function(replicas) {
            ...
            rses = [];
            $.each(replicas, function(index, replica) {
                let rse_container_elem = $('<div>').text(replica['rse']);
                rse_container_elem.prepend($('<i></i>', {'id': 'warning_bl_' + replica['rse'], 'hidden': '', 'title': 'This RSE is blocklisted for reading', 'class': 'step fi-alert size-18'}), ' ');
                let state_elem = $('<font></font>').text(replica['state']);
                ...
                } else if (replica['state'] == 'UNAVAILABLE') {
                    state_elem.attr('color', 'red');
                } else if (replica['state'] == 'COPYING') {
                    state_elem.attr('color', 'orange');
                } else if (replica['state'] == 'BEING_DELETED') {
                    state_elem.attr('color', 'black');
                } else if (replica['state'] == 'BAD') {
                    state_elem.attr('color', 'pink');
                } else if (replica['state'] == 'TEMPORARY_UNAVAILABLE') {
                    state_elem.attr('color', 'blue');
                } else {
                    state_elem = state_elem.html(); // remove font element
                }
                rses.push(replica['rse']);
                data.push({
                    'rse': rse_container_elem.html(),
                    'state': $('<div>').append(state_elem).html(),
                    'available_length': $('<div>').text(replica['available_length']).html(),
                    'available_bytes': $('<div>').text(filesize(replica['available_bytes'], {'base': 10})).html(),
                    'created_at': $('<div>').text(replica['created_at']).html(),
                    'accessed_at': $('<div>').text(replica['accessed_at']).html(),
                });
            });
...
handle_container = function(scope, name) {
    ...
    r.list_contents({
        ...
        success: function(dids) {
            ...
            $.each(dids, function(index, did) {
                data.push({
                    'did': $('<div>').append($('<a></a>', {'href': '/did?scope=' + did['scope'] + '&name=' + did['name']}).text(did['scope'] + ':' + did['name'])).html(),
                    'type': $('<div>').text(did['type']).html(),
                });
            });
...
build_page = function() {
    ...
    $('#subbar-details').text('[' + scope + ':' + name + ']');
    ...
    r.did_get_metadata({
        ...
        success: function(data) {
            $("#loading").html("");
            if (data == '') {
                $('#result').text('Could not find scope ' + scope);
            } else {
                ...
                for(var i=0; i<sorted_keys.length; ++i) {
                    if (data[sorted_keys[i]] != undefined) {
                        ...
                        } else if (sorted_keys[i] == 'scope') {
                            row.append($('<td></td>').append($('<a></a>', {'href': '/search?scope=' + data['scope'] + '&name=undefined'}).text(data['scope'])));
                        } else if (sorted_keys[i] == 'task_id') {
                            row.append($('<td></td>').append($('<a></a>', {'href': 'http://bigpanda.cern.ch/task/' + data['task_id'] + '/'}).text(data['task_id'])));
                        } else if (sorted_keys[i] == 'panda_id') {
                            row.append($('<td></td>').append($('<a></a>', {'href': 'http://bigpanda.cern.ch/job?pandaid=' + data['panda_id']}).text(data['panda_id'])));
                        } else {
