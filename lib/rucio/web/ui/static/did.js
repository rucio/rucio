                    ...
                    $.each(replicas, function(index, replica) {
                        ...
                        $.each(sorted_rses, function(index, rse) {
                            ...
                            if (blocklisted_rses.indexOf(rse) != -1) {
                                str_rses += '<i title="This RSE is blocklisted for reading" class="step fi-alert size-18"></i> ';
                                blocklisted = true;
                            }
                            str_rses += "<font color=";
                            if (state == 'AVAILABLE') {
...
load_dataset_replicas = function(scope, name) {
    ...
    r.list_dataset_replicas({
        ...
        success: function(replicas) {
            ...
            $.each(replicas, function(index, replica) {
                var tmp = {};
                tmp['rse'] = '<i id="warning_bl_' + replica['rse'] + '" hidden title="This RSE is blocklisted for reading" class="step fi-alert size-18"></i> ' + replica['rse'];
