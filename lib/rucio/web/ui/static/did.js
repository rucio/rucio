...
handle_file = async function(scope, name) {
    ...
    Promise.all([rules, did_informations, requests]).then(function(data) {
        ...
        data.forEach(function(result) {
            if(result['transfers']) {
                transferData = transferData.concat(result['transfers'])
                tableData = tableData.concat(result['transfers'])
            }
            if(result['dids']) {
                didData = didData.concat(result['dids'])
                tableData = tableData.concat(result['dids'])
            }
            if(result['rules']) {
                ruleData = ruleData.concat(result['rules'])
                tableData = tableData.concat(result['rules'])
            }
        })
        show_did_tracking_table(tableData)
        var timelineData = [{'label': 'Transfers', 'data': transferData}, {'label': 'DID', 'data': didData}, {'label': 'Rules', 'data': ruleData}]
        ...
        r.did_get_rules({'scope': scope, 'name': name, 'success': function(rules) {
            ...
            rules.forEach(function(rule) {
                data.push({
                    ...
                    desc: 'Created at: ' + new Date(rule.created_at).toLocaleString(),
                    table: 'Rule <a href="/rule?rule_id=' + rule.id + '">' + rule.id + '</a> created<br>RSE expression: ' + rule.rse_expression
                })
            })
            resolve({'rules': data});
        }})
...
get_requests = function(scope, name, rses) {
    ...
    return new Promise(function(resolve, reject) {
                ...
                r.get_request_by_did({'rse': rse, 'scope': scope, 'name': name, 'success': function(request) {
                    ...
                    if(request['request_type'] == 'TRANSFER') {
                        ...
                        data.push({
                            ...
                            table: 'Transfer ' + request.id + ' created<br>Destination RSE: ' + request.dest_rse_id
                        })
        ...
        Promise.all(promises).then(function(data) {
            concated_request_data = []
            data.forEach(function(request_data) {
                concated_request_data = concated_request_data.concat(request_data)
            })
            resolve({'transfers': concated_request_data})
...
show_did_tracking_table = function(tableData) {
    for(var i=0; i < tableData.length; i++) {
        if(tableData[i]['type'] == TimelineChart.TYPE.POINT) {
            $('#tracking_table').append($('<tr><th>' + tableData[i]['at'].toLocaleString() + '</th><td>' + tableData[i]['table'] + '</td></tr>'));
        }
    }
}
