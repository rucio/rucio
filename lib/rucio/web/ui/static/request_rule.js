    ...
    r.list_rses({
        ...
        success: function(rses) {
            ...
            $.each(rses, function(index, rse) {
                ...
                if (rse['rse'] in account_limits) {
                    has_quota = true;
                    button_enabled = true;
                    bytes_limit = filesize(account_limits[rse['rse']]);
                    bytes_remaining = bytes_limit;
                }
                if (rse['rse'] in account_usages) {
                    bytes_remaining = filesize(account_usages[rse['rse']]);
                }
            ...
            if (has_quota) {
                storage.set('approval', false);
                create_rse_table();
            } else {
                storage.set('approval', true);
                do_approval();
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
...
create_rse_select = function() {
    ...
    $('#search_rse_button').on('click', search_rses);
};
...
create_rules = function(approval) {
    ...
    options['error'] = function(jqXHR, textStatus, errorThrown) {
        ...
        if (error_details['ExceptionClass'] == 'DuplicateRule') {
            ...
            html_check = '<font color="red">The RSE that you chose is currently blacklisted. Please check <a href="http://atlas-agis.cern.ch/agis/ddmblacklisting/list/" target="_blank">here</a> for more information.</font>';
        } else {
...
create_summary = function(approval) {
    ...
    $.each(selected_dids, function(index, did) {
        if (nbfiles != null ) {
            ...
            data.push({'did': new_scope + ':' + new_name, 'copies': copies, 'size': '-', 'requested_size': '-'});
    ...
    $(dt.column(3).footer()).html(filesize(total_size));
    $(dt.column(4).footer()).html(filesize(total_requested_size));
