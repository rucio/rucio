...
create_did_list = function(scope, dids) {
    ...
    $.each(dids, function(index, value) {
        data.push({
            '$name': value,
            'name': $('<div>').text(value).html(),
            'selected': $('<div>').append($('<input>', {'type': 'checkbox', 'class': 'inline', 'name': 'checkbox_' + value})).html(),
        });
    });
...
create_multi_did_list = function(table_data) {
    ...
    $.each(table_data, function(index, value) {
        data.push({
            '$scope': value['scope'],
            'scope': $('<div>').text(value['scope']).html(),
            '$name': value['name'],
            'name': $('<div>').text(value['name']).html(),
            '$type': value['type'],
            'type': $('<div>').text(value['type']).html(),
            'selected': $('<div>').append($('<input>', {'type': 'checkbox', 'class': 'inline', 'name': 'checkbox_' + value['scope'] + '_' + value['name']})).html(),
        });
    });
...
create_rse_table = function(quota){
    ...
    $("#rse_total_did_size").text('Total size of selected DIDs: ' + filesize(total_selected_size, {'base': 10}));
    var html_continue = '';
    if (!quota) {
        ...
        success: function(rses) {
            ...
            $.each(rses, function(index, rse) {
                ...
                if ((rse['rse'] in account_limits) && (rse['rse'] in account_usages)) {
                    bytes_remaining = $('<font></font>').text(filesize(account_usages[rse['rse']], {'base': 10}));
                    bytes_limit = $('<div>').text(filesize(account_limits[rse['rse']], {'base': 10})).html();
                    if (account_usages[rse['rse']] >= total_size) {
                        has_quota = 'Y';
                        button_enabled = true;
                        bytes_remaining = bytes_remaining.html(); // remove font element
                    } else {
                        has_quota = 'R';
                        bytes_remaining = $('<div>').append(bytes_remaining.attr('color', 'red')).html();
                    }
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            if (error_detail['ExceptionClass'] == 'InvalidRSEExpression') {
                if (error_detail['ExceptionMessage'] == 'RSE Expression resulted in an empty set.') {
                    rseTable.append('<font color="red">No RSE found that matches your input.<font>');
                } else {
                    rseTable.append($('<font color="red"></font>').text(error_detail['ExceptionMessage']));
                }
            } else {
                rseTable.append($('<font color="red"></font>').text(error_details['ExceptionClass'] + ': ' + error_details['ExceptionMessage']));
            }
        }
    });
};
...
create_rules = function(approval) {
    ...
    options['success'] = function(data) {
        $("#main").html(html_ok);
        $("#list_rules").text('Or you can find a list of all your rules ').append($('<a></a>', {'href': '/r2d2?account=' + account}).text('here'));
        table_data = [];
        $.each(data, function(index, rule_id) {
            table_data.push({
                'id': $('<div>').append($('<a></a>', {'href': '/rule?rule_id=' + rule_id}).text(rule_id)).html(),
                'did': $('<div>').text(options['dids'][index]['scope'] + ":" + options['dids'][index]['name']).html(),
            });
        });
    ...
    options['error'] = function(jqXHR, textStatus, errorThrown) {
        ...
        if (error_details['ExceptionClass'] == 'DuplicateRule') {
            $("#error_msg").html('<font color="red">The rule that you are trying to create already exists, please go back and choose another DID or RSE.</font>');
        } else if (error_details['ExceptionClass'] == 'RSEWriteBlocked') {
            $("#error_msg").html('<font color="red">The RSE that you chose is currently not available for writing. Please check <a href="https://atlas-cric.cern.ch/atlas/ddmendpointstatus/list/" target="_blank">here</a> for more information.</font>');
        } else {
            $("#error_msg").empty().append($('<font color="red"></font>').text(error_details['ExceptionClass'] + ': ' + error_details['ExceptionMessage']));
        }
        $('#submit_progress').html('');
        $("#error_msg").html(html_check);
        console.log(jqXHR);
...
create_summary = function(approval) {
    ...
    $.each(selected_dids, function(index, did) {
        if (nbfiles != null ) {
            ...
            data.push({
                'did': $('<div>').text(new_scope + ':' + new_name).html(),
                'copies': $('<div>').text(copies).html(),
                'size': '-',
                'requested_size': '-',
                'files': $('<div>').text(nbfiles).html(),
            });
            ...
            r.get_did({
                ...
                success: function(metadata){
                    ...
                    did = $('<div>').text(did['scope'] + ':' + did['name']).html();
                    if (metadata['open']) {
                        open = true;
                        did = "<b>" + did + '</b>';
                    }
                    data.push({
                        'did': did,
                        'copies': $('<div>').text(copies).html(),
                        'files': $('<div>').text(files).html(),
                        'size': $('<div>').text(size).html(),
                        'requested_size': $('<div>').text(requested_size).html(),
                    });
                }
            });
        }
