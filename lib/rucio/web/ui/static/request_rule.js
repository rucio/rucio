        ...
        r.list_rses({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                if (errorThrown == 'Bad Request') {
                    if (jqXHR.responseText.split('\n')[1].split(': ')[1] == 'RSE Expression resulted in an empty set.') {
                        $("#rse_table").html('No RSEs are found that match your input.');
                    } else if (jqXHR.responseText.split('\n')[1].split(': ')[1] == 'Expression does not comply to RSE Expression syntax') {
                        $("#rse_table").html('Your RSE is expression syntax is malformed.');
                    } else {
                        $("#rse_table").html('No RSEs could found.');
                    }
                } else {
                    $("#rse_table").html('No RSEs could found.');
                }
