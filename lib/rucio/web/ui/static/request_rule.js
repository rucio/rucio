...
create_rse_table = function(data){
...
create_rse_select = function() {
    ...
    $('#search_rse_button').on('click', function() {
        ...
        r.list_rses({
            'expression': expr,
            success: function(data) {
                storage.set('rse_data', data);
                create_rse_table(data);
            },
            error: function(jqXHR, textStatus, errorThrown) {
                console.log(jqXHR);
            }
...
create_summary = function() {
    ...
    $.each(rses, function(index, rse) {
        r.get_account_usage({
            ...
            async: false,
            success: function(data) {
                bytes_remaining = data[0]['bytes_remaining'] / 1000 /1000/1000;
                bytes_limit = data[0]['bytes_limit'] / 1000 /1000/1000;
