    ...
    r.list_rules({
        ...
        success: function(data) {
            ...
            $.each(data, function(index, value) {
                ...
                filtered_data.push({
                    '$id': value.id, // not displayed, but used in API calls
                    '$scope': value.scope, // not displayed, but used in API calls
                    '$name': value.name, // not displayed, but used in API calls
                    'scope': $('<div>').text(String(value.scope)).html(),
                    'name': $('<div>').text(String(value.name)).html(),
                    'link': $('<div>').append($('<a></a>', {'href': '/rule?rule_id=' + value.id}).html(scope_name_html)).html(),
                    'account': $('<div>').text(String(value.account)).html(),
                    ...
                    'length': "[loading]",
                    'open': "[loading]",
                    'did_type': $('<div>').text(value.did_type).html(),
                    'grouping': $('<div>').text(value.grouping).html(),
                    'comments': $('<div>').text(value.comments).html(),
                    'approve': $('<div>').append($('<a></a>', {'id': 'approve_' + value.id, 'href': '#', 'class': 'button tiny success postfix inline'}).text('Approve')).html(),
                    'deny': $('<div>').append($('<a></a>', {'id': 'deny_' + value.id, 'href': '#', 'class': 'button tiny alert postfix inline'}).text('Deny')).html(),
                });
            });
            ...
            dt.rows().eq(0).each(function(index){
                ...
                r.get_did({
                    ...
                    success: function(meta) {
                        d.open = $('<div>').text(String(meta.open)).html();
                        d.filesize = $('<div>').text(String(filesize(meta.bytes, {'base': 10}))).html();
                        d.length = $('<div>').text(String(meta.length)).html();
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        let loader = $('#loader');
                        loader.empty();
                        loader.append($('<text color="red"></text>').text(jqXHR.responseText.split(':')[1]));
                        loader.append('<br/>');
                    }
                });
            });
...
$(document).ready(function(){
    $('#subbar-details').text('[' + url_param('account') + ', ' + url_param('name') + ', ' + url_param('state') + ', ' + url_param('rse_expression')+ ']');
