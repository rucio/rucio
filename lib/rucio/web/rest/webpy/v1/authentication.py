...
class OIDC(RucioController):
    """
    Requests a user specific Authentication URL (assigning a user session state,
    nonce, Rucio OIDC Client ID with the correct issuers authentication endpoint).
    In principle this step could be done 'by hand' from the Baseclient, but it
    would require hard-coding issuer & Rucio OIDC Client related configuration parameters
    in the Baseclient (such as authorisation endpoint, issuers URL, client_id etc.).
    """
    ...
    def GET(self):
        ...
        try:
            result = get_auth_OIDC(account, server_name)
        except AccessDenied:
...
class OIDC_token(RucioController):
    """
    Authenticate a Rucio account temporarily via ID, access and refresh tokens from an Identity Provider (XDC IAM as of June 2019).
    """
