# Copyright European Organization for Nuclear Research (CERN)
#
# Licensed under the Apache License, Version 2.0 (the "License");
# You may not use this file except in compliance with the License.
# You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
#
# Authors:
# - Vincent Garonne, <vincent.garonne@cern.ch>, 2012
# To generate documentation:
#    nosetests --nocapture lib/rucio/tests/test_demo.py > doc/source/cli_examples.rst; python setup.py build_sphinx

from os import remove

from rucio.db.session import build_database, destroy_database, create_root_account
from rucio.tests.common import execute


class TestRucioDemo:

    @classmethod
    def setUpClass(cls):
        #destroy_database(echo=False)
        #build_database(echo=False)
        #create_root_account()
        try:
            remove('/tmp/.rucio_root/auth_token_root')
        except OSError, e:
            if e.args[0] != 2:
                raise e
        cmd = 'rm -rf /tmp/download/*'
        exitcode, out, err = execute(cmd)
        cmd = 'rm -rf /tmp/rucio_rse/*/'
        exitcode, out, err = execute(cmd)

    @classmethod
    def tearDownClass(cls):
        pass
        destroy_database(echo=False)

    def setUp(self):
        self.marker = '   $> '

    def test_rucio_demo(self):
        """ CLI(DEMO): Test the rucio demo """

        cmd = 'source /afs/cern.ch/atlas/offline/external/GRID/ddm/rucio/testing/bin/activate'
        cmd = 'cat /afs/cern.ch/atlas/offline/external/GRID/ddm/rucio/testing/etc/rucio.cfg'
        cmd = 'curl -s  -X GET http://localhost/ping'
        cmd = 'curl -s -X GET http://atlas-rucio.cern.ch/ping'

        cmd = 'rucio ping'
        # print self.marker + cmd
        exitcode, out, err = execute(cmd)
        # print out

        cmd = 'rucio whoami'
        # print self.marker + cmd
        exitcode, out, err = execute(cmd)
        # print out
        header = '''..
      Copyright European Organization for Nuclear Research (CERN)

      Licensed under the Apache License, Version 2.0 (the "License");
      You may not use this file except in compliance with the License.
      You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0

==================
Rucio CLI Examples
==================

'''
        print header

        print '``rucio --version``'
        print '-------------------'
        print 'Print version information::\n'
        cmd = 'rucio --version'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio ping``'
        print '--------------'
        print 'Discover server version information::\n'
        cmd = 'rucio ping'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        print '   ' + out

        print '``rucio add-replicas``'
        print '----------------------'
        print 'Add file replicas::\n'
        cmd = 'rucio add-replicas  --lfns vgaronne:Myfile vgaronne:Myfile1 vgaronne:Myfile2 --rses MOCK1 MOCK1 MOCK1 --checksums ad:92ce22ac ad:92ce22ab ad:92ce22ac  --sizes 2849063278 2849063277 2849063279 --dsn vgaronne:MyDataset'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio add``'
        print '-------------'
        print 'Add dataset/container and content::\n'

        cmd = 'rucio add --dest vgaronne:MyDataset1 --srcs vgaronne:Myfile vgaronne:Myfile1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio add --dest vgaronne:MyDataset2 --srcs vgaronne:Myfile2'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio add --dest vgaronne:MyContainer1 --srcs vgaronne:MyDataset1 vgaronne:MyDataset2'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio add --dest vgaronne:MyBigContainer1 --srcs vgaronne:MyContainer1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio del``'
        print '--------------'
        print 'delete container/dataset or contents::\n'

        cmd = 'rucio del vgaronne:MyDataset1 --from vgaronne:MyContainer1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio list``'
        print '--------------'
        print 'List container/dataset contents::\n'

        cmd = 'rucio list vgaronne:MyDataset'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio list vgaronne:MyBigContainer1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio list vgaronne:MyContainer1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio list vgaronne:MyDataset1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio list-files``'
        print '-----------------------'
        print 'List file contents::\n'

        cmd = 'rucio list-files vgaronne:MyDataset1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio list-files vgaronne:MyBigContainer1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio list-replicas``'
        print '-----------------------'
        print 'List file replicas::\n'

        cmd = 'rucio list-replicas vgaronne:Myfile1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio list-replicas vgaronne:MyDataset'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'rucio list-replicas vgaronne:MyBigContainer1'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio upload``'
        print '----------------'
        print 'Upload data into rucio::\n'

        cmd = 'rucio upload --rse MOCK --scope vgaronne --files Myfile4 --dsn Mydataset4'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio download``'
        print '------------------'
        print 'download data from rucio::\n'

        cmd = 'rucio download --dir=/tmp/download  vgaronne:Myfile4'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        cmd = 'ls /tmp/download/vgaronne/'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

#         cmd = 'rucio download --dir=/tmp/download  vgaronne:Myfile4'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 'ls /tmp/download/vgaronne/'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 'rucio-admin rse add swift.cern.ch'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 's3cmd mb s3://RSETESTS3'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 'rucio upload --rse swift.cern.ch  --scope vgaronne --files Myfile5'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 'rucio download --dir=/tmp/download  vgaronne:Myfile5'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 'ls /tmp/download/vgaronne/'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 's3cmd ls  s3://RSETESTS3'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out
#
#         cmd = 's3cmd ls  s3://RSETESTS3/vgaronne'
#         print self.marker + cmd
#         exitcode, out, err = execute(cmd)
#         print out

        print '``rucio search``'
        print '----------------'
        print 'Search data identifiers::\n'
        print 'To Do\n'

        print '``rucio get-metadata``'
        print '----------------------'
        print 'To Do\n'

        print '``rucio set-metadata``'
        print '----------------------'
        print 'To Do\n'

        print '``rucio del-metadata``'
        print '----------------------'
        print 'To Do\n'

        print '``rucio list-rse-usage``'
        print '------------------------'
        print 'To Do::\n'
        cmd = 'rucio list-rse-usage MOCK'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l
        cmd = 'rucio list-rse-usage --history MOCK'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l

        print '``rucio list-account-usage``'
        print '----------------------------'
        print 'To Do::\n'
        cmd = 'rucio list-account-usage --history'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l
        cmd = 'rucio list-account-limits'
        print self.marker + cmd
        exitcode, out, err = execute(cmd)
        for l in out.split('\n'):
            print '   ' + l
    #  cmd = 'rucio close vgaronne:MyDataset1'
