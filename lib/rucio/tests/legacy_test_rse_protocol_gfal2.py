...
class TestRseGFAL2(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for f in MgrTestCases.files_remote:
            # i assume this just samples a random PFN

            pfns = mgr.lfns2pfns(rse_settings, {'name': f, 'scope': f'user.{cls.user}'}, scheme='srm')
            tmp = next(iter(pfns.values()))  # only value entry in the dict
            cmd = f'srmcp -2 --debug=false -retry_num=0  file:///{cls.tmpdir}/data.raw {tmp}'
            execute(cmd)
