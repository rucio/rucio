...
class TestRseSSH(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        clean_files = str(execute(list_files_cmd_user)).split('\n')[1:]
        list_files_cmd_group = 'ssh %s@%s find %s/group.%s' % (sshuser, hostname, prefix, cls.user)
        clean_files += str(execute(list_files_cmd_group)).split('\n')[1:]
        ...
        clean_directory = str(execute(list_directory)).split('\n')
        list_directory_group = 'ssh %s@%s find %s/group.%s' % (sshuser, hostname, prefix, cls.user)
        clean_directory +=  str(execute(list_directory_group)).split('\n')
