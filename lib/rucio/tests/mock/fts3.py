...
def submit(tinfo, session):
    ...
    tid = generate_uuid()
    record_timer('daemons.mock.fts3.submit.000-generate_uuid', (time.time() - time_start) * 1000)

    time_start = time.time()
    new_transfer = test_models.MockFTSTransfer(transfer_id=tid, transfer_metadata=str(tinfo))
    new_transfer.save(session=session)
    record_timer('daemons.mock.fts3.submit.001-new_transfer', (time.time() - time_start) * 1000)
...
def query(tid, session):
    ...
    record_timer('daemons.mock.fts3.query.000-random_sample', (time.time() - time_start) * 1000)
    ...
    res = {'job_state': str(new_state)}
    if new_state == FTSState.FAILED or new_state == FTSState.FINISHEDDIRTY:
        res['reason'] = 'Mock FTS decided to kill your transfer.'
        res['files'] = [{'source_surl': 'mock_src', 'dest_surl': 'mock_dest', 'reason': 'mock failure'}]
...
def cancel(tid, session):
    ...
    record_timer('daemons.mock.fts3.cancel.update_state', (time.time() - time_start) * 1000)
