...
class UploadClient:
    ...
    def __init__(
        ...
        :param _client: An existing Rucio `Client` instance to reuse. If not provided, a new one is created.
        :param logger: A logger function. If not provided, the default Python logger is used.
        :param tracing: Indicates whether to enable tracing to capture upload activity details.

        :raises InputValidationError: If the client account is not found or is invalid, preventing upload setup.
        """
        if not logger:
    ...
    def upload(
        ...
        An overview of this method's performed actions:
            1. Collects and validates file info from the passed `items` (directories may
               be also included), ensuring valid paths exist on the local filesystem. If
               an RSE expression is provided, a single RSE is picked at random from it.
            2. Checks the RSE's availability for writing (unless `ignore_availability` is True).
            3. Optionally registers each file in the Rucio Catalog, handling the DID
               creation, dataset creation/attachment, and replication rules as needed.
            4. Uploads the files using the underlying protocol handlers and verifies
               checksums if desired/possible. Partial or failed uploads raise exceptions.
            5. (Optional) Produces a JSON summary file at `summary_file_path`, listing the
                final PFNs, checksums, and other info for all successfully uploaded files.

        :param items: A sequence of dictionaries, each describing a file to upload (or a
            ...
            - `rse` (str, required):
              The target RSE or an RSE expression where the upload should be placed. If an
              expression is provided (e.g., "tier=1"), one RSE from that expression is
              chosen randomly.

            - `did_scope` (str, optional):
            ...
            - `lifetime` (int, optional):
              The lifetime (in seconds) to apply when creating a new replication rule. For
              file uploads without a dataset, a new rule with that lifetime is created if
              the file DID does not already exist in Rucio. For a new dataset, the dataset
              is created with a rule using this lifetime, but if the dataset already exists
              and you specify a lifetime, an error is raised. Note that lifetime is not
              automatically applied to nested containers or datasets in recursive mode.

            - `impl` (str, optional):
              Name of the protocol implementation to be used for uploading this item. For
              example `"rucio.rse.protocols.gfal.Default"`.
            ...
            - `transfer_timeout` (int, optional):
              A maximum duration (in seconds) to wait for each individual file transfer to
              complete. If the file transfer does not finish before this timeout elapses,
              the operation will be aborted and retried one last time. When transfer_timeout
              is None, no specific timeout is enforced, and the transfer may continue until
              it completes or fails for another reason.
            ...
            - `no_register` (bool, optional, default=False):
              ...
              you plan to register the replica or create rules separately. Note that if
              recursive=True, the method still creates datasets and/or containers for
              the directories when needed.

            - `register_after_upload` (bool, optional, default=False):
            ...
            - `dataset_scope` / `dataset_name` (str, optional):
              ...
              already exists and you specify a new lifetime, or if a checksum mismatch is
              detected, registration fails. In non-recursive mode, only files in the top-level
              directory are attached to the dataset and subdirectories are skipped with a
              warning. In recursive mode, the client aims to create containers for directories
              containing only subdirectories and datasets for directories containing only
              files (raising an error if the top-level folder mixes files and directories).
              If the top-level directory has subdirectories, the user-supplied dataset_name
              is effectively ignored at that level (each subdirectory becomes its own dataset
              or container); if there are no subdirectories, the entire folder is registered
              as a single dataset.

            - `dataset_meta` (dict, optional):
              Additional metadata (e.g., `{'project': 'myProject'}`) to attach to the newly
              created dataset when: the dataset does not already exist, `recursive=False`,
              `no_register=False` and both `dataset_scope` and `dataset_name` are provided.
              Note that if multiple files share the same `dataset_scope` and `dataset_name`,
              then if a dataset is created, it considers only the first item’s dataset_meta.

        :param summary_file_path: If specified, a JSON file is created with a summary of each
                                  successfully uploaded file, including checksum, PFN, scope,
                                  and name entries.

        :param traces_copy_out: A list reference for collecting the trace dictionaries that
                                Rucio generates while iterating over each file. A new trace
                                dictionary is appended to this list for each file considered
                                (even those ultimately skipped or already on the RSE).

        :param ignore_availability: If set to True, the RSE's "write availability" is not
                                    enforced. By default, this is False, and an RSE marked
                                    as unavailable for writing will raise an error.

        :param activity: If you are uploading files without a parent dataset, this string
                         sets the “activity” on the replication rule that Rucio creates for
                         each file (e.g., "Analysis"), which can affect RSE queue priorities.
                         Note that if your files are uploaded into a dataset, the dataset’s
                         replication rule does not use this activity parameter.

        :returns: Integer status code, always 0 if all files were uploaded successfully.

        :raises NoFilesUploaded:
            Raised if none of the requested files could be uploaded.

        :raises NotAllFilesUploaded:
            Raised if some files were successfully uploaded, but others failed.

        :raises RSEWriteBlocked:
            Raised if `ignore_availability=False` but the chosen RSE does not allow writing.

        :raises InputValidationError:
            Raised if mandatory fields are missing, if conflicting DIDs are found, or if
            no valid files remain after input parsing.
        """
        ...
        for file in files:
            ...
            rse = self.rse_expressions.setdefault(rse_expression,
                                                  _pick_random_rse(rse_expression))
        ...
        for file in files:
            ...
            if register_after_upload:
                if rsemgr.exists(rse_settings,
                                 ...
                                 vo=self.client.vo,
                                 logger=logger):  # type: ignore (pfn is str)
                    try:
                ...
                if dataset_did_str and not no_register:
                    try:
                        self.client.attach_dids(
                            file['dataset_scope'],
                            file['dataset_name'],
                            [file_did])  # type: ignore (`dataset_scope` and `dataset_name` always exist if `dataset_did_str`)
                    except Exception as error:
                        registration_succeeded = False
    ...
    def _add_bittorrent_meta(
        ...
        :param file: A dictionary that must include 'dirname', 'basename', 'did_scope',
    ...
    def _register_file(
        ...
        :param file: A dictionary containing file information (e.g., 'did_scope', 'did_name', 'adler32', etc.).
        :param registered_dataset_dids: A set of dataset DIDs already registered to avoid duplicates.
        :param ignore_availability: If True, creates replication rules even when the RSE is marked unavailable.
        :param activity: Specifies the transfer activity (e.g., 'User Subscriptions') for the replication rule.

        :raises InputValidationError: If a dataset already exists but the caller attempts to set a new lifetime for it.
        :raises DataIdentifierAlreadyExists: If the local checksum differs from the remote checksum.
        """
        logger = self.logger
    ...
    def _get_file_guid(
        ...
        :param file: A dictionary describing the file, expected to include:
                     - `basename`: The base filename.
                     - `path`: The path to the file.
                     - `guid` (optional): A pre-assigned GUID string.
                     - `no_register` (optional): If True, skip attempts to derive a GUID for ROOT files.

        :returns: A string containing the file's GUID, stripped of dashes and in lowercase.

        :raises RucioException: If GUID extraction using the `pool_extractFileIdentifier`
                                command fails.
        """
        guid = file.get('guid')
    ...
    def _collect_file_info(
        ...
        :param filepath: The local filesystem path to the file.
        :param item: A dictionary containing initial upload parameters (e.g., RSE name,
                     scope) for the file. Some of its fields may be updated or augmented
                     in the returned dictionary.

        :returns: A new dictionary enriched with relevant file descriptors.
        """
    ...
    def _collect_and_validate_file_info(
        ...
        """
        Collect and verify local file info for upload, optionally registering folders as datasets/containers.
        ...
        :param items: An iterable of dictionaries describing files or directories, where
                      each dictionary typically has:
                      - `path`: Local file system path
                      - `rse`: Name of the RSE destination
                      - `pfn` (optional): Physical file name (PFN)
                      - `impl` (optional): Protocol implementation
                      - `recursive` (optional): Whether to traverse directories recursively

        :returns: A list of dictionaries enriched with file descriptors (size, checksums, etc.)
                  and ready for further upload processing.

        :raises InputValidationError: If no valid files are found.
        """
        logger = self.logger
    ...
    def _convert_file_for_api(
        ...
        :param file: A dictionary describing a file, expected to include at least
                     `did_scope`, `did_name`, `bytes`, `adler32`, `md5`,
                     `meta`, `state`, and optionally `pfn`.

        :returns: A dictionary containing only the relevant file attributes for Rucio's REST API.
        """
        replica = {}
    ...
    def _upload_item(
        ...
        :param rse_settings: Dictionary containing the RSE configuration.
        :param rse_attributes: Additional attributes of the RSE (e.g. 'archive_timeout').
        :param lfn: An optional dictionary describing the logical file
                    (e.g., {'name': '1_rse_local_put.raw', 'scope': 'user.jdoe', ..}).
                    If the 'filename' key is present, it overrides 'name' in determining
                    the local file name to read from source_dir.
        :param source_dir: Local source directory path where the file to be uploaded resides.
        :param domain: Network domain for the upload, commonly 'wan' for wide-area networks.
        :param impl: Name of the protocol implementation to be enforced (if any).
        :param force_pfn: If provided, forces the use of this PFN for the file location on
                          the storage (use with care since it can lead to "dark" data).
        :param force_scheme: If provided, forces the protocol scheme (e.g. 'davs', 'https') to be used.
        :param transfer_timeout: Timeout (in seconds) for the transfer operation before it fails.
        :param delete_existing: If True, removes any unregistered or stale file on the storage that matches this PFN.
        :param sign_service: If set, requests a signed URL from the given service (e.g. gcs, s3, swift).

        :returns: The final PFN (physical file name) of the successfully uploaded file, or None if creation failed.

        :raises FileReplicaAlreadyExists: If the target file already exists and overwrite is not allowed.
        :raises RSEOperationNotSupported: If storage-side operations (delete/rename/put) are not supported or fail.
        :raises RucioException: If renaming or other critical operations cannot be completed.
        """
    ...
    def _retry_protocol_stat(
        ...
        :param protocol: The RSEProtocol instance to use for retrieving file statistics
        :param pfn: The physical file name (PFN) to be checked.

        :returns: A dictionary expected to include the filesize and adler32 for the provided pfn.

        :raises RSEChecksumUnavailable: If the protocol indicates a missing checksum for the file.
        :raises Exception: If the requested service is not available or permissions are not granted.
        """
    ...
    def _create_protocol(
        ...
        :param rse_settings: The dictionary containing RSE configuration.
        :param operation: The intended operation, such as 'read', 'write', or 'delete'.
        :param impl: An optional override for the default protocol implementation.
        :param force_scheme: If provided, forces the protocol to use this scheme.
        :param domain: The network domain to be used, defaulting to 'wan'.

        :returns: The instantiated `RSEProtocol` object.

        :raises Exception: If the protocol creation or connection attempt fails.
        """
        try:
    ...
    def _send_trace(
        ...
        :param trace: The trace object to be sent.
        """
        if self.tracing:
    ...
    def _recursive(
        ...
        structure. Note that currently, this method does not allow the top-level directory
        to contain both files and subdirectories.

        :param item: A dictionary describing the local path and upload parameters.
                     It must contain at least:
                     - `rse`: The target RSE for the upload.
                     - `path`: The local directory path to inspect.
                     - `did_scope` (optional): Custom scope for the resulting datasets/containers.

        :returns: A list of file descriptors enriched with collected file information,
                  each conforming to FileToUploadWithCollectedAndDatasetInfoDict.

        :raises InputValidationError: If a folder contains both files and subdirectories
                                      at its top level (invalid container/dataset structure).
        """
    ...
    def preferred_impl(
        ...
        :param rse_settings: A dictionary describing RSE details, including available protocols and their domains.
        :param domain: The network domain (e.g., 'lan' or 'wan') in which the protocol must support all operations.

        :returns: The name of a protocol implementation that can handle read/write/delete
