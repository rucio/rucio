...
class DownloadClient:
    ...
    def _list_scopes_str(self) -> list[str]:
        '''Get list of known scope names.'''
        # server versions >= 40 return dicts of scope, owner
        scopes = [
            scope['scope'] if isinstance(scope, dict) else scope
            for scope in self.client.list_scopes()
        ]
