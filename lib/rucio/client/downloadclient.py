...
class DownloadClient:
    ...
    def download_file_from_archive(self, items, trace_custom_fields={}):
        ...
        for item in items:
            ...
            if not rse_name or rse_name == None:
            ...
            dest_dir_subdirs = archive_scope
            if not item.get('no_subdir'):
                archive_subdir = archive_name
                archive_subdir += '.extracted'
                dest_dir_subdirs += '/%s' % archive_subdir
            dest_dir_path = self._prepare_dest_dir(item.get('base_dir', '.'),
                                                   dest_dir_subdirs, file_extract,
                                                   item.get('no_subdir'))
    ...
    def _prepare_dest_dir(self, base_dir, dest_dir_name, file_name, no_subdir):
        ...
        if no_subdir:
            ...
            dest_dir_names = dest_dir_name.split('/')
            for sub_dir in dest_dir_names:
                dest_dir_path = os.path.join(dest_dir_path, sub_dir)
