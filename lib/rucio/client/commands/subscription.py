...
class Subscription(CommandBase):
    ...
    def _operations(self) -> dict[str, "OperationDict"]:
        return {
            ...
            "set": {"call": self.set_, "docs": "Update an existing subscription.", "namespace": self._common_namespace},
    ...
    def list_(self):
        list_subscriptions(self.args, self.client, self.logger)

    def add(self):
        add_subscription(self.args, self.client, self.logger)

    def set_(self):
        update_subscription(self.args, self.client, self.logger)

    def touch(self):
        reevaluate_did_for_subscription(self.args, self.client, self.logger)
