...
class Config(CommandBase):
    ...
    def _operations(self) -> dict[str, "OperationDict"]:
        return {
            ...
            "set": {"call": self.set_, "docs": "Update the existing configuration settings. WARNING: Changes the global config.", "namespace": self.namespace},
    ...
    def list_(self):
        get_config(self.args, self.client, self.logger)

    def set_(self):
        set_config_option(self.args, self.client, self.logger)

    def remove(self):
        delete_config_option(self.args, self.client, self.logger)
