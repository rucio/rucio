...
class CommandBase(ABC):
    def __init__(self, client: Optional["Client"] = None, args: Optional["Namespace"] = None, logger: Optional["Logger"] = None) -> None:
    ...
    def _help(self) -> str:
        ...
        try:
            ...
            pass
        if len(self.implemented_subcommands()) != 0:
            help += f"\nSubcommands: {list(self.implemented_subcommands().keys())}"
    ...
    def parser(self, subparser: "_SubParsersAction[ArgumentParser]") -> "ArgumentParser":
        ...
        try:
            ...
            for _, subcommand in subcommands.items():
                ...
                subcommand(self.client, self.args, self.logger).subparser(subcommand_parser)
    ...
    def _execute_subcommand(self, subcommands: dict, requested_subcommand: str, requested_verb: Optional[str] = None):
        if requested_subcommand in subcommands.keys():
            subcommand_client = subcommands[requested_subcommand](self.client, self.args, self.logger)
