...
class RSE(CommandBase):
    ...
    def _operations(self) -> dict[str, "OperationDict"]:
        return {
            ...
            "info": {"call": self.info, "docs": "Show history of RSE usage (date created, date updated, etc)", "namespace": self.namespace},
            ...
            "set": {"call": self.set_, "docs": "Update an existing RSE.", "namespace": self.namespace},
        }
    ...
    def list_(self):
        list_rses(self.args, self.client, self.logger)

    def add(self):
        add_rse(self.args, self.client, self.logger)

    def set_(self):
        update_rse(self.args, self.client, self.logger)

    def info(self):
        info_rse(self.args, self.client, self.logger)

    def remove(self):
        disable_rse(self.args, self.client, self.logger)
...
class Attribute(RSE):
    ...
    def _operations(self) -> dict[str, "OperationDict"]:
        return {"list": {"call": self.list_, "docs": "Show existing attributes for an RSE."}, "set": {"call": self.set_, "docs": "Update an RSE's setting."}, "remove": {"call": self.remove, "docs": "Wipe an RSE's setting."}}
    ...
    def set_(self):
        set_attribute_rse(self.args, self.client, self.logger)

    def remove(self):
        delete_attribute_rse(self.args, self.client, self.logger)

    def list_(self):
        get_attribute_rse(self.args, self.client, self.logger)
...
class Distance(RSE):
    ...
    def list_(self):
        get_distance_rses(self.args, self.client, self.logger)

    def add(self):
        add_distance_rses(self.args, self.client, self.logger)

    def set_(self):
        update_distance_rses(self.args, self.client, self.logger)

    def remove(self):
        delete_distance_rses(self.args, self.client, self.logger)
...
class Protocol(RSE):
    ...
    def module_help(self) -> str:
        return "Manage RSE transfer and storage protocols. Use `$ rucio rse info` to view an RSE's existing protocols."
    ...
    def add(self):
        add_protocol_rse(self.args, self.client, self.logger)

    def remove(self):
        del_protocol_rse(self.args, self.client, self.logger)
...
class Limit(RSE):
    ...
    def namespace(self, parser: "ArgumentParser") -> None:
        ...
        parser.add_argument("--limit", dest="value", help="Value of the limit in ??? Units")  # TODO Units.

    def usage_example(self) -> list[str]:
        return ["$ rucio rse limit add --rse XRD1 --name MinFreeSpace --value 10000", "$ rucio rse limit --rse XRD3 --name MinFreeSpace"]

    def add(self):
        set_limit_rse(self.args, self.client, self.logger)

    def remove(self):
        delete_limit_rse(self.args, self.client, self.logger)
...
class QOS(RSE):
    ...
    def module_help(self) -> str:
        return "I have no idea what this does and documentation doesn't help. It's dire out here."
    ...
    def add(self):
        add_qos_policy(self.args, self.client, self.logger)

    def remove(self):
        delete_qos_policy(self.args, self.client, self.logger)

    def list_(self):
        list_qos_policies(self.args, self.client, self.logger)
