...
class Rule(CommandBase):
    ...
    def _operations(self) -> dict[str, "OperationDict"]:
        return {
            ...
            "info": {"call": self.info, "docs": "Retrieve information about a specific rule.", "namespace": self.info_namespace},
            "history": {"call": self.history, "docs": "List history of different replica rules impacting a DID", "namespace": self._common_namespace},
            "set": {"call": self.set_, "docs": "Update replication rule, can be used to move a rule from one RSE to another.", "namespace": self.set_namespace},
    ...
    def list_(self):
        list_rules(self.args, self.client, self.logger)

    def add(self):
        add_rule(self.args, self.client, self.logger)

    def remove(self):
        delete_rule(self.args, self.client, self.logger)

    def info(self):
        info_rule(self.args, self.client, self.logger)

    def history(self):
        list_rules_history(self.args, self.client, self.logger)

    def set_(self):
        # typing issue is due to trying to map two argparser objects to one function
        # Claims you can't add new args
        if self.args.move:  # type: ignore
            self.args.rule_activity = self.args.activity  # type: ignore
            move_rule(self.args, self.client, self.logger)

        else:
            self.args.rule_activity = self.args.activity  # type: ignore
            update_rule(self.args, self.client, self.logger)
