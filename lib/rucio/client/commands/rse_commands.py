...
def list_(ctx, rses):
    ...
    list_rses(Arguments({"rses": rses}), ctx.obj.client, ctx.obj.logger, ctx.obj.console, ctx.obj.spinner)
...
def show(ctx, rse_name):
    ...
    info_rse(Arguments({"rse": rse_name}), ctx.obj.client, ctx.obj.logger, ctx.obj.console, ctx.obj.spinner)
