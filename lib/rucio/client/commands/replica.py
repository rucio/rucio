...
class Replica(CommandBase):
    ...
    def usage_example(self) -> list[str]:
        return ["$ rucio replica --did user.jdoe:test_file  # Show all replicas for user.jdoe:test_file, with their pfn and rse.", "$ rucio replica dataset --did user.jdoe:test_dataset  # Show all replicas for the dataset user.jdoe:test_dataset", "$ "]

    def default_operation(self):
        return self.list_

    def list_(self):
        list_file_replicas(self.args, self.client, self.logger)

    def dataset(self):
        list_dataset_replicas(self.args, self.client, self.logger)

    def pfn(self):
        list_pfns(self.args, self.client, self.logger)

    def tombstone(self):
        set_tombstone(self.args, self.client, self.logger)
...
class State(Replica):
    ...
    def suspicious(self):
        list_suspicious_replicas(self.args, self.client, self.logger)

    def quarantine(self):
        quarantine_replicas(self.args, self.client, self.logger)

    def bad(self):
        declare_bad_file_replicas(self.args, self.client, self.logger)

    def temp_unavailable(self):
        declare_temporary_unavailable_replicas(self.args, self.client, self.logger)
