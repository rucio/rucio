...
class Rucio2:
    ...
    def parse_command():
        ...
        Rucio2.main_arguments(main_parser)
        Rucio2.auth_arguments(main_parser)

        # Verbs
        # TODO documentation table
        main_parser.add_argument("verb", choices={"add", "remove", "set", "unset", "list"}, nargs="?", default=None, help="Type of operation to execute. See <Table I'll link somewhere> for details.")

        # Main Commands:
        subparsers = main_parser.add_subparsers(dest="command", help="Command to execute, see `{command} -h` for more details and subcommands.")
    ...
    def main_arguments(parser_object):
        ...
        # TODO Documentation table
        main_args.add_argument("--format", dest="format", default="json", choices={"text", "rich", "json", "csv"}, help="Output format - view [Some table somewhere for details]")
    ...
    def run_command(self) -> tuple[int, Optional[Union[dict[str, Any], list[dict[str, Any]]]]]:
        oddball_commands = {"ping": self.ping, "whoami": self.whoami, "upload": Upload(self.client, self.args, self.logger), "download": Download(self.client, self.args, self.logger)}
    ...
    def __call__(self):
        ...
        exit_code, raw_result = self.run_command()
        if raw_result is not None:
            selected_result = self.select_output(raw_result)
            self.format_output(selected_result)
...
def main():
    parser_object = Rucio2.parse_command()
    if EXTRA_MODULES["argcomplete"]:
