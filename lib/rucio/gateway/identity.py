...
from rucio.db.sqla.constants import DBSessionOperation, IdentityType
...
def add_identity(
    ...
    """
    with db_session(DBSessionOperation.WRITE) as session:
...
def verify_identity(identity_key: str, id_type: str, password: Optional[str] = None) -> bool:
    ...
    """
    with db_session(DBSessionOperation.READ) as session:
