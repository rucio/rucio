...
matched_dids = FOREACH dids GENERATE time, REGEX_EXTRACT(request, '^(\\S+\\s+(\\/[^\\/]*){0,3})(.*)$', 1) as grp_uri, response_time, response_bytes;
matched_rest = FOREACH rest GENERATE time, REGEX_EXTRACT(request, '^(\\S+\\s+(\\/[^\\/]*){0,2})(.*)$', 1) as grp_uri, response_time, response_bytes;
