...
------------------------+-----------------------+---------------------------+
 -b / --purge-build     | drop_everything()     | build_database() +        |
                        ...
                        |                       | create_root_account()     |
------------------------+-----------------------+---------------------------+
 -s / --purge-stop      | drop_everything()     | nothing else..            |
...
if __name__ == '__main__':
    ...
    # ------------------------------------------------------------------
    if args.purge_build or args.purge_stop:
        drop_everything()  # Method should better be renamed to something like purge_db()
    else:
        destroy_database()  # Method should better be renamed to something like drop_orm_tables()

    # ------------------------------------------------------------------
    # 2. Decide what to rebuild
    # ------------------------------------------------------------------
    if not args.purge_stop:
        build_database()
        create_base_vo()
        create_root_account()
