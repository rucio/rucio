...
from rucio.tests.common import account_name_generator, execute, execute_cli, file_generator, rse_name_generator, scope_name_generator, with_each_cli_renderer
...
def test_account(rucio_client, file_config_mock):
    ...
    exitcode, _, err = execute_cli(command)
    assert exitcode == 0
    ...
    exitcode, out, err = execute_cli(command)
    assert exitcode == 0
    assert "ERROR" not in err
    ...
    execute_cli(f"rucio account add {tmp_account} USER")

    cmd = f"rucio account update {tmp_account} --ban"
    exitcode, _, new_ban_log = execute_cli(cmd)
    assert exitcode == 0
    assert "ERROR" not in new_ban_log

    cmd = f"rucio account update {tmp_account} --unban"
    exitcode, _, new_unban_log = execute_cli(cmd)
    assert exitcode == 0
...
def test_account_attribute(jdoe_account, file_config_mock):
    ...
    exitcode, _, log = execute_cli(cmd)
    assert exitcode == 0
...
def test_config(file_config_mock):
    ...
    exitcode, out, err = execute_cli(f"rucio config list --section {section} --key {option}")
    assert exitcode == 0
    assert "ERROR" not in err
    ...
    exitcode, out, err = execute_cli("rucio config list")
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_did(rucio_client, root_account, file_config_mock):
    ...
    exitcode, stdout, err = execute_cli(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_did_content(root_account, rucio_client, file_config_mock):
    ...
    execute_cli(f"rucio did content remove {scope}:{dataset} --from-did {scope}:{container}")
...
def test_rse_attribute(file_config_mock):
    ...
    _, _, err = execute_cli(f"rucio rse add {rse_name}")
    assert "ERROR" not in err
