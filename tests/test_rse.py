...
class TestRSECoreApi:
    ...
    def test_add_bidirectional_distance(self, vo):
        ...
        # remove one distance.
        delete_distances(src_rse_id=rse1_id, dest_rse_id=rse2_id)
        # add bidirectional distance again, should raise Duplicate exception.
        # as rse2 -> rse1 distance still exists.
        with pytest.raises(Duplicate):
            add_distance(src_rse_id=rse1_id, dest_rse_id=rse2_id, distance=2, bidirectional=True)
        dists = get_distances(src_rse_id=rse1_id, dest_rse_id=rse2_id)
        assert len(dists) == 0
        del_rse(rse1_id)
        del_rse(rse2_id)
...
class TestRSEClient:
    ...
    def test_add_protocol(self, vo, rucio_client):
        ...
        for p in resp['protocols']:
            if ((p['port'] == 19) and (p['domains']['lan']['read'] != 0)) or \
                    ((p['port'] == 20) and (p['domains']['lan']['read'] != 1)) or \
                    ((p['port'] == 18) and (p['domains']['lan']['read'] != 0)) or \
                    ((p['port'] == 17) and (p['domains']['lan']['read'] != 2)):
                print(resp)
                assert False
    ...
    def test_update_protocols_not_supported(self, rucio_client):
        ...
        try:
            with pytest.raises(exception.RSEProtocolNotSupported):
                rucio_client.update_protocols(protocol_rse, scheme='MOCK_UNDEFINED', hostname='localhost', port=17, data={'delete_lan': 0})
        finally:
            rucio_client.delete_rse(protocol_rse)
    ...
    def test_rsemgr_possible_protocols(self):
        ...
        rse_settings = {'availability_delete': True,
                        ...
                        'deterministic': True,
                        'domain': ['lan', 'wan'],
                        'protocols': [{'domains': {'lan': {'delete': 1, 'read': None, 'write': None},
                                                   'wan': {'delete': 1, 'read': 1, 'write': None}},
                                       ...
                                       'scheme': 'root'},
                                      {'domains': {'lan': {'delete': None, 'read': 0, 'write': None},
                                       ...
                                       'prefix': '//castor/ads.rl.ac.uk/prod/atlas/stripInput/atlasdatadisk/rucio/',
                                       'scheme': 'root'},
                                      {'domains': {'lan': {'delete': 0, 'read': None, 'write': 0},
