...
class TestOpenDataCLI:
    ...
    def test_opendata_cli_list_add_remove(self, mock_scope):
        ...
        # TODO: check the output matches what is expected for no opendata DIDs
        ...
        # TODO: deletion fails for some reason. It works locally, but not in the CI environment.
        ...
        # exitcode, stdout, stderr = execute("rucio opendata list --state draft")
        # assert exitcode == 0, f"Failed to list opendata with state draft after removal: {stderr.strip()}"
        # assert f"{name}" in stdout, f"Expected {mock_scope}:{name} not in opendata list with state draft after removal"
