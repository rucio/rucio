...
def test_replace_runs_new_callback_without_affecting_others() -> None:
    executed: list[str] = []
...
def test_multi_tag_runtime_runs_any_intersecting_check() -> None:
    ...
    assert 'daemon' in executed
    assert 'rest' in executed
    assert 'both' in executed
    assert 'global' in executed
...
def test_strict_mode_unknown_disabled_names_fail(config_lists) -> None:
    _register('alpha', lambda: None, tags={'daemon'})
    config_lists['disabled'] = ['ghost']
    config_lists.bools['strict'] = True
