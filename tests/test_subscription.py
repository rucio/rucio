...
from rucio.db.sqla.constants import AccountType, DatabaseOperationType, DIDType, RuleState, SubscriptionState
...
class TestSubscriptionCoreGateway:
    ...
    def test_list_subscription_rule_states_backward_compat(self, vo, rse_factory, root_account, scope_factory):
        ...
        assert result.state.value in ['R', 'O', 'S', 'U', 'W', 'I']  # Valid rule states
    ...
    def test_list_subscription_rule_states_with_details(self, vo, rse_factory, root_account, scope_factory):
        ...
        assert result['account'] == 'root'
        assert result['name'] == subscription_name
        assert result['state'].value in ['R', 'O', 'S', 'U', 'W', 'I']
        assert result['count'] == 1

        # Check subscription metadata fields
        assert 'subscription_state' in result
        assert result['subscription_state'] in [SubscriptionState.ACTIVE, SubscriptionState.INACTIVE, SubscriptionState.NEW, SubscriptionState.UPDATED, SubscriptionState.BROKEN]
...
def test_list_subscription_rule_states_without_details(vo, rse_factory, rest_client, auth_token, root_account):
    """SUBSCRIPTION (REST): Test list_subscription_rule_states without include_details parameter"""
    tmp_scope = InternalScope('mock_' + uuid()[:8], vo=vo)
    with db_session(DatabaseOperationType.WRITE) as session:
        add_scope(tmp_scope, root_account, session=session)
    ...
    rse, _ = rse_factory.make_mock_rse()
    subscription_name = uuid()
    comment_text = 'REST test without details'
    ...
    subid = add_subscription(
        ...
        account='root',
        filter_={'scope': [tmp_scope.external]},
        replication_rules=[{'rse_expression': rse, 'copies': 1, 'activity': activity}],
        lifetime=100000,
        retroactive=False,
        dry_run=False,
        comments=comment_text,
    ...
    # Test without include_details parameter (backward compatibility)
    response = rest_client.get(
        '/subscriptions/root/%s/rules/states' % subscription_name,
        headers=headers(auth(auth_token))
    )
    assert response.status_code == 200

    # Parse response - should have no details
    found = False
    for line in response.get_data(as_text=True).split('\n'):
        if line:
            data = loads(line)
            # Data should be array format for backward compatibility: [account, name, state, count]
            assert isinstance(data, list)
            assert len(data) >= 4
            if data[1] == subscription_name:
                assert data[0] == 'root'
                assert data[3] >= 1  # count
                found = True
                # Should NOT have extra fields in array format
                assert len(data) <= 5  # Only basic fields
                break
    assert found


def test_list_subscription_rule_states_with_details(vo, rse_factory, rest_client, auth_token, root_account):
    """SUBSCRIPTION (REST): Test list_subscription_rule_states with include_details=true"""
    tmp_scope = InternalScope('mock_' + uuid()[:8], vo=vo)
    with db_session(DatabaseOperationType.WRITE) as session:
        add_scope(tmp_scope, root_account, session=session)
    ...
    rse, _ = rse_factory.make_mock_rse()
    subscription_name = uuid()
    comment_text = 'REST test with details'
    ...
    # Test with include_details=true
    response = rest_client.get(
        '/subscriptions/root/%s/rules/states?include_details=true' % subscription_name,
        headers=headers(auth(auth_token))
    )
    assert response.status_code == 200

    # Parse response - should have details
    found = False
    for line in response.get_data(as_text=True).split('\n'):
        if line:
            data = loads(line)
            # Data should be dict format when include_details=true
            assert isinstance(data, dict)
            if data.get('name') == subscription_name:
                # Check basic fields
                assert data['account'] == 'root'
                assert data['count'] >= 1
                assert 'state' in data

                # Check subscription metadata fields
                assert 'subscription_state' in data
                assert 'last_processed' in data
                assert 'lifetime' in data
                assert 'expired_at' in data
                assert 'comments' in data
                assert data['comments'] == comment_text
                found = True
                break
    assert found
...
class TestSubscriptionClient:
    ...
    def test_list_subscription_rule_states_without_details(self, vo, rse_factory, rucio_client, root_account, scope_factory):
        """SUBSCRIPTION (CLIENT): Test list_subscription_rule_states without include_details"""
        _, scopes = scope_factory([vo])
        tmp_scope = scopes[0]

        rse, _ = rse_factory.make_mock_rse()
        subscription_name = uuid()
        comment_text = 'Test client subscription'
        ...
        rucio_client.add_subscription(
            ...
            account='root',
            filter_={'scope': [tmp_scope.external]},
            replication_rules=[{'rse_expression': rse, 'copies': 1, 'activity': self.activity}],
            lifetime=100000,
            retroactive=False,
            dry_run=False,
            comments=comment_text
        ...
        # Get subscription ID to create rule
        subs = list(rucio_client.list_subscriptions(name=subscription_name, account='root'))
        assert len(subs) == 1
        sub_id = subs[0]['id']
        ...
        # Test without include_details (default)
        results = list(rucio_client.list_subscription_rule_states(account='root', name=subscription_name))
    ...
    def test_list_subscription_rule_states_with_details(self, vo, rse_factory, rucio_client, root_account, scope_factory):
        """SUBSCRIPTION (CLIENT): Test list_subscription_rule_states with include_details=True"""
        _, scopes = scope_factory([vo])
        tmp_scope = scopes[0]

        rse, _ = rse_factory.make_mock_rse()
        subscription_name = uuid()
        comment_text = 'Test client subscription with details'
        ...
        # Test with include_details=True
        results = list(rucio_client.list_subscription_rule_states(
            account='root',
            name=subscription_name,
            include_details=True
        ))
        ...
        # Should HAVE subscription metadata fields when include_details=True
        assert 'subscription_state' in result
        assert 'last_processed' in result
        assert 'lifetime' in result
        assert 'expired_at' in result
        ...
        # Verify the values
        assert result['comments'] == comment_text
        assert result['subscription_state'] in ['ACTIVE', 'INACTIVE', 'NEW', 'UPDATED', 'BROKEN']
