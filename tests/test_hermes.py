...
from rucio.tests.common import rse_name_generator, skip_missing_elasticsearch_influxdb_in_env, skip_missing_kafka_in_env
...
@pytest.mark.noparallel(reason="fails when run in parallel")
@skip_missing_kafka_in_env
@pytest.mark.parametrize(
    "core_config_mock",
    [{"table_content": [
        ("hermes", "services_list", "kafka"),
        ("messaging-hermes-kafka", "use_ssl", False),
        ("messaging-hermes-kafka", "brokers", "kafka:9092"),
        ("messaging-hermes-kafka", "topic", "hermeskafka"),
    ]}],
    indirect=True,
)
@pytest.mark.parametrize("caches_mock", _KAFKA_CACHES, indirect=True)
def test_hermes_kafka_integration(core_config_mock, caches_mock):
    """HERMES (DAEMON): deliver to a real Kafka broker and read it back"""
    ...
    topic = config_get("messaging-hermes-kafka", "topic")
    consumer = Consumer({
        "bootstrap.servers": brokers,
        "group.id": f"hermes-test-{uuid.uuid4().hex}",  # fresh group -> reads from the beginning
    ...
    try:
        while len(received) < nb_messages and time.time() < deadline:
            msg = consumer.poll(1.0)
            if msg is None or msg.error():
                continue
            body = loads(msg.value())
            # The topic persists across runs, so filter on this run's unique RSE
            if body["payload"].get("rse") == mock_rse:
                received.append(body)
    finally:
        consumer.close()

    assert len(received) == nb_messages
    assert all(m["event_type"] == "transfer-done" for m in received)
    got = sorted((m["payload"]["name"], m["payload"]["bytes"]) for m in received)
    assert got == [(f"file_{i}", 100 + i) for i in range(nb_messages)]
