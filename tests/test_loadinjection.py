...
from rucio.db.sqla.constants import DatabaseOperationType, LoadInjectionState, LockState
from rucio.db.sqla.models import AccountType, DatasetLock, DIDType
from rucio.db.sqla.session import Session, db_session, transactional_session
...
def generate_random_plans(
    ...
    for _ in range(nplan):
        plan = {
            ...
            # INJECTING is excluded: tests that need it set the state
            # explicitly. Random INJECTING plans made cleanup deletes
            # flaky (delete refuses INJECTING plans).
            "state": random.choice(
                [s for s in LoadInjectionState
                 if s != LoadInjectionState.INJECTING]
            ),
...
class TestPlanCore:
    def test_add_injection_plan(self):
        ...
        assert not plan["big_first"]
        assert plan["rule_lifetime"] == 3600
        assert plan["comments"] is None
    ...
    def test_delete_injection_plan(self):
        ...
        new_plan = generate_random_plans()[0]
        # A plan in INJECTING state cannot be deleted; force a deletable state.
...
@pytest.mark.noparallel(reason="creates many replication rules; enqueues UpdatedCollectionReplica rows shared with abacus daemon tests")
@skip_multivo
class TestDatasetCore:
    ...
    def test_validate_unique_rse_pair_dataset(self):
        ...
        generate_random_extra_rules(datasets, rse_ids)
        for src_rse_id in rse_ids:
            for dest_rse_id in rse_ids:
                if src_rse_id == dest_rse_id:
                    continue
...
class TestPlanPermission:
    ...
    def test_deny_add_plan_non_root(self, vo, rucio_client):
        ...
        with db_session(DatabaseOperationType.WRITE) as session:
            add_account(test_account, AccountType.USER, f"{account_name_generator()}@test.com", session=session)
    ...
    def test_deny_delete_plan_non_root(self, vo, rucio_client):
        ...
        test_account = InternalAccount(account_name_generator())
        with db_session(DatabaseOperationType.WRITE) as session:
            add_account(test_account, AccountType.USER, f"{test_account}@test.com", session=session)
...
class TestDaemon:
    ...
    def test_submitter_executes_plan_to_completion(self):
        ...
        logger_fn = logging.getLogger("test_submitter").log
...
class TestMonteCarloScanner:
    ...
    def test_scanner_distinct_under_multi_lock(self):
        ...
        with db_session(DatabaseOperationType.WRITE) as session:
            add_account(account2, AccountType.USER, str(account2) + "@test.com", session=session)
        # Give account2 quota on the target RSE
        with db_session(DatabaseOperationType.WRITE) as session:
            set_local_account_limit(account2, src, 10**15, session=session)
...
class TestCLIIntegration:
    """Integration tests exercising the CLI → Client → Core path.

    Uses subprocess invocation of the real `rucio` binary (as upstream
    test_bin_rucio.py does) rather than Click's in-process CliRunner,
    because the CLI framework registers signal handlers in its entrypoint,
    which only works on the main thread of an interpreter — not guaranteed
    under pytest-xdist workers.
    """

    @staticmethod
    def _invoke(*args: str) -> tuple[int, str]:
        """Invoke `rucio` in a subprocess; return (exitcode, combined_output)."""
        cmd = "rucio " + " ".join(args)
        exitcode, out, err = execute(cmd)
        return exitcode, out + err

    def test_cli_add_test_mode(self, vo):
        """CLI INTEGRATION: add --test prints JSON, creates no plan"""
        rse1 = rse_name_generator()
        rse2 = rse_name_generator()
        rse_id1 = add_rse(rse1, vo=vo)
        rse_id2 = add_rse(rse2, vo=vo)

        exitcode, output = self._invoke(
            "loadinjection", "add",
    ...
    def test_cli_add_and_list(self, vo):
        ...
        # Verify via list
        _, output = self._invoke("loadinjection", "list")
        assert "cli-test" in output
        assert rse1 in output

        # Cleanup
        delete_injection_plan(rse_id1, rse_id2)

    def test_cli_list_empty(self, vo):
        ...
        _, output = self._invoke("loadinjection", "list",
                                 "--src-rse", rse1, "--dest-rse", rse2)
        assert "No load injection plans found" in output

    def test_cli_info(self, vo):
        ...
        _, output = self._invoke("loadinjection", "info",
                                 "--src-rse", rse1, "--dest-rse", rse2)
        assert plan["plan_id"] in output

        delete_injection_plan(rse_id1, rse_id2)

    def test_cli_remove(self, vo):
        ...
        exitcode, _ = self._invoke("loadinjection", "remove",
                                   "--src-rse", rse1, "--dest-rse", rse2)
        assert exitcode == 0
        state = get_injection_plan_state(rse_id1, rse_id2)
        assert state is None

    def test_cli_update(self, vo):
        ...
        exitcode, _ = self._invoke("loadinjection", "update",
                                   "--src-rse", rse1, "--dest-rse", rse2,
                                   "--inject-rate", "777")
        assert exitcode == 0
    ...
    def test_cli_kill(self, vo):
        ...
        exitcode, _ = self._invoke("loadinjection", "kill",
                                   "--src-rse", rse1, "--dest-rse", rse2)
        assert exitcode == 0
    ...
    def test_cli_validation_errors(self, vo):
        ...
        exitcode, _ = self._invoke("loadinjection", "add",
                                   "--src-rse", rse1, "--dest-rse", rse2,
                                   "--interval", "0")
        assert exitcode != 0
...
class TestDaemonIntegration:
    ...
    @staticmethod
    def _require_xrd_rses(*names: str) -> None:
        """Skip the test when the XRD storage RSEs are not registered.

        The XRD RSEs only exist in the dev docker-compose storage profile;
        the autotest CI matrix does not provide them.
        """
        for name in names:
            try:
                get_rse_id(name)
            except RSENotFound:
                pytest.skip(f"RSE {name} not available (requires storage profile)")

    def test_scanner_finds_unique_datasets_on_xrd(self):
        """DAEMON INTEGRATION: scanner finds unique datasets between XRD1 and XRD3"""
        self._require_xrd_rses("XRD1", "XRD3")
        src = get_rse_id("XRD1")
    ...
    def test_submitter_with_real_plan_dry_run(self):
        ...
        from rucio.daemons.loadinjector.submitter import plan_submitter
        logger_fn = logging.getLogger("test_daemon_int").log
        plan_submitter(plan, logger_fn)
    ...
    def test_submitter_kill_works(self):
        ...
        self._require_xrd_rses("XRD2", "XRD4")
        src_name = "XRD2"
        dest_name = "XRD4"
        src_id = get_rse_id(src_name)
        dest_id = get_rse_id(dest_name)
    ...
    @skip_multivo(reason="test requires single-VO RSE visibility")
...
class TestMultiVOIsolation:
    """Multi-VO isolation: plans are scoped by the VO of their RSEs.

    These tests only run in the multi_vo suite — the ``second_vo`` fixture
    skips otherwise. They validate that the load injector correctly isolates
    plans between VOs rather than leaking them across the VO boundary.
    """

    def test_plan_invisible_from_other_vo(self, vo, second_vo):
        """MULTI VO: a plan on VO-A RSEs is not returned when querying VO-B"""
        src_rse_id = add_rse(rse_name_generator(), vo=vo)
        dest_rse_id = add_rse(rse_name_generator(), vo=vo)
        ...
        try:
            # Querying with the plan's own VO returns it (RSE-join VO filter)
            own_ids = {p["plan_id"] for p in get_injection_plans(vo=vo)}
            assert plan["plan_id"] in own_ids

            # Querying with a different VO must NOT return it
            other_ids = {p["plan_id"] for p in get_injection_plans(vo=second_vo)}
            assert plan["plan_id"] not in other_ids

            # The unscoped daemon view (vo=None) still sees every plan
            all_ids = {p["plan_id"] for p in get_injection_plans()}
            assert plan["plan_id"] in all_ids
        finally:
            delete_injection_plan(src_rse_id, dest_rse_id)

    def test_get_plan_vo_mismatch_not_found(self, vo, second_vo):
        """MULTI VO: fetching a single plan under the wrong VO raises NotFound"""
        src_rse_id = add_rse(rse_name_generator(), vo=vo)
        dest_rse_id = add_rse(rse_name_generator(), vo=vo)
        ...
        try:
            # Visible under its own VO
            found = get_injection_plan(src_rse_id, dest_rse_id, vo=vo)
            assert found["plan_id"] == plan["plan_id"]

            # Not visible under a different VO
            with pytest.raises(NoLoadInjectionPlanFound):
                get_injection_plan(src_rse_id, dest_rse_id, vo=second_vo)
        finally:
            delete_injection_plan(src_rse_id, dest_rse_id)
