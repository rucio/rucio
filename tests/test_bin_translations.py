...
def test_account(rucio_client):
    ...
    command = f"rucio account set --account {new_account} --key email --value {update_email}"
    exitcode, _, err = execute(command)
    assert exitcode == 0
    assert "ERROR" not in err
    assert update_email == [account["email"] for account in rucio_client.list_accounts() if account["account"] == new_account][0]

    command = "rucio account usage --account root"
    exitcode, _, err = execute(command)
    assert exitcode == 0
    assert "ERROR" not in err
    ...
    command = f"rucio account info --account {new_account}"
    exitcode, out, err = execute(command)
    assert exitcode == 0
    assert "ERROR" not in err
    assert new_account in out
...
def test_account_ban():
    ...
    cmd = f"rucio account ban add --account {tmp_account}"
    exitcode, _, new_ban_log = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in new_ban_log

    cmd = f"rucio account ban remove --account {tmp_account}"
...
def test_config():
    ...
    cmd = f"rucio config set --section {section} --option {option} --value {value}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_did(rucio_client, root_account):
    ...
    cmd = f"rucio did touch --did {scope}:{dataset}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err

    cmd = f"rucio did --did {scope}:* --filter type=all"
    exitcode, out, err = execute(cmd)
    assert exitcode == 0
    assert f"{scope}:{dataset}" in out
    assert "ERROR" not in err

    cmd = f"rucio did stat --did {scope}:{dataset}"
    exitcode, out, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
    assert dataset in out  # At least list the name properly

    cmd = f"rucio did attach --did {scope}:{dataset} --to {scope}:{container}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err

    cmd = f"rucio did parent --did {scope}:{dataset}"
    exitcode, out, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
    assert container in out

    cmd = f"rucio did detach --did {scope}:{dataset} --from {scope}:{container}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_did_content(root_account, rucio_client):
    ...
    execute(f"rucio did detach --did {scope}:{dataset} --from {scope}:{container}")
    ...
    cmd = f"rucio did content close --did {scope}:{container}"
    exitcode, out, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
    ...
    cmd = f"rucio did content open --did {scope}:{container}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_did_container(rucio_client, root_account):
    scope = scope_name_generator()
    rucio_client.add_scope(account=root_account.external, scope=scope)
    dataset = file_generator().split("/")[-1]

    cmd = f"rucio did container add --did {scope}:{dataset}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_did_dataset(root_account, rucio_client):
    scope = scope_name_generator()
    rucio_client.add_scope(account=root_account.external, scope=scope)
    dataset = file_generator().split("/")[-1]

    cmd = f"rucio did dataset add --did {scope}:{dataset}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_lifetime_exception(rucio_client, mock_scope):
    ...
    cmd = f"rucio lifetime-exception -f {input_file.name} --reason mock_test -x 2100-12-30"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    if "ERROR" not in err:
...
def test_rse(rucio_client):
    ...
    cmd = f"rucio rse set --rse {rse_name} --setting city --value {value}"
    exitcode, out, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_rule(rucio_client, mock_scope):
    ...
    move_rse = "MOCK2"
    cmd = f"rucio rule set --rule-id {rule_id} --move --rse {move_rse}"
    exitcode, out, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err

    cmd = f"rucio rule set --rule-id {rule_id} --priority 3"
    exitcode, out, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
...
def test_subscription(rucio_client, mock_scope):
    ...
    rule = json.dumps({})
    cmd = f"rucio subscription set --name {subscription_name} --filter '{filter_}' --rule {rule}"
    exitcode, _, err = execute(cmd)
    assert exitcode == 0
    assert "ERROR" not in err
